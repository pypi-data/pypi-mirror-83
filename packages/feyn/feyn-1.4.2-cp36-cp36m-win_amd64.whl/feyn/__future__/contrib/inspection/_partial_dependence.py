...
def plot_partial_dependence(graph:feyn.Graph, data:Iterable, by:str, fixed:Optional[dict] = None) -> None:
    ...
    if data[by].dtype == 'object':
        support = data[target] 
        ax_scatter.scatter(support, data[by], alpha = 0.3, color = 'grey')
        ...
        for i, group in enumerate(total_df.id.sort_values().unique()):
            ...
            ax_scatter.scatter(scatter_df['preds'], scatter_df[by], color = palette(i%number_of_colors), label = group)
        ax_scatter.set_xlabel(f"Predicted {target}")
        ax_scatter.legend(title = cols, loc='center left', bbox_to_anchor=(1.35, 0.5))

        ax_histx.hist(support, bins=30, color = 'grey', rwidth=0.9, alpha =.6)
        ax_histy.hist(data[by], bins=30, color = 'grey', rwidth=0.9, alpha =.6, orientation='horizontal')
        ...
        for i, group in enumerate(total_df.id.unique()):
            ...
            ax_scatter.plot(line_df[by], line_df['preds'], color = palette(i%number_of_colors), label = group, linewidth = 1.5)
