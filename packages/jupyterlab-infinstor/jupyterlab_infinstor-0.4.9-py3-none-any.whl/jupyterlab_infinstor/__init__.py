...
class AddXformHandler(APIHandler):  # pylint: disable=abstract-method
    def process_xformcode(self, xfin):
        ...
        # Dockerfile is provided as follows
        # """
        # %infinstor-dockerfile
        # blah
        # blah
        # %infinstor-dockerfile
        # """
        # We first extract dockerfile from within %infinstor-dockerfile
        # Note: this leaves the empty tripe quotes block in clean_code
        # The empty triple quote block is removed in the next code section
        clean_code = ''
        mode = 0
        dockerfile = ''
        ...
        # Here we remove the empty triple quote block
        code_str_lines = clean_code.splitlines()
        clean_code1 = ''
        prev_line_was_triple_quote = False
        for one_line in code_str_lines:
            if (one_line.lstrip() == '"""'):
                if (prev_line_was_triple_quote):
                    prev_line_was_triple_quote = False
                else:
                    prev_line_was_triple_quote = True
            else:
                if (prev_line_was_triple_quote):
                    clean_code1 += '"""\n'
                clean_code1 += (one_line + '\n')
                prev_line_was_triple_quote = False

        quoted_clean_code = quote(clean_code1)
