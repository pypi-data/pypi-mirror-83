...
class Encryption(object):
	...
	def encrypt_directory(self, path):
		...
		# encrypt zip.
		response = self.encrypt_file(zip.file_path.path)
		if response["success"]:
			response["success"] = True
	...
	def decrypt_directory(self, path):
		...
		# decrypt zip.
		response = self.decrypt_file(zip_path)
...
class EncryptedDictionary(Files.Dictionary):
	...
	def __init__(self,
		...
		if not self._file_.file_path.exists():
			self._file_.save(json.dumps(default, indent=4, ensure_ascii=False))
			response = self.encryption.encrypt_file(self.file_path.path)
			created = True
		elif load:
			self.load()
		if not created and default != None:
			if self.dictionary == None: self.load()
			self.check(default=default)

		#
	def load(self):
		encrypted = self._file_.load()
		response = self.encryption.decrypt_string(encrypted)
		if not response["success"]:
			raise ValueError(response["error"])
		self.dictionary = json.loads(response["decrypted"])
		return self.dictionary
	def save(self, dictionary=None):
		if dictionary == None: dictionary = self.dictionary
		response = self.encryption.encrypt_string(json.dumps(dictionary, indent=4, ensure_ascii=False))
		if not response["success"]:
			raise ValueError(response["error"])
		self._file_.save(response["encrypted"])
	def check(self, default=None, save=True):
		if not isinstance(default, dict): raise ValueError("Parameter [default] must be a dictionary.")
