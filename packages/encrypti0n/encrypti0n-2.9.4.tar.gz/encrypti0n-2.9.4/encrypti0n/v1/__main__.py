...
class CLI(object):
	def __init__(self, alias=None):
		...
		self.options={
			...
			"-p / --passphrase 'Passphrase123!'":"Specify the key passphrase.",
		}
		self.alias = ALIAS
	...
	def start(self):
		...
		# encrypt.
			...
			if os.path.isdir(file): encryption.encrypt_directory(file)
			else: encryption.encrypt_file(file)
		...
		# decrypt.
			...
			encryption = Encryption(key=key, passphrase=passphrase)
			encryption.load_keys()
			if os.path.isdir(file): encryption.decrypt_directory(file)
			else: encryption.decrypt_file(file)
