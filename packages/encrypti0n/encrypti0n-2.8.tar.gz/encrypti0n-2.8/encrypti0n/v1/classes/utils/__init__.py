...
import os, sys, ast, json, pathlib, glob, string, getpass
...
# get network info.
def __get_network_info__(silent=False):
	loader = None
	...
	try: 
		...
		del(info["ip"])
		info["public_ip"] = x
	except KeyError: print("Unable to fetch netork info.")
	try: info["private_ip"] = __get_private_ip__()
	except: info["private_ip"] = "unkown"
	try: info["hostname"] = socket.gethostname()
	except: info["hostname"] = "unkown"
	return info
def __get_private_ip__():
