...
__all__ = ['PriorityQueue', 'PriorityDict', 'SimplePriorityQueue', 'SimplePriorityDict']
...
class PriorityQueue:
    ...
    def set_empty_return(self, empty_return=_DEFAULT_MARKER):
        self._empty_return = empty_return

    def _default_or_error(self, default, errors):
        if default is _DEFAULT_MARKER:
            if self._empty_return is _DEFAULT_MARKER:
                raise errors
            else:
    ...
    # args: i.e. [key,] value
    def _wrap(self, *args, prior=_DEFAULT_MARKER):
        if self.no_ind_prior:
            prior = prior if prior is not _DEFAULT_MARKER else args[-1] if self.v_is_p else args[-1][self.p_key]
            prior_wrap = self._prior_wrap(prior)
            # return [*args, prior_wrap, len(self)]
        else:
            prior = None if prior is _DEFAULT_MARKER else prior
            prior_wrap = self._prior_wrap(prior)
            # return [*args, prior, prior_wrap, len(self)]
        return [*args, prior, prior_wrap, len(self)]

    def _unwrap(self, wrapper, fmt=_DEFAULT_MARKER, skip=0):
        fmt = self.format_ou() if fmt is _DEFAULT_MARKER else fmt
        idx = self.var_idx
    ...
    def peek_wrap(self, default=_DEFAULT_MARKER):
        try:
            return self.heap[0]
        except IndexError as errors:
            return self._default_or_error(default, errors)

    def peek(self, default=_DEFAULT_MARKER):
        try:
            return self._unwrap(self.heap[0])
        except IndexError as errors:
            return self._default_or_error(default, errors)

    def push(self, *args, prior=_DEFAULT_MARKER, fmt=_DEFAULT_MARKER):
        args_ln = len(args)
        if not 0 < args_ln < 3:
        ...
        fmt_in = self.format_in() if fmt is _DEFAULT_MARKER else fmt

        if args_ln == 1:
            if fmt_in and not self.no_ind_prior:  # confirm args[0] is iterable
                args = args[0]
                if len(args) > 1:
                    prior = prior if prior is not _DEFAULT_MARKER else args[-1]
                else:
                    args = (args[0], self._none_wrap)
                    prior = prior if prior is not _DEFAULT_MARKER else None
            else:
                prior = prior if prior is not _DEFAULT_MARKER else (
...
class PriorityDict(PriorityQueue, MutableMapping):
    ...
    def get_wrap(self, key, default=_DEFAULT_MARKER):
        try:
            return self.dic[key]
        except (KeyError, IndexError) as errors:
            return self._default_or_error(default, errors)

    def get(self, key, default=_DEFAULT_MARKER):
        try:
            return self[key]
        except (KeyError, IndexError) as errors:
            return self._default_or_error(default, errors)

    def peek_wrap(self, key=_DEFAULT_MARKER, default=_DEFAULT_MARKER):
        try:
            return self.heap[0] if key is _DEFAULT_MARKER else self.dic[key]
        except (KeyError, IndexError) as errors:
            return self._default_or_error(default, errors)

    def peek(self, key=_DEFAULT_MARKER, default=_DEFAULT_MARKER):
        try:
            return self._unwrap(self.heap[0]) if key is _DEFAULT_MARKER else \
                self._unwrap(self.dic[key])
        except (KeyError, IndexError) as errors:
            return self._default_or_error(default, errors)

    def pop_wrap(self, key=_DEFAULT_MARKER, default=_DEFAULT_MARKER):
        try:
            if key is _DEFAULT_MARKER:
                return self.popitem_wrap()
            else:
    ...
    def pop(self, key=_DEFAULT_MARKER, default=_DEFAULT_MARKER):
        try:
            if key is _DEFAULT_MARKER:
                return self.popitem()
            else:
    ...
    def push(self, *args, prior=_DEFAULT_MARKER, fmt=_DEFAULT_MARKER):
        ...
        if len(args) == 1:
            if prior is _DEFAULT_MARKER:
                if key in self.dic:
                    return
                else:
                    prior = None
            value = _DEFAULT_MARKER
        else:
            value = args  # check format_ou move to __setitem__
            prior = prior if prior is not _DEFAULT_MARKER else args[-1] if len(args) == 3 \
    ...
    def __setitem__(self, key, value=_DEFAULT_MARKER, prior=_DEFAULT_MARKER, fmt=_DEFAULT_MARKER):
        _not_new = key in self.dic

        if prior is not _DEFAULT_MARKER:  # from push
            ...
            _dft_pri = False
            value = (key, value)
        elif self.format_in() if fmt is _DEFAULT_MARKER else fmt:  # que[key] = (value, [prior])
