...
TaskQue(
    ...
    auto_key=False,         auto generate unique key for 'put' or not
    ...
    time_limit=None,        every data expire after 'time_limit' if 'use_coroutine'
...
import threading
from .priority_datatypes import PriorityQueue, PriorityDict
...
class QueBase:
    ...
    def get(self, key=_DEFAULT_MARKER, block=True, timeout=None, wait_free=None, **kwargs):
        with self.not_empty:
            _s_time = _time()
            if wait_free is None:
    ...
    def get_nowait(self, key=_DEFAULT_MARKER, **kwargs):
        return self.get(key=key, block=False, **kwargs)

    def peek(self, key=_DEFAULT_MARKER, block=True, timeout=None):
        with self.not_empty:
            _flag = self.not_empty.wait_for(self._get_enabled, timeout) if block else self._qsize()
            if not _flag:
                raise TimeoutError if self._frozen else self.Empty
    ...
    def peek_nowait(self, key=_DEFAULT_MARKER):
    ...
    def _get(self, key=_DEFAULT_MARKER, **kwargs):
        return self.queue.popleft()

    # Peek an item from the queue
    def _peek(self, key=_DEFAULT_MARKER):
        pass
...
class TaskQue(QueBase):
    def _init(self, maxsize=0, auto_key=False, use_retain=True, use_trace=True, default_cb=None,
    ...
    def _get(self, key=_DEFAULT_MARKER, typ=None):
        kvp = self.queue.pop(key=key)
        dic = self.data[kvp[0]]

        _key = key or kvp[0]
        _typ = typ or ('get' if key is _DEFAULT_MARKER else 'drop')
