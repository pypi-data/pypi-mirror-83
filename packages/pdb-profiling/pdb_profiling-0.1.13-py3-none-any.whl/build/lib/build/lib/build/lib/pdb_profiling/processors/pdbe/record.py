...
class SIFTS(PDB):
    ...
    @unsync
    async def pipe_score(self, sifts_df=None, weight=array([1, -1, -1, -1.79072623, -2.95685934, -4.6231746])):
        if sifts_df is None:
            try:
    ...
    @unsync
    async def pipe_score_unit(self, sifts_df, pec_df, weight):
    ...
    async def pipe_score_for_pdb_entry(self, sifts_df, exp_cols, weight):
        ...
        full_df, s_df = await self.pipe_score_unit(sifts_df, pec_df, weight)
