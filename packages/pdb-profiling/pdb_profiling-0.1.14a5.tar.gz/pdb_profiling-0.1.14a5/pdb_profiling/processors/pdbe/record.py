...
class PDB(Base):
    ...
    @unsync
    async def set_assembly(self, focus_assembly_ids:Optional[Iterable[int]]=None, discard_multimer_chains_cutoff:int=26):
        '''
        ...
        NOTE Discard multimer contains more then 26 chains (except water | bound | carbohydrate_polymer)
        '''
...
class PDBInterface(PDBAssemble):
    ...
    async def set_interface_res(self, keep_interface_res_df:bool=False):
        ...
        if len(check) != 0:
            ...
            warn(f"Unexpected Data in Residue DataFrame: \n{check.head(3)[['pdb_id', 'struct_asym_id', 'author_residue_number']].to_dict('index')}", PISAErrorWarning)
            interfacedetail_df = interfacedetail_df.drop(index=check.index)
