...
def hitBhoonidhi(jsons):
    ...
    for j in jsons:
        i=i+1
        response = requests.post('https://bhoonidhi.nrsc.gov.in/bhoonidhi/ProductSearch', json=j, timeout=5)
        try:
            output.append(response.json()['Results'])
        except:
            pass
    return output
...
def bhoonidhiDownload(product_obj, user_id, password, output_path=''):
    ...
    if sat == 'L8':
        sen = 'O'
         ...
         sen = product_obj['OTS_SATELLITE']
    year = product_obj['OTS_DATE_OF_DUMPING'].split('-')[-1]
    month = product_obj['OTS_DATE_OF_DUMPING'].split('-')[1]
    prdId = product_obj['OTS_PRODUCTID']
