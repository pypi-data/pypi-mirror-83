...
def bhoonidhiSmartEventSearch(user_text, user_tokens):
    def hasNumbers(inputString):
        return bool(re.search(r'\d', inputString))
    init_date, final_date, user_text, feature_count = findDates(user_text,0)
...
def bhoonidhiSmartSearch(user_text, text_based=True,write_json = False):
    ...
    if re.search(r'flood',user_text) or re.search(r'avalanche',user_text):
        return bhoonidhiSmartEventSearch(user_text, user_tokens)
    
    ################################################################## Finding geo-locations in the string
    location = findCities(user_text,text_based=True)
    feature_count+=len(location)
   
    ######################################################################## Finding satellite in the tokens
    try:
        SatSen, user_pos, feature_count = findSatSen(user_text, user_pos, feature_count)
    except Exception as e:
        logging.error(str(e))
        raise Exception(f'Error finding Satellite and Sensor configuration in the text: {e}')
