...
#            if not hasNumbers(date[0]) and date[0] not in ['month','week','today','yesterday','tomorrow','fornite','year']:
#                user_text = user_text.replace(date[0],f'1st {date[0]} till 31st {date[0]}')
        dates = search_dates(user_text)
#        print(dates)
        if dates != None:
            dates = np.array(dates)
            ...
            if len(dates) > 1:
                init_date = dates.min().strftime("%d %B, %Y")
                final_date = dates.max().strftime("%d %B, %Y")
                feature_count+=2
            elif len(dates) == 1:
                init_date = dates[0].strftime("%d %B, %Y")
                final_date = dates[0].strftime("%d %B, %Y")
                feature_count+=1
            else:
                init_date = (datetime.datetime.now() - datetime.timedelta(days=10)).strftime("%d %B, %Y")
...
def findSatSen(user_text, feature_count):
    ...
    for sat in SATS_AVAIL:
        if re.search(sat,user_text):
            user_sat.append(sat)
            user_text = user_text.replace(sat,'')
            feature_count+=1
    for sen in SENS_AVAIL:
        if re.search(sen,user_text):
            user_sen.append(sen)
            user_text = user_text.replace(sen,'')
            feature_count+=1
    for sat in SATS_STRONG:
        if re.search(sat,user_text) and sat not in user_sat:
            user_sat_guess.append(sat)
    for sen in SENS_STRONG:
        if re.search(sen,user_text) and sen not in user_sen:
