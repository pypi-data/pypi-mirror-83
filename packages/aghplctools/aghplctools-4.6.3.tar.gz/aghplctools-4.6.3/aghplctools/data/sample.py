...
class HPLCSampleInfo(object):
    ...
    def get_values_from_xml(cls, xml_path: Union[str, pathlib.Path]) -> dict:
        """
        Attempts to find a Result.xml file and parse sample information from that. This is an old-style ChemStation
        metadata file (~B.04.03 era).

        :param xml_path: path to xml or directory containing xml file
        """
        if isinstance(xml_path, pathlib.Path) is False:
            xml_path = pathlib.Path(xml_path)
        if xml_path.is_dir():
            try:
                xml_path = list(xml_path.glob('Result.xml'))[0]
            except IndexError:
                raise FileNotFoundError(f'no Result.xml files could be found in the directory {xml_path}')
