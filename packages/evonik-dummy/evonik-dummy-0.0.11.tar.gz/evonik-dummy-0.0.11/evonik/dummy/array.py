...
class Array(Dummy):
    ...
    def _invalids(self):
        ...
        if self.min_items and self.min_items > 0:
            invalids.append((self._valid_list(self.min_items - 1), "too short"))
        # too long
        if self.max_items:
            invalids.append((self._valid_list(self.max_items + 1), "too long"))
        # invalid item
        if self._valid_length_range()[1] > 0:
            ...
            if len(inv) > 0:
                ...
                invalid_[-1] = inv[0]
                invalids.append((invalid_, "invalid item"))
        # duplicate items
        if self.unique_items:
            if self._valid_length_range()[1] > 1:
                invalid_ = self._valid_list(self._valid_length_range()[1])
                invalid_[-1] = invalid_[0]
                invalids.append((invalid_, "duplicate item"))
