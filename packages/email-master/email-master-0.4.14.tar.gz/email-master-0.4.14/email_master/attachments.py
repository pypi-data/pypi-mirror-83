...
class EmailAttachmentList(object):
    ...
    def filter_by_filename(self, filenames):
        ...
        """
        def do_filter(attch):
            for fn_endings in filenames:  # Filter out attachments that end with any in the list
                return not bool(unicode(attch.filename).lower().endswith(fn_endings))

        new_a = filter(do_filter, self.attachments)
        eal = EmailAttachmentList()
        eal.attachments = list(new_a)
        return eal
