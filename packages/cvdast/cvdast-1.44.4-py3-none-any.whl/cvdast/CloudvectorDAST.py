...
class CloudvectorDAST(object):
    ...
    def create_fuzz_test_methods(self, cv_events):
        ...
        shutil.copytree(os.path.join(templates_dir, "data"), "tests/data/", dirs_exist_ok=True)
        ...
        for collection in params_collection:
            for k, v in collection.items():
                ...
                if type(v) is list:
                    ...
                    if input_params.get(k,{}).get(k) and v > input_params.get(k, {}).get(k, 0):
                        processed_params.append(k)
                        if input_params:
                            new_collection = deepcopy(collection)
                            new_collection["$TEST_ID"] = None
    ...
    def generate_auto_input_params_file():
        ...
        for api, info in f1.items():
            ...
            if api not in input_params:
                input_params[api] = {"200": {}, "201": {}, "202": {}, "204": {}, "400": {}, "401": {}}
            data_set = []
            for method, method_info in info.items():
                for status in ["200", "201", "202", "204"]:
                    data_set_for_status = CloudvectorDAST.create_test_data_sets(api, method, status)
                    if data_set_for_status:
                        data_set+=data_set_for_status
                    #print(status, data_set)
