...
class CStreetData(object):
    ...
    class params_object:
        ...
        __slots__=("__output_dir","__output_name","__cell_cluster_pca_n","__cell_cluster_knn_n","__cell_cluster_resolution",
            ...
            "__max_outgoing","__min_cell_number","__random_state","__sampling_n","__figsize","__char_n")
        ...
        def __str__(self):
            ...
            s+=f"\n#Step6: calculate probability#\n"
            s+=f"random_state={self.__random_state}\n"
            s+=f"sampling_n={self.__sampling_n}\n"
            
            s+=f"\n#Step7:plot graph#\n"
            s+=f"figsize={self.__figsize}\n"
            s+=f"char_n={self.__char_n}\n"
        ...
        @property
        def sampling_n(self):
            return self.__sampling_n
        @sampling_n.setter
        def sampling_n(self,value):
            if not isinstance(value,int):
                raise ValueError('sampling_n must be be an integer')
            if value <= 0:
                raise ValueError('sampling_n must be bigger than 0')
            self.__sampling_n = value
        ...
        def random_state(self,value):
            if not isinstance(value,int):
                raise ValueError('random_state must be be an integer')
            if value <= 0:
                raise ValueError('random_state must be bigger than 0')
        ...
        def figsize(self,value):
            if not isinstance(value,tuple):
                raise ValueError('figsize must be be a tuple')
            self.__figsize = value

        @property
        def char_n(self):
            return self.__char_n
        @char_n.setter
        def char_n(self,value):
            if not isinstance(value,int):
                raise ValueError('char_n must be be a tuple')
            if value <= 0:
                raise ValueError('random_state must be bigger than 0')
            self.__char_n = value
    ...
    def __get_cgKNN(self,knn_graph,node_cluster,inner=True,random_state=6886,k=15,sampling_n=10):
        ...
        def get_edge_score(df,node_cluster,random_state,k,sampling_n):
            ...
            for i in range(sampling_n):
                ...
                if random.choice([True, False]):
                    Node1_cluster_sample_cells=list(Node1_df.sample(frac=0.5, random_state=random_state+i, axis=0)["cell_id"])
                    Node2_cluster_sample_cells=list(Node2_df.sample(frac=1, random_state=random_state+i, axis=0)["cell_id"])
                else:
                    Node1_cluster_sample_cells=list(Node1_df.sample(frac=1, random_state=random_state+i, axis=0)["cell_id"])
                    Node2_cluster_sample_cells=list(Node2_df.sample(frac=0.5, random_state=random_state+i, axis=0)["cell_id"])
        ...
        knn_graph_egde['probability_rep']=knn_graph_egde.apply(get_edge_score,args=(node_cluster,random_state,k,sampling_n),axis=1)
        knn_graph_egde[["probability"]+[f"probability_rep{i+1}" for i in range(sampling_n)]]=knn_graph_egde['probability_rep'].apply(pd.Series)
    ...
    @except_output
    @function_timer
    @params_filter(["sampling_n","random_state"])
    def get_cluster_trajectory(self,**kwargs):
        sampling_n=self.params.sampling_n=kwargs.setdefault("sampling_n",self.params.sampling_n)
        random_state=self.params.random_state=kwargs.setdefault("random_state",self.params.random_state)        
        inner_k=self.params.inner_graph_knn_n
        link_k=self.params.link_graph_knn_n
        #inner graph
        ...
        for (timepoint,adata) in self.timepoint_scdata_dict.items():
            ...
            node_cluster.index=adata.obs["cell_id"]
            adata.uns["cluster_graph"]=self.__get_cgKNN(knn_graph,node_cluster,inner=True,random_state=random_state,k=inner_k,sampling_n=sampling_n)
            
            all_node_cluster=pd.concat([all_node_cluster,node_cluster])

        #link graph
        knn_graph_egde=self.__get_cgKNN(self.knn_graph,all_node_cluster,inner=False,random_state=random_state,k=link_k,sampling_n=sampling_n)
    ...
    @except_output
    @function_timer
    @params_filter(["topN","min_cell_number"])
    ...
    @except_output
    @function_timer
    @params_filter(["char_n","figsize"])
    def draw_nxG(self,**kwargs):
        figsize=self.params.figsize=kwargs.setdefault("figsize",self.params.figsize)
        char_n=self.params.char_n=kwargs.setdefault("char_n",self.params.char_n)
        ...
        for (timepoint,adata) in self.timepoint_scdata_dict.items():
            ...
            for i,nodes in enumerate(list(G.nodes())):
                ...
                labels2[nodes]=(" "*2+"_".join(str(nodes).split("_")[1:])+" "*100)[:char_n+2]
                ...
                pos4[nodes]=(x_pos+char_n*1.3,y_pos) #start pos
        ...
        width_list=[]
        for (u, v, wt) in link_G.edges.data("probability"):
            width_list.append(wt)
        if len(width_list)!=0:
            width_list=[3*i/max(width_list) for i in width_list]
        timepoint_list=[]
        ...
        link_edges=nx.draw_networkx_edges(link_G, all_pos, ax=ax,width=width_list,edge_color="#0060A8",arrowstyle="->", arrowsize=20,alpha=1,min_source_margin=0)
        for i in range(len(timepoint_list)):
            tp=timepoint_list[i].split("timepoint")[1]
            link_edges[i].set_zorder((ZORDER+0.025)*int(tp))
        
        link_edges2=draw_networkx_edges(link_G, all_pos2,all_pos3, ax=ax,width=width_list,edge_color="#0060A8",arrowstyle="->", arrowsize=20,alpha=1,min_source_margin=10,node_size=100)
        for i in range(len(timepoint_list)):
            tp=timepoint_list[i].split("timepoint")[1]
