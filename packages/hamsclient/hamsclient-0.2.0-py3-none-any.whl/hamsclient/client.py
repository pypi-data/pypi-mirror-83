...
class meteoSwissClient():
    ...
    def __get_all_stations(self):
        ...
        for line in descriptionLines:
            ...
            lineParts = lineClean.split(";")
                
                        ## Saving station data to a dictionnary
            if len(lineParts)>10 and re.match("^\w{3}$",lineParts[1]):
                stationData = {}
                stationData["code"] = lineParts[1]
                stationData["name"] = lineParts[0]
                stationData["lat"] = lineParts[10]
                stationData["lon"] = lineParts[11]
                        #stationData["coordianteKM"] = lineParts[4]
                        #stationData["altitude"] = lineParts[5].strip()
                         
                stationList[stationData["code"]] = stationData
            else:
                _LOGGER.debug("Ignoring from station list : %s",lineClean)    
        return stationList 


    def get_closest_station(self,currentLat,currentLon):
        if(self._allStations is None):
           self._allStations  = self.__get_all_stations()
        _LOGGER.debug("Getting closest statsion from lat : %s - lon : %s "%(currentLat,currentLon))
        hPoint = geopy.Point(currentLat,currentLon)
        data =[]
        for station in self._allStations:
            print("Station %s - lon : %s lat : %s"%(station,self._allStations[station]["lat"],self._allStations[station]["lon"]))
        ...
        try:
            ...
            _LOGGER.warning("Unable to get closest station for lat : %s lon : %s"%(currentLat,currentLon))
            return None
