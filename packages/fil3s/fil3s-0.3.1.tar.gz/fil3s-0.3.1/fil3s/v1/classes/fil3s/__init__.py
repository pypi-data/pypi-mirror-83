...
class Files():
    ...
    class Directory(object):
        ...
        def generate_path(self, length=24, extension="/"):
            path, paths = None, self.paths()
            for x in range(1000):
                path = self.join(utils.generate.shell_string(length=length), extension)
    ...
    class Zip(object):
        ...
        def create(self,
            ...
            # create tmp dir.
            name = self.file_path.name().replace("."+self.file_path.extension(),'')
            ...
            if isinstance(source, str):
                ...
                name = target.name().replace("."+target.extension(),'')
...
class Formats():
    ...
    class String(object):
        ...
        def generate(self, length=6):
            return utils.generate.shell_string(length=length)
            #
