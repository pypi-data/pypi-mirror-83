...
class MapSerialiser(type):
    ...
    def __new__(
        mcs, key_serialiser: Type[EncodingType], value_serialiser: Type[EncodingType]
    ) -> MapSerialiser:
        """ Class constructor. """
        name = pascalcase(f"{key_serialiser.__name__}{value_serialiser.__name__}MapSerialiser")
        if name not in mcs._serialisers:
            mcs._serialisers[name] = cast(
                MapSerialiser,
                type(
