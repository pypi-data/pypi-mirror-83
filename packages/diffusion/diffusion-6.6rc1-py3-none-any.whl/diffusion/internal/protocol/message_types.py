...
from diffusion.internal import encoded_data, services, utils
from diffusion.internal.conversations import ConversationID
from diffusion.internal.services.topics import TOPICS_DATA_KEY
...
class TopicMessage(MessageProtocol):
    ...
    async def process(
        ...
        topic = session.data[TOPICS_DATA_KEY][topic_id]
        body = stream.read()
        if compressed:
