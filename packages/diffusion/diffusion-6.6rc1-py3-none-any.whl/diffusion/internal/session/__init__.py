...
import asyncio
from typing import Callable, Dict, Hashable, MutableMapping, Optional

import structlog

from diffusion.internal import exceptions, utils
from diffusion.internal.conversations import Conversation, ConversationID, ConversationSet
...
HandlersMapping = MutableMapping[Hashable, Callable]
...
class InternalSession:
    ...
    async def send_request(self, service: Service, conversation: Optional[Conversation] = None):
        ...
        LOG.debug(
            "Sending request.", cid=conversation.cid, service=service, request=service.request,
        )
    ...
    async def handle(self, handler_key: Hashable, *args, **kwargs):
        ...
        """
        LOG.debug("Handling event.", handler_key=handler_key, args=args, kwargs=kwargs)
        try:
            handler = self.handlers[handler_key]
        except KeyError:
            raise UnknownHandlerError(f"A handler was not registered for key '{handler_key}'.")
        else:
            return await utils.coroutine(handler)(*args, **kwargs)
