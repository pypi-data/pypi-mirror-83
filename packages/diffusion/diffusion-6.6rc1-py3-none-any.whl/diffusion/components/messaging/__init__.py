...
from .handlers import MessagingRequestHandler
...
class Messaging(Component):
    ...
    def add_request_handler(
        ...
        """ Registers a request handler.
            ...
            The `callback` callable is executed when the session receives a request sent
            to the given path or session filter.

            Args:
                path: The handler will respond to the requests to this path.
                callback: A callable to handle the request.
                request_type: The required data type of the request.
                response_type: The data type to be returned as the response.
        ...
        service_callback = MessagingRequestHandler(callback, request_type, response_type)
        self.session.handlers[(service_type, path)] = service_callback

    def add_filter_response_handler(
        self, session_filter: str, callback: Callable, error_callback: Optional[Callable] = None
    ) -> None:
        """ Registers a session filter response handler.

            The `callback` callable is executed when the session receives a response
            to a request sent to the session filter.

            Args:
                session_filter: A session filtering query.
                callback: A callable to handle the response.
                error_callback: A callable to handle error response.
        """
        service_type = type(self.services.FILTER_RESPONSE)
        self.session.handlers[(service_type, session_filter)] = callback
        if error_callback is None:
            error_callback = lambda *args, **kwargs: None  # noqa: E731
        self.session.handlers[(service_type, session_filter, "error")] = error_callback
    ...
    async def register_request_handler(
        ...
        """ Register the session as the handler for requests to a given path.
            ...
            these requests is added at the same time, using `add_request_handler` internally.

            Args:
                path: The handler will respond to the requests to this path.
                callback: A callable to handle the request.
                request_type: The expected data type of the request.
                response_type: The data type to be returned by the callback.
        ...
        """
        self.add_request_handler(path, callback, request_type, response_type)
    ...
    async def _send_request(
        ...
        return response_type.from_bytes(response.serialised_value.to_bytes())
