...
def get_selector(selector: str) -> SelectorProtocol:
...
class PathSelector(SelectorProtocol):
    ...
    def match(self, topic_path: str) -> bool:
        ...
        return not any(
            (
                ...
                self._match_descendants_inclusive is False and parts_count <= patterns_count,
                self._match_descendants_inclusive is None and parts_count != patterns_count,
            )
        ) and all(pattern.fullmatch(part) for pattern, part in zip(self._patterns, path_parts))


class SelectorSet(Set[SelectorProtocol], SelectorProtocol):
