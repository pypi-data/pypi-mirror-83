import aiohglib.client  # Circular dependency.
from aiohglib import util, templates
from aiohglib.error import CommandError
from aiohglib.util import b, strtobytes, integertypes
...
class changectx(object):
    ...
    def __init__(self, repo, changeid=b('')):
        ...
        if changeid == b(''):
            changeid = b('.')
        self._repo = repo
        if isinstance(changeid, aiohglib.client.revision):
            cset = changeid
        elif changeid == -1:
            cset = _nullcset
        else:
