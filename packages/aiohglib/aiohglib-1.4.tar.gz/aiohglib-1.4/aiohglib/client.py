...
from aiohglib.util import b, u, cmdbuilder, BytesIO, stream_limit, strtobytes, POSSIBLE_TIMEZONES
    ...
    def __new__(cls, rev, node, tags, branch, author, desc, date, p1node, p2node, extras):
        ...
        for extra in extras.split(aiohglib.util.b("_;_")):
            ...
            if extra.startswith(aiohglib.util.b("close=1")):
                close = True
        return tuple.__new__(cls, (u(rev), u(node), u(tags), u(branch), u(author, True), u(desc, True), date, u(p1node), u(p2node), u(source), u(inter_source), close))
...
class hgclient(object):
    ...
    async def _readchannel(self):
        ...
        if channel in b('IL'):
            ...
            data = b("")
            while length > 0:
                true_limit = min(stream_limit, length)
                try:
                    data += await self.server.stdout.readexactly(true_limit)
                except asyncio.IncompleteReadError:
                    raise error.ServerError('server stdout stream ended prematurely')
                length -= true_limit
            return channel, data
    ...
    async def log(self, revrange=None, files=[], follow=False,
            ...
            include=None, exclude=None, safe_template=False):
