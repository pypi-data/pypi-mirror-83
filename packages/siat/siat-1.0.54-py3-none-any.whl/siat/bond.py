...
def cf_day_coupon_trend(clist,v,m,f=2,r=0.03):
    ...
    footnote='票息率 -->'+ \
        "\n【债券】名义券利率："+str(r)+', 每年付息次数：'+str(f)+ \
        "\n到下一付息日的天数："+str(v)+", 下一付息日后剩余的付息次数："+str(m)
    gf.plot_line(df,colname,collabel,ylabeltxt,titletxt,footnote)
...
def cf_day_remain_trend(c,v,mlist,f=2,r=0.03):
    ...
    footnote='下一付息日后剩余的付息次数 -->'+ \
        "\n【债券】名义券利率："+str(r)+", 债券票面利率："+str(c)+', 每年付息次数：'+str(f)+ \
        "\n到下一付息日的天数："+str(v)
    gf.plot_line(df,colname,collabel,ylabeltxt,titletxt,footnote)
