...
def sector_list_china(indicator="新浪行业"):
    ...
    try:
        ...
        print("#Error(sector_list_china): data source tentatively banned your access for",indicator)
...
def sector_rank_china(comp="涨跌幅",indicator="新浪行业",num=10):
    ...
    try:
        ...
        print("#Error(sector_rank_china): data source tentatively banned your access for",indicator)
        print("Solutions:1) try an hour later, or 2) switch to another IP address.")
        return None
    ...
    #处理空记录
    if len(df3) == 0:
        print("#Error(sector_rank_china):data source tentatively banned your access for",comp,indicator)
        print("Solutions:1) try an hour later, or 2) switch to another IP address.")
        return
...
def sector_detail_china(sector="new_dlhy",comp="涨跌幅",num=10):
    ...
    try:
        ...
        print("#Error(sector_rank_china): data source tentatively banned your access for",sector)
        print("Solutions:1) try an hour later, or 2) switch to another IP address.")
        return None
    ...
    if comp == "涨跌幅":
        df3=df2[['个股名称','涨跌幅%','换手率%','收盘价','市盈率','市净率','流通市值']]
        df3.sort_values(by=['涨跌幅%'],ascending=False,inplace=True)
    if comp == "换手率":
        df3=df2[['个股名称','换手率%','涨跌幅%','收盘价','市盈率','市净率','流通市值']]
        df3.sort_values(by=['换手率%'],ascending=False,inplace=True)
    if comp == "收盘价":
        df3=df2[['个股名称','收盘价','换手率%','涨跌幅%','市盈率','市净率','流通市值']]
        df3.sort_values(by=['收盘价'],ascending=False,inplace=True)
    if comp == "市盈率":
        df3=df2[['个股名称','市盈率','市净率','收盘价','换手率%','涨跌幅%','流通市值']]
        df3.sort_values(by=['市盈率'],ascending=False,inplace=True)
    if comp == "市净率":
        df3=df2[['个股名称','市净率','市盈率','收盘价','换手率%','涨跌幅%','流通市值']]
        df3.sort_values(by=['市净率'],ascending=False,inplace=True)
    if comp == "流通市值":
        df3=df2[['个股名称','流通市值','总市值','市净率','市盈率','收盘价','换手率%','涨跌幅%']]
        df3.sort_values(by=['流通市值'],ascending=False,inplace=True)
    if comp == "总市值":
        df3=df2[['个股名称','总市值','流通市值','市净率','市盈率','收盘价','换手率%','涨跌幅%']]
