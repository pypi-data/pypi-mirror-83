...
class ReplicatedDatablock(object):
            ...
            # Client side
                ...
                try:
                    ...
                    logging.error(f"Failed to deserialize {repr(self)} :\n \
                                    Incompatible data received, maybe an \
                                    unsuported addon.")
                    self.state = ERROR
    ...
    def commit(self):
        if self.state in [MODIFIED, ADDED, UP]:
            try:
                ...
                logging.error(
                    "Lost reference, trying to resolve the datablocks.")
                self.resolve()
            except Exception:
            ...
            logging.warning(
                f"Trying to commit data in a wrong state:{repr(self)}")
    ...
    def _serialize(self):
        ...
        JSON -> BUFFER
        """
        self.buffer = pickle.dumps(self.data)
    ...
    def diff(self):
        ...
        if self.diff_method == DIFF_JSON:
            ...
            diff = pickle.dumps(self.data) != pickle.dumps(new_version)
    ...
    def __repr__(self):
        return f"- uuid: {self.uuid} \n \
                 ...
                 - type: {self.str_type} \n \
                 - data: {self.data.keys() if self.data else 'Empty'} \n \
