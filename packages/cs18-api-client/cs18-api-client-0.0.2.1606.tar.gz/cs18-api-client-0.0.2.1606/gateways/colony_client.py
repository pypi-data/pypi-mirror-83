...
from gateways.common.cs18_api_classes import BlueprintRepositoryDetails, AccessLink, \
    AddAccountBlueprintRepositoryRequest
...
class Colony:
    ...
    def add_repository_to_account(self, request: AddAccountBlueprintRepositoryRequest):
        method_url = self._api_root.settings.add_repository_by_token()
        resp = self.session.post(url=method_url, json=request.__dict__)
        GatewayUtils.handle_response(response=resp, return_codes=[200])

    def get_account_repositories(self) -> List[AccountRepositoryResponse]:
    ...
    def get_forecast(self, space: str, start_date: datetime, end_date: datetime):
        formatted_start = quote(start_date.isoformat())
        formatted_end = quote(end_date.isoformat())

        method_url = self._api_root.cost.forecast(space, formatted_start, formatted_end)
        resp = self.session.get(url=method_url)
