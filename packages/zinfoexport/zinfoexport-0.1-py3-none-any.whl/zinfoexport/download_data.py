...
def get_zinfo_token(username, password):
    """
...
def get_zinfo_data(my_token, my_url):
    """
...
def make_url(waterschapnummer, spcid, startdate, enddate, tagnr, wbm, periode):
    """ 
...
def construct_dataframe(parameters):
    """
    ...
    date_splits = date_range(startdate, enddate, interval)
    for i,tagnr in enumerate(input_file['tagnr']):
        ...
        for date_split in date_splits:
            print('...')
            token = get_zinfo_token(username, password)
            URL = make_url(waterschapnummer, spcid, date_split[0], date_split[1], tagnr, input_file.loc[i,'wbm'], periode)
            tag_dataframe = tag_dataframe.append(get_zinfo_data(token, URL))
        if i == 0:
            export_dataframe = export_dataframe.append(tag_dataframe)
            export_dataframe = export_dataframe[['dem', 'hstWaarde']].rename(columns={"dem":"tijdstip", "hstWaarde":name})
        else:
            export_dataframe[name] = tag_dataframe['hstWaarde']
    return export_dataframe

def run(parameter_file):
    with open(f'{parameter_file}.json') as file:
        parameters = json.load(file)
    export_dataframe = construct_dataframe(parameters)
    return export_dataframe
