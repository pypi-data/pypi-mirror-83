...
class OptimalExtraction:
    """
    OptimalExtraction is a python class to perform optimal extraction of a spectrum from an image.
    #####
    + Inputs:
    ...
    + Save:
      - container
      - wavelength
      - optimalextraction.fits
      - optimalextraction.csv
    #####
    + References:
    ...
    def _compute_kernel_fit(self):
        ...
        for i in np.arange(nx):
            ...
            if self.kernel_mode=='Gaussian1D':
                ...
                kernel = fitter(kernel,cross_dispersion_array,val,weights=var)
                kernel_fit.append(kernel)
