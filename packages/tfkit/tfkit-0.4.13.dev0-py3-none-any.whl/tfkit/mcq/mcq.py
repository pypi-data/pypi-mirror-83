...
from tfkit.mcq.data_loader import get_feature_from_data
...
class MCQ(nn.Module):
    def __init__(self, tokenizer, pretrained, maxlen=512):
        ...
        self.model = nn.Linear(self.pretrained.config.hidden_size, 1)
        self.maxlen = maxlen
    ...
    def forward(self, batch_data, eval=False):
        ...
        indices_tensor = torch.tensor(targets_pos).to(self.device)

        output = self.pretrained(tokens_tensor, attention_mask=mask_tensors)
        sequence_output = output[0]
        prediction_scores = self.model(sequence_output).squeeze(-1)
        if eval:
            self.eval()
            result_dict = {
                'label_map': [],
            ...
            maxprob_result = logit_prob.argmax(1).item()
            result_dict['label_max'].append(str(maxprob_result))
            result_dict['label_map'] = logit_prob.data.tolist()
            return result_dict
        else:
            loss_fct = nn.BCEWithLogitsLoss()
            print(prediction_scores.shape, loss_tensors.shape)
            masked_lm_loss = loss_fct(prediction_scores, loss_tensors.float())
