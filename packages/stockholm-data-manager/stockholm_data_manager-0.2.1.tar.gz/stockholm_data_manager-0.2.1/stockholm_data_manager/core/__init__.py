...
from stockholm_data_manager.plugins.json2sheets import slow__sync_data2sheets
...
class SDM:
    ...
    def add_generated_data(self, language: str, worksheet_name: str, data: List[str]):
        if language not in self.languages:
            raise ValueError(
                f"{language} is not valid. Pick from {self.languages}.")
        ...
        try:
            ...
            worksheet = data_generation_sheet.add_worksheet(
                title=worksheet_name, rows=1000, cols=15)
    ...
    def sync_local2sheets(self, language: str, sheet_type: str, update_columns: List[str]):
