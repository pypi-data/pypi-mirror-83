...
import sdm.constants as const
from sdm.sterile.gsprd import Gsprd, WorksheetNotFound, Spreadsheet, Worksheet
from sdm.sterile import time
from sdm.plugins.json2sheets import slow__sync_data2sheets
from sdm.utils import train_test_split
from sdm.utils.logger import log, set_log_level
...
class SDM:
    ...
    def generated_data_increment(self, language: str):
        """[summary]
        ...
        if generated_data:
            ...
            train_data, test_data = train_test_split.split(generated_data_dfs)
    ...
    def sync_local_to_remote(self, language: str, sheet_type: str, update_columns: List[str]):
        if sheet_type not in self.SHEET_TYPES:
            raise ValueError(
                "Expected sheet_type to be one of %s.",
                self.SHEET_TYPES
            )
        ...
        data_path = os.path.join(
            ...
            self.to_version,
            language,
            f"{dataset_type}_error_records.json"
        )

        with open(data_path, "r", encoding="utf-8") as f:
            data = json.load(f)
        index = self.__col2index(self.index_column)
        update_indices = [self.column_names.index(
            column) for column in update_columns]

        spreadsheet_id = self.spreadsheets[language][sheet_type]

        slow__sync_data2sheets(self.gsprd, data, spreadsheet_id, self.worksheet_names,
                               self.column_names, self.__col2index(self.index_column), update_indices)
