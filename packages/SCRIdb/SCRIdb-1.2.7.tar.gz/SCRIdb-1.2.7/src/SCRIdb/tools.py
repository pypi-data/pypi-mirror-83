...
def sample_data_frame(sd: pd.DataFrame) -> pd.DataFrame:
    ...
    sample_data["source"] = (
        sample_data["project_loc"].str.cat(sample_data["old_basename"], sep="/") + "/"
    )
    sample_data["s3uri"] = (
        sample_data["s3_loc"].str.cat(sample_data["sample"], sep="/") + "/"
    )
    sample_data["igo_id"] = sample_data.project_loc.str.lstrip(
        "/Volumes/peerd/FASTQ/Project_"
    )
    sample_data["igo_id"] = sample_data.igo_id.str.split(pat="/", n=1, expand=True)[0]
...
def prepare_statements(sample_data: pd.DataFrame) -> dict:
    ...
    for sample_ in sample_data.itertuples():
        ...
        sample_set[str(sample_.id) + "_" + sample_.label] = dict(cmd=cmd)
