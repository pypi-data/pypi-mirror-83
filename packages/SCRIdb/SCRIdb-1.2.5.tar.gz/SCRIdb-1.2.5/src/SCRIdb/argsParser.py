...
def parse_arguments(argsv: str = None) -> argparse.Namespace:
    ...
    ggroup.add_argument(
        ...
        action="store",
        nargs="?",
        help="For `data_submission` sub-command: string, path to html form. "
        "For `process` sub-command: string, override path to `.csv` file in config, "
        "listing newly delivered sequencing data from the genome center core. "
        "A special case for `process` sub-command: pass `-`, and follow "
        "the prompt on screen. For more details and examples follow the documentation "
        "at https://tinyurl.com/y5f9pzx8 ",
    )
