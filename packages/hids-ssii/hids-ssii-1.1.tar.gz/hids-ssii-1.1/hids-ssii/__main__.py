...
def readLogFile():
    text = str()
    with open("C:\top_secret\log.log", "r") as reader:
        text = reader.read()
    return text
...
def importConfig():
    ...
    if (os.path.exists("C:\top_secret\config.config")):
        try:
            with open("C:\top_secret\config.config", "r") as config:
                for line in config:
                    if "#" not in line:
        ...
        try:
            with open("C:\top_secret\config.config", "w") as file:
                file.write(
...
def exportConfig():
    ...
    with open("C:\top_secret\config.config", "w") as config:
...
def exportHashedFiles():
    ...
    with open("C:\top_secret\hashes.hash", "w") as writer:
...
def importHashedFiles():
    ...
    try:
        with open("C:\top_secret\hashes.hash", "r") as reader:
...
def iniciar():
    logging.basicConfig(format='%(levelname)s:%(asctime)s: %(message)s',
                        datefmt='%m/%d/%Y %H:%M:%S', filename='C:\top_secret\log.log', level=logging.INFO)
    importConfig()
    gui()
