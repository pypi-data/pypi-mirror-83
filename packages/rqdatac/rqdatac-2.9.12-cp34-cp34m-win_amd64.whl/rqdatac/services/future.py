...
def get_warehouse_stocks(underlying_symbols, start_date=None, end_date=None, market="cn"):
    ...
    for symbol in set(underlying_symbols) & set(multi_symbol_map):
        ...
        if end_date > date_line:
            import warnings
            warnings.warn('You are using the old symbol: {}, however the new symbol: {} is available after {}.'.format(symbol, multi_symbol_map[symbol], date_line))

    # 对传入的 underlying_symbols 依照 multi_symbol_map 生成一个对照 DataFrame
    symbol_map_df = pd.DataFrame([(symbol, multi_symbol_map.get(symbol, symbol)) for symbol in underlying_symbols],
                                 columns=['origin', 'new'])
