    ...
    if host is not None:
        for idx, row in host.iterrows():
            if np.unique(row['NED_name']) != "":
                print("Found host %s.\n"%row['NED_name'].values[0])
            else:
                print("Found host %s"%row['objName'])
            print("RA, DEC (J2000): %f, %f"%(row['raMean'], row['decMean']))
            if row['NED_redshift'].values[0] != '':
                print("Redshift: %f"%row['NED_redshift'].values[0])
            print("PS1 rMag: %.2f"%row['rApMag'].values[0])
            print("g-r    r-i    i-z")
            print("%.2f   %.2f   %.2f"%(row['g-r'].values[0], row['r-i'].values[0], row['i-z'].values[0]))
        ...
        #    print("i-z: %.2f."%host['i-z'].values[0])
            ...
            if np.unique(row['NED_name']) != "":
                ...
                tempHost = fullTable[fullTable['objName'] == row['objName'])
...
def getHostImage(transientName='', band="grizy", rad=60, save=0):
    if transientName == '':
        ...
        return
    fullTable = fullData()
    host = getDBHostFromTransientName(transientName)
