...
class PatrowlHearsApi:
    ...
    def action(self, url, method='GET', data=None):
        ...
        try:
            r = requests.Request(method=method, url=self.url+url, data=data, headers={'Authorization': 'Token {}'.format(self.auth_token)}, timeout=self.timeout)
            pr = r.prepare_request()
