...
def item_sel_filter(item: LiszItem) -> bool:
    ...
    :return:            True if item is a selected leaf or if item is a node with only selected sub-leaves, else False.
    """
    if 'node' in item:
...
def item_unsel_filter(item: LiszItem) -> bool:
    ...
    :param item:        item data structure to check.
    :return:            True if item is an unselected leaf or if item is a node with only unselected leaves, else False.
    """
    if 'node' in item:
        for sub_item in item['node']:
...
class LiszDataMixin(ABC):
        ...
        if want_node:
            if err_msg:
                ...
                if item_idx >= 0:                                   # found the blocking/duplicate item id
                    ...
                    err_msg = get_f_string("(ignorable) ") + err_msg
                ...
                node_to_add_to[new_item_index]['node'].extend(nid['node'])
    ...
    def on_node_extract(self, flow_path_text: str, event_kwargs: dict) -> bool:
        ...
        if extract_action in ('copy', 'cut'):
            ...
            self.call_method('share_node', flow_path, node=extract_node)
