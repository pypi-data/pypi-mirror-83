...
def flow_path_items_from_text(text: str) -> Tuple[str, LiszNode]:
    ...
    :return:                    tuple of flow path (empty string if root or not given) and node list.
    """
    flow_path_text = ""
    if text.startswith('{'):
        node_lit = '[' + text + ']'
...
"""
        ...
        print("=== sent flow path and node", text)
        flow_path_text, node_literal = text.split("\n", maxsplit=1)
        flow_path = self.flow_path_from_text(flow_path_text, skip_check=True)
        print("    resulting flow_path:", flow_path)
        node = ast.literal_eval(node_literal)
        print("    resulting node:", node)
        node_to_add_to = self.flow_path_node(flow_path, create=True)
        self.add_items(node, node_to_add_to=node_to_add_to)
    else:
        item_id = text
        print("=== sent item id", item_id)
        self.add_item(dict(id=item_id))
"""
...
class LiszDataMixin(ABC):
    ...
    def export_node(self, flow_path: List[str], file_path: str = ".", node: Optional[LiszNode] = None
        ...
        try:
            ...
            with open(os.path.join(file_path, file_name), 'w') as file_handle:
                file_handle.write(pformat(node))
    ...
    def on_item_sel_change(self, item_id: str, event_kwargs: dict) -> bool:
        ...
        if node:
            ...
            # assert not set_sel_to == bool(self.current_node_items[node_idx].get('sel'))
    ...
    def sub_item_ids(self, node: Optional[LiszNode] = None, item_id: str = '',
        ...
        """ return item names of all items including their sub-node items (if exists and recursive==True).
        ...
        for item in node:           # type: ignore # mypy does not recognize that node cannot be None
            ...
            if recursive and item.get('node'):
