...
def selectplots(plotsDF):
    ...
    for num in RandomTable[int(stmgrp)]:
        if int(num) <= tplot:
            RNUM=int(num)
            break
    splots = [ RNUM+splotinterval*i  if RNUM+splotinterval*i <= tplot else RNUM+splotinterval*i-tplot  for i in range(1,5) ] # Four Randomly Selected Plot SNo.
        ...
        for plt in splots:
            ...
            for sno in selectedplots[plt]:
                ...
                if sno==plt:
                    ...
                    spltstr.append('  {:>2}'.format(n)+'.  {:>5}'.format(sno))
                spltstr.append('\n') if not n%5 else ''
        return { 'param':[ RNUM,splotinterval ] , 'splots': spltstr }
...
def agriplot(FILE_NAME, checkfile=''):
    ...
    '''
    wb = openpyxl.load_workbook(getFilename(FILE_NAME))
