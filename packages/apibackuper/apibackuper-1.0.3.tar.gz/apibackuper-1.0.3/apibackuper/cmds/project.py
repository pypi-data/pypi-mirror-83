...
class ProjectBuilder:
    ...
    def run(self, mode):
        ...
        if http_mode == 'GET':
            if resp_type == 'json':
                content = self.http.get(start_url, params=params).content
                start_page_data = self.http.get(start_url, params=params).json()
            else:
                content = self.http.get(start_url, params=params).content
                start_page_data = xmltodict.parse(content)
        else:
            if resp_type == 'json':
                start_page_data = self.http.post(start_url, json=params).json()
            else:
                content = self.http.post(start_url, params=params).content
                start_page_data = xmltodict.parse(content)
...
#        print(json.dumps(start_page_data, ensure_ascii=False))
        ...
        for page in range(1, num_pages + 1):
            ...
            if iterate_by == 'page':
                ...
                params[count_skip_param] = (page-1) * page_limit
            elif iterate_by == 'range':
                params[count_from_param] = (page-1) * page_limit
                params[count_to_param] = page * page_limit
            if http_mode == 'GET':
                    ...
                    try:
                        for item in get_dict_value(data, data_key, splitter=field_splitter):
                            for key in files_keys:
                                for uniq_id in get_dict_value(item, key, as_array=True, splitter=field_splitter):
