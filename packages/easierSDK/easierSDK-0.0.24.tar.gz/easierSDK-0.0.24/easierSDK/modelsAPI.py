...
class ModelsAPI():
    ...
    def show_models(self, repo_name:str=None, category:Categories=None):
        if repo_name is None:
            repo_name = self.GLOBAL
        if category:
            objects = self.minio_client.list_objects(repo_name, prefix=self.MODELS + '/' + category.value, recursive=True)
        else:
            objects = self.minio_client.list_objects(repo_name, prefix=self.MODELS + '/' + Categories.MISC, recursive=True)
        ...
        if len(objects)>0:
            ...
            print("No models found in repository " + repo_name)
    ...
    def download(self, repo_name:str, category:Categories, model_name:str, path_to_download:str):
        if not self.minio_client.bucket_exists(repo_name):
            print('ERROR: Repository name does not exist. Please check and try again')
            return None
    ...
    def load_from_local(self, path:str, print_files=False):
        model = Model()
    ...
    def load_from_repository(self, repo_name:str, category:Categories, model_name:str, print_files=False):
        # TODO revise experiment_id
        if not os.path.isdir('/tmp/download'): os.mkdir('/tmp/download')
        path = self.download(repo_name, category, model_name, path_to_download='/tmp/download')
    ...
    def upload(self, category:Categories, model:Model, public=False, remove_dir=True):
        ...
        try:
            for obj in os.listdir(path):
                ...
                # TODO Create bucket if doesn't exist??
