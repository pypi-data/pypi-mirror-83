...
def progress_file(path: str,
                  mode: str,
                  progress: Optional[ProgressInterface]) -> IO[bytes]:
    """Associate an input file with a progress object.

    :param path: file to which progress should be associated.
    :param mode: file access mode, e.g. 'rb', 'r'.
    :param progress: ProgressInterface to which the file is associated.
    :return: file object with backed-in progress.
    """
    return cast(IO[bytes], Opener(
        path, mode=mode, wrapper=with_progress_decorator(progress)))
...
def progress_file_iter(files: List[str],
    ...
    if sum(file_size) == 0:
        yield from (cast(IO[bytes], Opener(f, mode)) for f in files)
        return
