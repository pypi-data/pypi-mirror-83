...
@singledispatch
def calc_value(term, decoded_sol: dict, placeholder={}, **kwargs)->float:
...
@calc_value.register(Placeholder)
...
@calc_value.register(ArraySizePlaceholder)
...
def calc_tensor_value(term: Tensor, decoded_sol: dict, placeholder={}, fixed_indices={})->float:
    ...
    index_label = [to_index(label) for label in term.index_labels]
    return np.array(sol)[tuple(index_label)]


@calc_value.register(Sum)
def calc_sum_value(term: Sum, decoded_sol: dict, placeholder={}, fixed_indices={})->float:
    ...
    for child in term.children:
        for ind in sum_index:
            ...
            term_value += child_val if child_val is not None else 0.0
    return term_value



@calc_value.register(Constraint)
