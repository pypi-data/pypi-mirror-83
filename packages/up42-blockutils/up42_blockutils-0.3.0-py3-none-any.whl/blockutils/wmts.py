...
class TileMergeHelper(TileMergeHelperSettings):
    ...
    def _process(response: requests.Response, tile: Tile) -> Path:
        ...
        """
        temp_tile: rasterio.MemoryFile = io.BytesIO(response.content)
        temp_tile_filename = Path(f"/tmp/output/{str(uuid.uuid4())}.tif")

        bands: list = []
        with rasterio.open(temp_tile) as image:
    ...
    def get_final_image(self, output_path: Path, apply_to_cog: bool = True):
...
class MultiTileMergeHelper:
    ...
    def get_multiband_tif(
        self, filename_path: Path, band_descriptions: List[str] = None
    ):
        outputs_path = []
        valid_tiles = {}
        for i, tilemergehelper in enumerate(self.tilemergehelper_list):
            ...
            valid_tiles[i] = tilemergehelper.get_final_image(
                Path(tmp), apply_to_cog=False
            )
            outputs_path.append(Path(tmp))

        if band_descriptions:
            for old_key, new_key in zip(valid_tiles.keys(), band_descriptions):
                valid_tiles[new_key] = valid_tiles.pop(old_key)

        create_multiband_tif(outputs_path, filename_path, band_descriptions)
