...
class TimescaleClient:
    ...
    """
    def __init__(self, tbl_name, pk, standardized=True, **kwargs):
    ...
    def standardize_df(*args, **kwargs):
    ...
    def execute_values(self, df, method='fail', parse_df=False):
        ...
        if 'simulation' in df.columns:
            pk = self.pk + ['received_date', 'simulation']
        else:
            pk = self.pk
        ...
        if method == 'fail':
            query = "INSERT INTO %s(%s) " \
                    "VALUES %%s " \
                    "RETURNING %s " % (self.tbl_name, cols, ','.join(pk))
        elif method == 'ignore':
            query = "INSERT INTO %s(%s) " \
                    "VALUES %%s " \
                    "ON CONFLICT (%s)" \
...
class TimescaleClientNarrow(TimescaleClient):
    ...
    def get_latest_metrics(self, device_ids, metrics,
                           simulation=None, reference_time=None, window=None):
        """
        :param device_ids: A list of device_ids to be queried
        :param metrics: A list of metrics (Strings) to be queried for
        each device_id
        ...
        """
        cond, cond_vals = \
            h.generate_ts_conditionals(simulation, reference_time, window)
        query = sql.SQL(
            "SELECT device_id, "
            "device_metric_type_id, "
...
class TimescaleClientJSON(TimescaleClient):
    ...
    def standardize_df(self, df, **kwargs):
        ...
        result format.
        :param df:
        :param **kwargs --> metrics: List fo metrics to be returned in the
        function
        :return: Data frame in the format:
            - Index = ('measurement_date', 'device_id', 'device_metric_type_id')
            - Columns = ['value']
