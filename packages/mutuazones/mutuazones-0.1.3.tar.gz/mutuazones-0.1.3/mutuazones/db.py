...
class Zone(object):
    ...
    def generate_profile(self, df, minimal=True):
        """Genera un perfil de un dataframe concreto. Si no se asigna a una variable lo imprime por pantalla"""
        profile = pandas_profiling.ProfileReport(df, title='Pandas Profiling Report', explorative=True, minimal=minimal, progress_bar=True)
                                                #, html={'style': {'full_width': True}})
        #profile.to_widgets()
        return profile
 
    def save_profile_to_file(self, pathFile, df=None, profile=None, minimal=True):
        ...
        if profile is not None:
            ...
            return profile
        if df is not None:
            profile = pandas_profiling.ProfileReport(df, title='Pandas Profiling Report', explorative=True, minimal=minimal, progress_bar=True)#html={'style': {'full_width': True}})
            #profile.to_widgets()
            profile.to_file(output_file=pathFile)
            return profile
