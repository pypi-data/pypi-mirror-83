...
async def absent(hub, ctx, name: str, **kwargs) -> Tuple[bool, Dict[str, Any]]:
    status, result = await hub.exec.aws.eks.cluster.delete(ctx, name=name, **kwargs)
    if not status:
        return False, {"comment": result.get("exception", result)}

    return status, {"comment": f"Deleted eks cluster: {name}"}


async def present(
    hub, ctx, name: str, role: str, tags: Dict[str, str] = None, **kwargs
    ...
    status, _ = await hub.exec.aws.eks.cluster.get(ctx, name)
    comments = []

    if not status:
        # The eks cluster doesn't exist, create it
        status, result = await hub.exec.aws.eks.cluster.create(
            ctx, name, role, **kwargs
        )
        if not status:
            return False, {"comment": result.get("exception", result)}
        comments.append(f"Created eks cluster: {name}")

    if kwargs:
        # TODO determine if updating is necessary, maybe by getting the revision_id from cluster?
        new_status, _ = await hub.exec.aws.eks.cluster.update(ctx, name, **kwargs)
