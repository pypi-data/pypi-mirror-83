...
def get_frames_configuration(
    ...
    if len(pt_frames) == 3:
        ...
        def group_frames(pt_frames: [Frame]) -> List[List[Frame]]:
            return [[
                cv2.rotate(pt_frames[1][:-20], cv2.ROTATE_90_COUNTERCLOCKWISE),
        ...
        def concatenate_groups(frame_groups: List[List[Frame]]) -> List[Frame]:
            frames_depths = [np.max(frame) - np.min(frame) for frame in frame_groups[0]]
            group_max_depth = max(frames_depths)
            frame_groups = [
                normalize_frame(
                    frame,
                    scale=(frames_depths[frame_index] / group_max_depth),
    ...
    if len(pt_frames) == 4:
        ...
        def concatenate_groups(frame_groups: List[List[Frame]]) -> List[Frame]:
            frames_depths = [[np.max(frame) - np.min(frame) for frame in group] for group in frame_groups]
            group_max_depths = [max(group) for group in frames_depths]
            frame_groups = [
                [
                    normalize_frame(
                        frame,
                        scale=(frames_depths[group_index][frame_index] / group_max_depths[group_index]),
