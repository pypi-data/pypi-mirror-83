...
class quarchDevice:
    ...
    # TODO: forceFind taking an IP address is not very nice.  We should ideally locate devices outside the local 
    # subnet without needing to enter the IP address a second time, the variable name is also confusing here.
    def __init__(self, ConString, ConType="PY", timeout="5", forceFind=0):
        ...
        ## QPS
            ...
            # TODO: forceFind is not used here...will QPS find a device on another subnet?  If not we need to add it here.  If so, we should use the same mechanism for QIS (ie
            # can we have QIS/QPS auto scan for the $default module if it does not currently exist in the scan results?
            # Creates the connection object.
...
def checkModuleFormat(ConString):
    ...
    # TODO: Cannot raise an exception AND return false! Return option is better
