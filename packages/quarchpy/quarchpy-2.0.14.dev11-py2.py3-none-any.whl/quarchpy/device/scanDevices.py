...
from quarchpy.config_files.configparser import return_module_type_list
...
def scanDevices(target_conn="all", debugPrint=False, lanTimeout=1, scanInArray=True, favouriteOnly=True, filterStr=None,
                ipAddressLookup=None, module_type_filter=None):
    found_devices = dict()
    scanned_arrays = list()

    if target_conn.lower() == "all":
        found_devices = list_USB()
        found_devices = mergeDict(found_devices, list_serial())
        found_devices = mergeDict(found_devices, list_network("all", ipAddressLookup=ipAddressLookup))
        # print(found_devices)

    if target_conn.lower() == "serial":
        found_devices = list_serial()

    if target_conn.lower() == "usb":
        found_devices = list_USB()

    if target_conn.lower() == "tcp" or target_conn.lower() == "rest" or target_conn.lower() == "telnet":
        found_devices = list_network(target_conn, ipAddressLookup=ipAddressLookup)

    if scanInArray:
        found_devices = scan_modules_in_array(found_devices, scanned_arrays)

    if favouriteOnly:
        found_devices = filter_module_by_favourite(found_devices)

    # Sort by alphabetic order of key
    sortedFoundDevices = {}
    sortedFoundDevices = sorted(found_devices.items(), key=operator.itemgetter(1))
    found_devices = dict(sortedFoundDevices)
    if filterStr != None:
        found_devices = filter_modules_by_string(filterStr, found_devices)

    # used to filter module via type ( Power / Drive / ...)
    if module_type_filter:
        found_devices = filter_module_type(module_type_filter, found_devices)

    return found_devices


def scan_modules_in_array(foundDevices, scannedArrays):
...
def filter_module_by_favourite(foundDevices):
...
def filter_modules_by_string(filterStr, foundDevices):
    filteredDevices = {}
    for k, v in foundDevices.items():
        for j in filterStr:
            if (
    ...
    while True:
        ...
        if nice:  # Prepair the data for niceListSelection using displayTable().
            ...
            for option in additionalOptions:
                ...
                # adOp.append([option,"-"])
            userStr = listSelection(title, message, tempList, additionalOptions=adOp, indexReq=True, nice=nice,
                                    tableHeaders=["Selection", "Description"])
