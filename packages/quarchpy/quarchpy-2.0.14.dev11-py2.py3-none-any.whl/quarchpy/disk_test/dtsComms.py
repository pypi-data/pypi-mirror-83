...
class DTSCommms:
    ...
    def create_request_log(self, logTime, messageType, messageText, messageSource, messageData=None, uId=""):

        root_tag = Element(self.request_tag)

        self.create_request_root(root_tag, request_type="log", response_required=False)

        self.notifyTestLogEventXml(root_tag, uId, logTime, messageType, messageText, messageSource,
                                   messageData)
    ...
    def notifyTestLogEventXml(self, tree_tag, unique_id, timeStamp, logType, logText, logSource, log_details=None):
