...
class BaseTest(ABC):
    ...
    def select_drive(self, drive_type="all"):
        ...
        if isinstance(drive, DriveWrapper):
            self.comms.sendMsgToGUI(
                self.comms.create_request_log(time.time(), "testResult",
                                              "User Select drive : {0} , {1}".format(drive.identifier_str,
                                                                                     drive.description),
                                              os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name,
                                              {"testResult": "True"}, uId=self.test_id.gen_next_id()))

        if self.document_mode:
            drive = DriveWrapper()

        if drive.drive_type in "Unknown":
            self.test_point(debug="Unknown drive types do not support Lane Width and Link Speed detection")
        elif drive.drive_type in "sas":
            self.test_point(debug="No current support for Lane Width and Link Speed detection for Sas drives")
        else:
    ...
    def select_quarch_module(self, use_qps=False, ip_lookup=None, filter_module=None):
        ...
        self.comms.sendMsgToGUI(
            self.comms.create_request_log(time.time(), "testResult",
                                          "User Select module : {0}".format(user_choice),
                                          os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name,
                                          {"testResult": "True"}, uId=self.test_id.gen_next_id()))
