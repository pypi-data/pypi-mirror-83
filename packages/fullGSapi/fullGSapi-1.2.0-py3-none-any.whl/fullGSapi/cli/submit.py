...
@click.option("--submission", "-s", "submission", default=".", help="This is the file/folder which you want to submit.", type=click.Path())
...
@click.option("--programming/--pdf", help="This is to set if you are submitting a programming vs PDF assignment.", type=bool)
@click.pass_context
def submit(ctx, course: str, assignment: str, submission: str, method: str, path: str, repository: str, branch: str, email: str, leaderboard: str, programming: bool):
    """
    ...
    if method == "upload":
        ...
        if programming:
            ...
            files = {f[(len(str(pathlib.Path(path).parent)) if os.path.isfile(path) else len(path)) + 1:]:open(f, 'rb') for f in filenames}
    ...
    if method == "github":
        ...
        if gs.submit_via_repo(course, assignment, method, repo_id, branch, leaderboard_name=leaderboard, owner_id=email):
            click.echo("Success!")
        else:
            click.echo("Failed!")
        return
    ...
    # BITBUCKET ==============================
        ...
        for b in branches:
            ...
            return
        click.echo("Submitting...")
        if gs.submit_via_bitbucket(course, assignment, method, repository, branch, leaderboard_name=leaderboard, owner_id=email):
            click.echo("Success!")
        else:
            click.echo("Failed!")
