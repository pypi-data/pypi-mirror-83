...
def get_experiment(experiment_token=None, **kwargs):
    if not experiment_token:
        return

    from savvihub.api.savvihub import SavviHubClient
    client = SavviHubClient(token=experiment_token)
    ...
    from savvihub.common.experiment import Experiment
    return Experiment.from_given(savvihub_experiment.id, client)
...
class Context:
    token = Value(global_config_yml=".token", default_value=None)
    experiment_token = Value(env="EXPERIMENT_ACCESS_TOKEN", default_value=None)
    ...
    def __init__(self, login_required=False, experiment_required=False, savvihub_filename=DEFAULT_SAVVIHUBFILE_YAML):
        attrs = dict()
        for key in dir(self):
        ...
        savvihub_filename = os.path.join(CUR_DIR, savvihub_filename)
        savvihub_yml_config = self.load_experiment_config_file(savvihub_filename)
        ...
        for key, attr in attrs.items():
            ...
            if attr.global_config_yml and global_config:
                data[key] = pyjq.one(attr.global_config_yml, global_config)

            if attr.savvihub_file_yml and savvihub_yml_config:
                data[key] = pyjq.one(attr.savvihub_file_yml, savvihub_yml_config)
