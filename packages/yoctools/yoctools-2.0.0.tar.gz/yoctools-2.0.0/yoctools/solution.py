    ...
    def __init__(self, components):
        ...
        for component in self.components:
            ...
            for name in component.depends:
                if type(name) == dict:
                    name = list(name.keys())[0]
                if name not in cmp_name_list:
                    cmp_name_list.append(name)
        ...
        # filter components which you do not want to depend.
        components = ComponentGroup()
        for component in self.components:
            if component.name in cmp_name_list or component.type == 'solution':
                components.add(component)
        self.components = components
