...
class HTMLData:
    ...
    def __init__(self, file, data=None, show_missed_elements=False):
        ...
        self.common_prefix = ''.join(c[0] for c in takewhile(lambda x: all(x[0] == y for y in x), zip(*strings)))
    ...
    def stats(self, should_print=True):
        ...
        for filename, data in self.files.items():
            ...
            total += current_total
            avg = round(current_cov / current_total * 100, 1) if current_total else 'N/A'
            avg_find = round(current_cov_find / current_total * 100, 1) if current_total else 'N/A'
...
class TrackCoverageChrome(Chrome):
    ...
    def __init__(self, request, *args, **kwargs):
        ...
        self.plugin = self.pytest_config.pluginmanager.get_plugin('pytest-track-html-coverage')
