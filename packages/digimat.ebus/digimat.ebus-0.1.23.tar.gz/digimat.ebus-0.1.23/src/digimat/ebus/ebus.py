...
class EBusItems(object):
    ...
    def table(self):
        if self.count()>0:
            with self._lock:
                t=PrettyTable()
                t.field_names=['type', 'index', 'name', 'value', 'age']
                for item in self.inputs():
                    t.add_row(['input', item.index, item.name, item.valuestr(), item.age()])
                for item in self.outputs():
                    t.add_row(['output', item.index, item.name, item.valuestr(), item.age()])
