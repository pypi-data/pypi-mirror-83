...
class Test(unittest.TestCase):
  ...
  def testGetWelded(self):
    ...
    self.assertEquals("['abdomen:1', 'rThigh:1', 'lThigh:1']", str(ls))

    ls = sc1.getWelded(1, "rHand:1")
    self.assertTrue(ls != None)
    self.assertEqual(len(ls), 6)
    self.assertEquals("['rForeArm:1', 'rThumb1:1', 'rIndex1:1', 'rMid1:1', 'rRing1:1', 'rPinky1:1']", str(ls))
...
#
    ...
    self.assertEquals("yrot", rl[0].getName())
    self.assertEquals("PHFEMALEThumbSide", rl[1].getName())
    self.assertEquals("PHFEMALEThumbBend", rl[2].getName())
  ...
  def testDeleteChannelImpact(self):
    ...
    zOffsetCh = body2.getChannels().findAttribut(PoserToken.E_zOffsetA, "zOffset")
    ...
    chan = chest2.getChannels().findAttribut(PoserToken.E_targetGeom, "PHFEMLeftBreastUp-Down")

    rl = pfv3.deleteChannelImpact(chan)
    self.assertEquals(2, len(rl))
    self.assertEquals("PHFEMLeftBreastUp-Down", rl[0].getName())
    self.assertEquals("xRotate", rl[1].getName())

    lhead2 = fig.findActor("head:2")
    yrotCh = lhead2.getChannels().findAttribut(PoserToken.E_targetGeom, "PHFEMEyesSide-Side")

    rl = pfv3.deleteChannelImpact(yrotCh)
    self.assertEquals(3, len(rl))
    self.assertEquals("PHFEMEyesSide-Side", rl[0].getName())
    self.assertEquals("yrot", rl[1].getName())
    self.assertEquals("lEye:2", rl[1].getPoserMeshedObject().getName())
    self.assertEquals("yrot", rl[2].getName())
    self.assertEquals("rEye:2", rl[2].getPoserMeshedObject().getName())

    pfmm1 = PoserFile("srcdata/scenes/MapMonde+Prop+Channels-02.pz3")
    fig = pfmm1.getFigure(2)
    pp = pfmm1.findAllMeshedObject("box_1")[0]
    self.assertEquals("box_1", pp.getName())
    m1Ch = pp.getChannels().findAttribut(PoserToken.E_targetGeom, "m1")

    rl = pfmm1.deleteChannelImpact( m1Ch)
    self.assertEquals(2, len(rl))
    self.assertEquals("m1", rl[0].getName())
    self.assertEquals("Vagues", rl[1].getName())

    pfmm2 = PoserFile("srcdata/scenes/MapMonde+Prop+Channels-01.pz3")
    fig = pfmm2.getFigure(1)
    globe1 = fig.findActor("Globe:1")
    vaguesCh = globe1.getChannels().findAttribut(PoserToken.E_targetGeom, "Vagues")

    rl = pfmm2.deleteChannelImpact(vaguesCh)
    self.assertEquals(1, len(rl))
    self.assertEquals("Vagues", rl[0].getName())

    mountsCh =  globe1.getChannels().findAttribut(PoserToken.E_targetGeom, "Montagnes")
    rl = pfmm2.deleteChannelImpact( mountsCh)
    self.assertEquals(2, len(rl))
    self.assertEquals("Montagnes", rl[0].getName())
    self.assertEquals("m1", rl[1].getName())
  ...
  def testDeleteChannel(self):
    ...
    rl = pfv3.deleteChannelImpact( chan)
    self.assertEquals(49, len(chest2.getChannels().getLstAttr()))
    pfv3.deleteChannel(chan, rl)
    self.assertEquals(48, len(chest2.getChannels().getLstAttr()))
...
#     self.assertEquals(14, len(l1))
  ...
  #  
  # TODO: Replace PoserFileParser with a 'readline' version to save time
  #
  def testPerfRead(self):
    c = ChronoMem.start("PERFO READ PoserFile.read-PHF-LoRes.cr2+read")
    f = open("srcdata/PoserRoot/Runtime/Librairies/Character/tu1/PHF-LoRes.cr2", 'rb')
    sc = [0]
    while sc:
      sc = f.read(1)
    c.stopRecord("PoserFilePerf.txt")
    ...
    fn = 'srcdata/PoserRoot/Runtime/Librairies/Character/tu1/PHF-LoRes.cr2'
    c = ChronoMem.start("PERFO READ PoserFile.read-PHF-LoRes.cr2+readlines")
    f = open(fn, 'rt')
    t = f.readlines()
    c.stopRecord("PoserFilePerf.txt")
    print("readLines:" + str(len(t)))
    f.close()
    
    fn = "srcdata/PoserRoot/Runtime/Librairies/Character/tu1/PHF-LoRes.cr2"
    c = ChronoMem.start("PERFO READ PoserFile.read-PHF-LoRes.cr2+readline")
    with open(fn) as fp:
      line = fp.readline()
      cnt = 1
      while line:
