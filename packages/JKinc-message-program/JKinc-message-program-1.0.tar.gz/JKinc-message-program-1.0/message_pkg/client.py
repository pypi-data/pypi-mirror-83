...
def receive():
    ...
    while True:
        try:
            ...
            messages = client_socket.recv(BUFSIZ).decode("utf8")
            ...
            for i in range(0, len(messages)):
               results += characters_in_order[shuffled_list.index(messages[i])]

            msg_list.insert(tkinter.END, (results ))
...
r_seed = input('Enter Encryption code: ')
if not PORT:
    PORT = 33000
else:
    PORT = int(PORT)
