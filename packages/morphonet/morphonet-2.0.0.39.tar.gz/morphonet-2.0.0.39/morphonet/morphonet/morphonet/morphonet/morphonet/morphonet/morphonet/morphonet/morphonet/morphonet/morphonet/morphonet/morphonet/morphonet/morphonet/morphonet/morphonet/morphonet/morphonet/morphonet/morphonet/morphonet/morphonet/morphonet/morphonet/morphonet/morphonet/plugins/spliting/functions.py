...
def applyNewLabel(data,xmin,ymin,zmin,labelw,minVol=0):
    import numpy as np
    ...
    newId=True
    if lastID==data.max()+1:
        newId=False
    else:
        coords=np.where(data==BigestL)
        print('     ----->>>>>  Keep object '+str(BigestL)+ " with "+str(len(coords[0]))+ " voxels ")
