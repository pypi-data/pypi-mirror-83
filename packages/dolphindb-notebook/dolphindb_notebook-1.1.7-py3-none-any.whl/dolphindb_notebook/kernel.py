...
class DolphinDBKernel(Kernel):
    ...
    # return a deep copy of original list of dict
    def deep_copy(self, myList):
        copyList = []
        for myDict in myList:
            copyDict = {}
            for myKey in myDict:
                copyDict[myKey] = myDict[myKey]
            copyList.append(copyDict)
        return copyList
    ...
    def do_execute(self, code, silent, store_history=True, user_expressions=None, allow_stdin=False):
        if code.startswith('connect-to-ddb-pre') or code.startswith('connect-to-ddb-new'):
            ...
            self.s = ddb.session(enableSSL=(enableSSL=='true'))
            ...
            if code.startswith('connect-to-ddb-new'):
                ...
                copyCreds = self.deep_copy(self.creds)                
                copyCreds.append(new_cred)
                s = json.dumps(copyCreds)
                f = open("./dolphindb/creds.py", "w")
                f.write(s)

        elif code.startswith('retrieve-credentials'):
            copyCreds = self.deep_copy(self.creds)        
            for cred in copyCreds:
