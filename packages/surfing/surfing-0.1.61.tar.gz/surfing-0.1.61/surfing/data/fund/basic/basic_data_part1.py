...
class BasicDataPart1:
    ...
    def fund_size_hold_rate(self, start_date: str, end_date: str, is_history=True):
        try:
            ...
            fund_scale['fund_id'] = fund_scale.apply(
                lambda x: self._data_helper._get_fund_id_from_order_book_id(x['CODES'].split('.')[0], x['DATES']), axis=1)
            fund_scale = fund_scale[fund_scale.fund_id.notna() & fund_scale.FUNDSCALE.notna()]
            fund_scale = fund_scale.drop(columns=['CODES']).rename(columns={'DATES': 'datetime', 'FUNDSCALE': 'size'})
            ...
            if not is_history:
                ...
                if now_df is not None:
                    now_df = now_df.astype({'size': 'float64', 'personal_holds': 'float64', 'institution_holds': 'float64', 'hold_num': 'float64'})
    ...
    def fund_hold_asset(self, start_date: str, end_date: str, is_history=True):
        try:
            ...
            df = df.drop(columns='_update_time').dropna(subset=['CODES']).drop_duplicates(subset='CODES')
            name_dict = {
                'CODES': 'fund_id',
                'DATES': 'datetime',
    ...
    def fund_hold_industry(self, end_date: str):
        # history, update only on report date
        em_hold_industry = self._raw_data_api.get_em_fund_hold_industry(end_date=end_date)
    ...
    def fund_hold_bond(self, end_date: str):
        #history, update only on report date
        em_hold_bond = self._raw_data_api.get_em_fund_hold_bond(end_date=end_date)
