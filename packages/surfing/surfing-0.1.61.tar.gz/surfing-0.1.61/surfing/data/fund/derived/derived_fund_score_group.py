...
class FundScoreGroupProcess:
    ...
    def _calc_score(self, x: pd.DataFrame, by_key: str) -> pd.DataFrame:
        ...
        # 第六个维度 基金经理评分
        wind_class_2 = self._fund_info.loc[fin_data.index, 'wind_class_2']
        our_type = wind_class_2.map(self._wind_class_to_type)
        managers = self._fund_manager_info.loc[self._fund_manager_info.index.intersection(fin_data.index), 'mng_id']
        comp_key = pd.DataFrame(managers).join(our_type)
        result3 = comp_key.join(self._fund_manager_score['score'], on=['mng_id', 'wind_class_2'], how='outer')
        result3 = result3[result3.score.notna()]
        # 在这里干掉重复的fund+基金经理的数据
        result3 = result3.reset_index().drop_duplicates().set_index('fund_id')
        if not result3.empty:
            result3 = result3['score'].groupby(by='fund_id', sort=False).mean()

        # 合并所有的评分并计算总分
        total = result1.join(result2, how='outer')
        if not result3.empty:
            total = total.join(result3.rename('team_score'), how='outer')
