...
class DataStore:
    ...
    @staticmethod
    def refresh_dm(score_pre_calc=True,start_time='20050101',end_time=None,print_time=False,use_weekly_monthly_indicators=False,s3_retriever_activation=False):
        print(f'start load at {datetime.datetime.now()}')
        t1 = time.time()
        if end_time is None:
            end_time = datetime.datetime.now().strftime('%Y%m%d')
        dm = FundDataManager(start_time=start_time, end_time=end_time, s3_retriever_activation=s3_retriever_activation)
