...
class ManagerProcessor:
    ...
    def init(self, start_date=str, end_date=str, print_time=False, is_mng_index=False, is_mng_indicator=False):
        ...
        with BasicDatabaseConnector().managed_session() as quant_session:
            ...
            ## 计算基金经理指数时去掉场内基金 封闭基金 分级基金 EHR类
            self.fund_info = self.fund_info[
                                            (~self.fund_info.fund_id.isin(self.close_fund_list))
                                            & (~self.fund_info.fund_id.isin(self.in_market_fund_list))
                                            & (self.fund_info.structure_type == 0)
                                            & (~self.fund_info.fund_id.isin(self.ehr_funds))]
    ...
    def _do_calc_part3(self, eval_start_date, eval_date):
        ...
        for class_name, index_id in self.REPLACE_DICT.items():
            ...
            for row in stats.itertuples():
                try:
                    _fund_df = self.fund_manager_info[self.fund_manager_info['manager_id'].isin([row.Index]) & fund_ids]
                    _fund_list = _fund_df.fund_id.tolist()
                    stats.loc[row.Index, 'start_date'] = min(_fund_df['start'])
                    stats.loc[row.Index, 'trade_year'] = (-stats.loc[row.Index, 'start_date'] + stats.loc[row.Index, 'end_date']).days / 365
                    stats.loc[row.Index, 'fund_size'] = self.fund_size.loc[eval_date,_fund_list].sum()
                except:
                    continue
            rf = 1.025 ** (1 / 365) - 1
            cl_result = []
