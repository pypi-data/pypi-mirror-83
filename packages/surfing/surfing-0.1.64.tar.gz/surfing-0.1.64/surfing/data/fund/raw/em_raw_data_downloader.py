...
class EmRawDataDownloader:
    ...
    def em_fund_nav(self, start_date, end_date, fund_list: Tuple[str] = ()):
        try:
            ...
            for date in trade_date_df.TRADEDATES:
                # 基金 万份基金单位收益 7日年化收益率
                data1 = self._em_css_wrapper(fund_list, "10KUNITYIELD,YIELDOF7DAYS", f"EndDate={date},Ispandas=1")
                if data1 is None:
                    print('[em_fund_nav] failed to get fund nav data1')
                    return False
                # 基金 单位净值(原始披露) 累计单位净值(原始披露) 复权单位净值
                data2 = self._em_css_wrapper(fund_list, "ORIGINALUNIT,ORIGINALNAVACCUM,NAVADJ", f"TradeDate={date},Ispandas=1")
                if data2 is None:
    ...
    def em_fund_status(self, end_date: str, is_history: bool = True):
        try:
            ...
            data1 = self._em_css_wrapper(fund_list, 'FUNDSCALE,PCHMINAMT,REDMMINAMT', f'EndDate={end_date},Ispandas=1')
            if data1 is None:
            ...
            if not is_history:
                ...
                if now_df is not None:
                    ...
                    df = df.merge(now_df, how='left', on=['CODES', 'FUNDSCALE', 'LPLIMIT', 'OTCLPLIMITJG', 'OTCLPLIMITGR', 'PURCHSTATUS', 'REDEMSTATUS', 'TRADESTATUS', 'PCHMINAMT', 'REDMMINAMT'], indicator=True, validate='one_to_many')
    ...
    def em_stock_fin_fac(self, end_date: str):
        ...
        df_no_param = self._em_css_wrapper(stock_list,
                                              ...
                                              BPS,MBREVENUE,MBCOST,BALANCESTATEMENT_25,BALANCESTATEMENT_31,BALANCESTATEMENT_46,BALANCESTATEMENT_93,BALANCESTATEMENT_103,BALANCESTATEMENT_141,BALANCESTATEMENT_195,\
                                              ...
                                              INCOMESTATEMENT_14,CASHFLOWSTATEMENT_9,CASHFLOWSTATEMENT_39,CASHFLOWSTATEMENT_59,CASHFLOWSTATEMENT_77,CASHFLOWSTATEMENT_82,CASHFLOWSTATEMENT_86',
        ...
        # 获取主营收入构成(按产品)
        df_mb_sales_cons = self._em_css_wrapper(stock_list, "MBSALESCONS", f'ReportDate={end_date},type=2,Ispandas=1')
        ...
        # 获取合并报表(调整)数据
        df_combined_with_adj = self._em_css_wrapper(stock_list, "BALANCESTATEMENT_141", f'ReportDate={end_date},type=3,Ispandas=1')
        ...
        df_with_param_1 = self._em_css_wrapper(stock_list, "DEDUCTEDINCOME", f'ReportDate={end_date},DataAdjustType=1,Ispandas=1')
        ...
        df_with_param_2 = self._em_css_wrapper(stock_list, "DEDUCTEDINCOME", f'ReportDate={end_date},DataAdjustType=2,Ispandas=1')
        ...
        try:
            ...
            for date in tradedates:
                temp_df = self._em_css_wrapper(stock_list,
                                               'TOTALSHARE,HOLDFROZENAMTACCUMRATIO,PETTMDEDUCTED,PBLYRN,PSTTM,AHOLDER,ESTPEG,SHAREHDPCT',
    ...
    def em_fund_asset(self, end_date: str):
        try:
            ...
            df = self._em_css_wrapper(fund_list, "PRTSTOCKTONAV,PRTBONDTONAV,PRTCASHTONAV,PRTOTHERTONAV,MMFFIRSTREPOTONAV,MMFAVGPTM", f"ReportDate={real_date},Ispandas=1")
            if df is None:
                print(f'[em_fund_assetem_fund_asset] failed to get data')
                return False
            df['DATES'] = real_date
            df = df[df.drop(columns=['DATES']).notna().any(axis=1)].reset_index()
    ...
    def em_fund_stock(self, end_date: str):
        ...
        for dt in dt_list:
            ...
            for rank in rank_list:
                data = c.css(fund_list,"PRTKEYSTOCKNAME,PRTKEYSTOCKCODE,PRTKEYSTOCKVALUE,PRTKEYSTOCKTONAV",f"ReportDate={dt},Rank={rank},Ispandas=1")
    ...
    def em_fund_bond(self, end_date: str):
        try:
            ...
            real_date = RawDataHelper.get_prev_target_date(end_date, QUARTER_UPDATE_DATE_LIST)
            for rank in range(1, 11):
                date = self._em_css_wrapper(fund_list, "PRTKEYBONDNAME,PRTKEYBONDCODE,PRTKEYBONDVALUE,PRTKEYBONDTONAV", f"ReportDate={real_date},Rank={rank},Ispandas=1")
                if date is None:
                    print(f'[em_fund_bond] failed to get data')
                    return False
                date = date.rename(columns={'PRTKEYBONDNAME': f'rank{rank}_bond', 'PRTKEYBONDCODE': f'rank{rank}_bond_code', 'PRTKEYBONDVALUE': f'rank{rank}_bondval', 'PRTKEYBONDTONAV': f'rank{rank}_bondweight'})
                data = date.drop(columns='DATES')
                _res.append(data)
            df = pd.concat(_res, axis=1)
            df = df[df.notna().any(axis=1)].reset_index()
            df['DATES'] = real_date
