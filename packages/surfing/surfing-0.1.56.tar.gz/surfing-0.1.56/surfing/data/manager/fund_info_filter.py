...
def get_ipos_funds(ipo_weight_df):
    # 过去一年如果打新金额占比大于0.15， 作为打新型基金
    ipo_weight_df = ipo_weight_df.pivot_table(index='end_date', columns='fund_id', values='ipo_allocation_weight').fillna(0)
    ipo_weight_last_year_df = ipo_weight_df + ipo_weight_df.shift(1).fillna(0) + ipo_weight_df.shift(2).fillna(0) + ipo_weight_df.shift(3).fillna(0)
    ipo_funds = ipo_weight_last_year_df.columns[ipo_weight_last_year_df.max() > 0.15].values.tolist()
    return ipo_funds

def get_conv_funds(fund_conv_stats):
