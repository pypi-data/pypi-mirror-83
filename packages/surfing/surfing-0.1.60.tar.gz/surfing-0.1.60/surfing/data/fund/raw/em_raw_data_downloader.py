...
class EmRawDataDownloader:
    ...
    def em_fund_holding_rate(self, end_date: str):
        try:
            ...
            df['HOLDINSTIHOLDINGPCT'] = df.transform(lambda x: 0 if x.HOLDPERSONALHOLDINGPCT == 100 else x.HOLDINSTIHOLDINGPCT, axis=1)
            df['HOLDPERSONALHOLDINGPCT'] = df.transform(lambda x: 0 if x.HOLDINSTIHOLDINGPCT == 100 else x.HOLDPERSONALHOLDINGPCT, axis=1)
            now_df = RawDataApi().get_em_fund_holding_rate(start_date=real_date, end_date=real_date)
            if now_df is not None:
                df = df.astype({'HOLDPERSONALHOLDINGPCT': 'float64', 'HOLDINSTIHOLDINGPCT': 'float64', 'HOLDNUM': 'float64'})
        ...
        try:
            ...
            if not is_history:
                ...
                if now_df is not None:
                    ...
                    df = df.merge(now_df, how='left', on=['CODES', 'FUNDSCALE', 'LPLIMIT', 'OTCLPLIMITJG', 'OTCLPLIMITGR', 'PURCHSTATUS', 'REDEMSTATUS', 'TRADESTATUS'], indicator=True, validate='one_to_many')
    ...
    def em_fund_asset(self, end_date: str):
        try:
            ...
            if now_df is not None:
                df = df.astype({'PRTSTOCKTONAV': 'float64', 'PRTBONDTONAV': 'float64', 'PRTCASHTONAV': 'float64', 'PRTOTHERTONAV': 'float64', 'MMFFIRSTREPOTONAV': 'float64', 'MMFAVGPTM': 'float64'})
                now_df = now_df.astype({'PRTSTOCKTONAV': 'float64', 'PRTBONDTONAV': 'float64', 'PRTCASHTONAV': 'float64', 'PRTOTHERTONAV': 'float64', 'MMFFIRSTREPOTONAV': 'float64', 'MMFAVGPTM': 'float64'})
                # 过滤出来新增基金benchmark以及旧基金的benchmark发生变化的情况
                now_df = now_df.drop(columns=['_update_time']).sort_values(by=['CODES', 'DATES']).drop_duplicates(subset='CODES', keep='last')
                # merge on all columns
                df = df.merge(now_df, how='left', indicator=True, validate='one_to_one')
