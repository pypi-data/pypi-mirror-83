...
class ColonFilterSchema(BaseFilterRuleSchema):
    '''
    The Tenable.io API Version 2 filter format schema.
    ...
    def reformat_filter(self, rule, idx=0):
        '''
        Converts the basic filter rule format into the v2 (colon) format.
        '''
        return '{name}:{oper}:{value}'.format(**rule)