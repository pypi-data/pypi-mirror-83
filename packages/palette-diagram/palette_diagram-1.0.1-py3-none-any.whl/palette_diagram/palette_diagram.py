...
def preprocessing(df, remove_empty_groups):
    df = df.fillna(0)
    if (remove_empty_groups == 0) or (remove_empty_groups == 2):
        df = remove_all_zeros(df, axis=0)
    if (remove_empty_groups == 1) or (remove_empty_groups == 2):
        df = remove_all_zeros(df, axis=1)
    return df

def circular_palette_diagram(df, n_neighbors, n_epochs, lr, norm, export, export_table, group_names, cmap_name):
    ...
    group_names_internal = df.columns.tolist()
    group_names_internal = np.array(group_names_internal)[layer_order]
    if group_names == None:
        group_names = df.columns.tolist()

    if cmap_name != None:
        color_palette = qualitative_color_pallete_cmap(len(group_names), cmap_name)
    else:
        if len(group_names) <= 5:
            color_palette = qualitative_color_pallete_5(len(group_names))
        else:
            color_palette = equidistant_color_pallete(len(group_names))

    color_codes = {name:color_palette[idx] for idx,name in enumerate(group_names)}
    color_list = np.array([color_codes[name] for name in group_names_internal])
    ...
    for i in range(X.shape[1] + 1):
        ...
        y2 = y1 + X[:, i - 1]
        plt.fill_between(angles, y1, y2, facecolor=color_list[i - 1], alpha=0.7, label=group_names_internal[i-1])
...
def linear_palette_diagram(df, n_neighbors, norm, export, export_table, group_names, cmap_name):
    group_names_internal = df.columns.tolist()
    if group_names == None:
        group_names = df.columns.tolist()
    ...
    color_codes = {name:color_palette[idx] for idx,name in enumerate(group_names)}
    color_list = [color_codes[name] for name in group_names_internal]
    ...
    ax.stackplot(range(X.shape[0]), X.T, labels=group_names_internal, 
...
#-------- main --------
def palette_diagram(df, palette_type='circular', n_neighbors=100, n_epochs=100, lr=0.0005, norm=True, export=True, export_table=True, group_names=None, cmap_name=None, remove_empty_groups=-1):
    df = preprocessing(df, remove_empty_groups)
    ...
    if palette_type == 'circular':
        circular_palette_diagram(df, n_neighbors_, n_epochs, lr, norm, export, export_table, group_names, cmap_name)

    elif palette_type == 'linear':
        linear_palette_diagram(df, n_neighbors_, norm, export, export_table, group_names, cmap_name)
