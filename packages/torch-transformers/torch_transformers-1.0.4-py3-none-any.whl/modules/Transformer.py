...
from modules.utils import create_padding_mask, create_look_ahead_mask
...
class Transformer(nn.Module):
    ...
    """
    def __init__(self, vocab_size, d_model, d_ff, num_heads, num_layers, dropout=0.1, use_embed_weights=True,
                 set_seq_len=None, device=torch.device("cpu")):
        """
        ...
        self.embedding_scale = np.sqrt(d_model)    # the paper suggests that we divide our embeddings by sqrt(d_model)
        ...
        nn.init.normal_(self.embedding_module.weight, mean=0, std=d_model ** -0.5)

        self.embedding_dropout = nn.Dropout(dropout)    # regularization for embeddings
        ...
        if use_embed_weights:
            self.project_to_vocabs = lambda decoder_out: self.embedding_module.weight.transpose(0, 1) @ decoder_out
        ...
        self.positional_encoder = PositionalEncoding(d_model, set_seq_len, device)
    ...
    def encode(self, input_sentences):
        ...
        input_sentences = self.embedding_module(input_sentences)    # we convert our input into word embeddings
        ...
        input_sentences.masked_fill_(padding_mask.unsqueeze(2), 0)    # this does an inplace fill
    ...
    def decode(self, decoded_outputs, encoder_outputs, cache=None):
        ...
        look_ahead_mask = create_look_ahead_mask(decoded_outputs, padding_mask=padding_mask)
        decoded_outputs = self.embedding_module(decoded_outputs)    # convert to word embeddings
        ...
        return self.decoder(decoded_outputs, encoder_outputs, padding_mask, look_ahead_mask, cache)
