...
class Geometry():
    ...
    def from_points(cls, points_coords, contacts_positions, resolution = 20):
        """Constructs a polygonal geometry given a list of points and contact positions
        ...
        #creates a polygon from the list of points
    ...
    def from_txt_file(cls, filename, resolution = 20):
        """Constructs a polygonal geometry from a txt file.
        The file format must be as follows: points defining the polygon should be ordered
        points belonging to a contact should be marked by integers from 0 to Nmax 
        ...
        n_contacts = int(np.nanmax(contacts_column)) + 1
        for i in range(n_contacts):
            contacts_positions.append((np.nonzero(contacts_column == i)[0]).tolist())
        return cls.from_points(points_coords= points_coords, contacts_positions = contacts_positions, resolution = resolution)

    @classmethod
    def from_msh_file(cls, filename, n_contacts, dim = 2, dim_keep = None):
        """Constructs a geometry given a .msh file 
        ...
        #if np.unique(mesh_dic['boundary_marker'].array()) != np.arange(1,n_contacts + 1):
        #    warn('contacts {} will be ignored'.format(np.setdiff1d(np.unique(mesh_dic['boundary_marker'].array()), np.arange(1,n_contacts + 1))))
        print(dol.assemble(dol.Constant(1) * mesh_dic['volume_measure']))
    ...
    def plot(self, **kwargs):
        """plots the mesh and the contacts 
        """        
        plt.axis('equal')
    ...
    def check_dimensions(self):
        """prints total D-dimensional volume and areas of the contacts (area and lenghts in 2D)
        """        
...
class Problem():
    ...
    def __init__(self, geometry, n_fields = 1, self_adjoint = False,
        ...
        self._function_space = dol.FunctionSpace(self.geometry.mesh, element)
        self._scalar_function_space = dol.FunctionSpace(self.geometry.mesh, dol.FiniteElement('P', self.geometry.mesh.ufl_cell(), degree = 1))
        self._vector_function_space = dol.VectorFunctionSpace(self.geometry.mesh, 'DP', degree = 0)
    ...
    def currents_cells(self):
        ...
        for alpha in range(self.n_fields):
            current_cells.append([self._currents[alpha].vector().get_local() [self._vector_function_space.dofmap().cell_dofs(cell.index())] for cell in dol.cells(self.geometry.mesh)])
    ...
    def _solve(self, L, Gamma, contact_resistances, biases, F):
        ...
        u = dol.TrialFunction(self._function_space) 
        v = dol.TestFunction(self._function_space)    
        # quadratic form
        quadratic_terms = []
        for alpha in range(self.n_fields):
        ...
        if self.n_fields > 1:
            for n in range(self.geometry.n_contacts):
                for beta in range(self.n_fields):
                    if contact_resistances[beta, n] == 0.:
                        bc.append(dol.DirichletBC(self._function_space.sub(beta), 
            ...
            for n in range(self.geometry.n_contacts):
                if contact_resistances[0, n] == 0.:
                    bc.append(dol.DirichletBC(self._function_space, 
                                              dol.Constant([biases[0, n]]), 
                                              self.geometry.contacts_marker,
                                                n+1))
        #solve linear system
        phi = dol.Function(self._function_space) #clear phi
    ...
    def solve(self, biases = None, F = None):
        ...
        if (not F is None):
            ...
            self.F = F
        #add checks
        #check if the solution can be found as a linear combination
    ...
    def compute_currents(self):
        ...
        if self.n_fields > 1.:
            self._currents = [dol.project(- sum([self.L[alpha][beta] * dol.grad(self._fields[beta]) for beta in range(self.n_fields)]), self._vector_function_space) for alpha in range(self.n_fields)]
        else:
            self._currents = [dol.project(- self.L[0][0] * dol.grad(self._fields[0][0]) , self._vector_function_space)]
...
#changed sign
    ...
    def compute_solution_basis(self):
        '''$ Calculates a solution basis for the homogeneous equation given by 
        $\phi^{(\alpha,m)}(\bm r)$, which are the solutions of the homogeneous 
        problem with $V^{\beta, m } = \delta_{\alpha \beta}\delta{m n}$ 
        '''
        if self._solution_basis is None:
    ...
    def compute_adjoint_solution_basis(self):
        '''$ Calculates a solution basis for the adjoint homogeneous equation given by 
...
#changed sign
    def compute_response_matrix(self, method = 1):
        '''$ Calculates the response matrix G
        $G^{(\alpha\,m,\beta\,n)} = \sum_{ij\gamma}\int_{C_m} ds \,\hat{n}_i L_{\alpha\gamma, i j}(\bm r) \partial_j \Phi^{(\beta,n)}_\gamma(\bm r)$        
        Here $\phi^{(\alpha,m)}(\bm r)$ is the solution of the homogeneous problem 
        with $V^{\beta, m } = \delta_{\alpha \beta}\delta{m n}$
...
#changed sign
    def compute_source_vector(self, method = 1):
        '''$Calcualtes the matrix S[F]

        $S^{(\alpha\,m)}=\int_\Omega d \bm r [L_{\gamma\delta\, ij}(\bm r)
        \partial_i\phi_\gamma^{(\alpha,m)}(\bm r) \partial_j\phi^F_\delta(\bm r)
        +\Gamma_{\gamma\delta}(\bm r)\phi_\gamma^{(\alpha,m)}(\bm r)\phi^F_\delta
        (\bm r)
        -F_\gamma (\bm r) \phi_\gamma^{(\alpha,m)}(\bm r)]$.

        Here $\phi_F(\bm r)$ is the solution of the inhomogeneous equation with no 
        biases applied.
        '''
        if self._solution_inhomogeneous is None:
            self.compute_solution_inhomogeneous()
    ...
    def save(self, filename):
        """Save fields and currents to XDMF file

        Parameters
        ----------
        filename : str
            name of the .xdmf file
        """        
        with dol.XDMFFile(filename) as xdmf:
            if self._n_fields > 1:
                for alpha in range(self.n_fields):
                    xdmf.write_checkpoint(self._fields[alpha], 'field{}'.format(alpha),append =True)
            else:
                xdmf.write_checkpoint(dol.project(self._fields[0][0], self._scalar_function_space), 'field{}'.format(0),append =True)
            if not(self._currents is None):
                for alpha in range(self.n_fields):
                    xdmf.write_checkpoint(self._currents[alpha], 'current{}'.format(alpha),append =True)
