...
class XTController(rpyc.Service):
    ...
    def process_multi_run_context(self, multi_run_context_fn):
        ...
        if self.node_restart:
            ...
            completed_run_docs = self.store.db.get_info_for_runs(self.ws_name, filter_dict, {"run_index": 1})
    ...
    def write_connect_info_to_job(self, store, job_id):
        ...
        if os.getenv("PHILLY_CONTAINER_PORT_RANGE_START"):
            ...
            if store.db:
                store.db.update_connect_info_by_node(self.ws_name, job_id, self.node_id, connect_info)
    ...
    def spawn_child(self, parent, run_index, entry):
        ...
        child_aml_run = None
        child_name = self.store.db.get_child_name(entry, self.parent_run_name, self.first_run_index)
