...
def get_client_cs(core, ws_name, job_id, node_index):
    ...
    filter = {"_id": job_id}
    jobs = core.store.db.get_info_for_jobs(ws_name, filter, None)
    if not jobs:
...
def get_job_info_with_backend(store, core, workspace, job_id, node_index):
    ...
    if not "service_info_by_node" in job_info:
        ...
        service_info_by_node = store.db.get_service_info_by_node(workspace, job_id)
...
def get_service_job_info(store, core, ws_name, job_id):
    ...
    # service_info_by_node = store.db.get_service_info_by_node(ws_name, job_id)
