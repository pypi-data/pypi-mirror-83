...
class StoreV2():
    ...
    def __init__(self, storage_creds=None, config=None, max_retries=25, run_cache_dir=None, feedback_enabled=True,  db_creds=None, 
        ...
        if self.db_type == "mongo":
            self.db = mongo_v2.MongoDB2(self.db_conn_str, run_cache_dir=run_cache_dir, store=self, 
                db_options=db_options) 
        
        elif self.db_type == "odbc":
            self.db = odbc.OdbcDatabase(db_creds, store=self, db_options=db_options) 

        elif self.db_type:
            errors.config_error("Unsupported database type: {}".format(self.db_type))

        if self.db:
            self.helper.validate_storage_and_db(self.db)
    ...
    def delete_share(self, share_name):
        ...
        if result:
            ...
            self.db.remove_cache(share_name)
    ...
    def ensure_workspace_exists(self, ws_name, flag_as_error=True):
        ...
        if True:  # created:
            self.db.create_workspace_if_needed(ws_name)
    ...
    def create_workspace(self, ws_name, database=None):
        ...
        self.db.create_workspace_if_needed(ws_name, database)
    ...
    def delete_workspace(self, ws_name):
        ...
        if result:
            ...
            result2 = self.db.delete_workspace_if_needed(ws_name)
    ...
    def get_workspace_names(self, use_database=False):
        ...
        if use_database:
            ...
            names = self.db.get_workspace_names()
        else:
    ...
    #     return self.db.get_job_workspace(job_id)
    ...
    def get_experiment_names(self, ws_name):
        ...
        if use_runs:
            ...
            items = self.db.get_all_experiments_in_ws(ws_name)
    ...
    def get_run_names(self, ws_name):
        ...
        records = self.db.get_info_for_runs(ws_name, {"ws_name": ws_name}, {"run_name": 1})
    ...
    def _create_next_run_directory(self, ws_name, is_parent):
        ...
        while True:
            run_id = self.db.get_next_run_id(ws_name, default_next=default_next)
    ...
    def _create_next_child_directory(self, ws_name, parent_name, child_name):
        ...
        if not child_name:
            child_id = self.db.get_next_child_id(ws_name, parent_name, default_next=1)
    ...
    def update_run_at_end(self, ws_name, run_name, status, exit_code, end_time, log_records, hparams, metrics):
        ...
        if self.db:
            result = self.db.update_run_at_end(ws_name, run_name, status, exit_code, end_time, log_records, hparams, metrics)
    ...
    def download_all_runs_blobs(self, ws_name, aggregator_dest, job_or_exper_name, dest_fn, 
        ...
        if aggregator_dest == "job":
            ...
            nodes = self.db.get_info_for_nodes(ws_name, {"job_id": job_or_exper_name}, {"node_id": 1})
    ...
    def rollup_and_end_run(self, ws_name, run_name, aggregate_dest, dest_name, status, exit_code, primary_metric, maximize_metric, 
        ...
        # if self.db:
        #     self.db.update_run_at_end(ws_name, run_name, status, exit_code, end_time, log_records, hparams, metrics)
    ...
    def log_run_event(self, ws_name, run_name, event_name, data_dict=None, event_time=None, job_id=None):
        ...
        if self.db:
            ...
            self.db.process_run_event(ws_name, run_name, event_name, data_dict, record_dict)

    def get_job_names(self, filter_dict=None, fields_dict=None):
        return self.db.get_job_names(filter_dict, fields_dict)
    ...
    def get_all_runs(self, aggregator_dest, ws_name, job_or_exper_name, filter_dict=None, fields_dict=None, use_cache=False, 
        ...
        return self.db.get_all_runs(aggregator_dest, ws_name, job_or_exper_name, filter_dict, fields_dict, use_cache,
    ...
    def wrapup_run(self, ws_name, run_name, aggregate_dest, dest_name, status, exit_code, primary_metric, maximize_metric, 
        ...
        if self.db:
            ...
            self.db.run_exit(ws_name, run_name, status, exit_code, db_retries, storage_retries, 
                 start_time)
            ...
            if not is_parent:
                ...
                self.db.job_run_exit(ws_name, job_id, exit_code)

                utils.log_info("calling DATABASE node_run_end", run_name)
                self.db.node_run_end(ws_name, job_id, node_index, exit_code)
    ...
    def create_job(self, ws_name):
        job_num = self.db.get_next_job_id(ws_name, "next_job_number")
    ...
    def get_job_secret(self, ws_name, job_id):
        ...
        records = self.db.get_info_for_jobs( filter_dict={"_id": key}, fields_dict={"job_secret": 1} )
