...
class Run():
    ...
    def __init__(self, config=None, store=None, xt_logging=True, aml_logging=True, checkpoints_enabled=True,
        ...
        if self.xt_logging and self.direct_run and self.store:
            ...
            if self.store.db:
                self.store.db.run_start(self.ws_name, self.run_name)
                if self.context:
                    self.store.db.job_run_start(self.ws_name, self.context.job_id)
    ...
    def close(self):
        ...
        has_store = self.store.actual_store is not None
        ...
        if has_store and self.store.db:        
            self.store.db.on_run_close(self.ws_name, self.run_name)

            print("RUN: database call stats")
            self.store.db.print_call_stats()
    ...
    def log_metrics(self, data_dict, step_name=None, stage=None):
        ...
        if self.store and self.xt_logging:
            ...
            if metric_names_changed:
                ...
                if self.store.db:
                    self.store.db.update_run_info(self.ws_name, self.run_name, ddx)
