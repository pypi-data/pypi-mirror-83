...
class StoreV1():
    ...
    def __init__(self, storage_creds=None, config=None, max_retries=25, run_cache_dir=None, feedback_enabled=True,  db_creds=None, 
        ...
        self.db = mongo_v1.MongoDB(mongo_conn_str, run_cache_dir) if mongo_conn_str else None
        ...
        self.helper.validate_storage_and_mongo(self.db)
    ...
    def delete_workspace(self, ws_name):
        ...
        if result:
            ...
            self.db.delete_workspace_if_needed(ws_name)
    ...
    def log_run_event(self, ws_name, run_name, event_name, data_dict=None, event_time=None, job_id=None):
        ...
        self.db.process_run_event(ws_name, run_name, record_dict)
    ...
    def get_ws_runs(self, ws_name, filter_dict=None, include_log_records=False):
        return self.db.get_ws_runs(ws_name, filter_dict, include_log_records)
    ...
    def wrapup_run(self, ws_name, run_name, aggregate_dest, dest_name, status, exit_code, primary_metric, maximize_metric, 
        ...
        self.db.run_exit(ws_name, run_name, status=None, exit_code=None, start_time=start_time)
    ...
    def update_run_at_end(self, ws_name, run_name, status, exit_code, end_time, log_records, hparams, metrics):
        return self.db.update_run_at_end(ws_name, run_name, status, exit_code, end_time, log_records, hparams, metrics)
    ...
    def create_job(self, ws_name):
        ...
        job_id = self.db.get_next_job_id(default_next=default_next)

        job_name = "job{}".format(job_id)
        return job_name

    def get_job_secret(self, ws_name, job_id):
        records = self.db.get_info_for_jobs( filter_dict={"_id": job_id}, fields_dict={"job_secret": 1} )
    ...
    def write_job_info_file(self, ws_name, job_id, text):
        ...
        dd = json.loads(text)
        self.db.update_job_info(ws_name, job_id, dd)
