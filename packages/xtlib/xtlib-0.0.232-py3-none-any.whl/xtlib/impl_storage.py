...
class ImplStorage(ImplBase):
    ...
    def view_metrics(self, runs, workspace, steps, merge, metrics, hparams, export, clean):
        ...
        pure_run_list, ws_name = run_helper.expand_run_list(self.store, self.store.db, workspace, runs)
        ...
        run_log_records = self.store.db.get_info_for_runs(ws_name, fd, fields, include_log_records=True)
    ...
    def plot(self, runs, col_list, x, layout, break_on, title, show_legend, plot_titles, legend_titles, 
        ...
        max_workers, x_int, y_int, child, outer, parent, bins, source, plot_type):
        ...
        pb = plot_builder.PlotBuilder(self.store, obj_names, col_list, x_col, layout, break_on, title, 
            ...
            legend_suffix, source, x_int, y_int, bins, legend_vars, plot_args)
    ...
    def extract(self, runs, dest_dir, workspace, response, browse):
        ...
        if extract:
            ...
            pure_run_list, actual_ws = run_helper.expand_run_list(self.store, self.store.db, workspace, runs)
            ...
            for run_name in pure_run_list:
                ...
                if nodes == "exper":
                    ...
                    record = self.store.db.get_info_for_runs(actual_ws, {"_id": run_name}, {"job_id": 1, "exper_name": 1})
                    ...
                    record = self.store.db.get_info_for_runs(actual_ws, {"_id": id}, {"job_id": 1})
                    job_id = utils.safe_cursor_value(record, "job_id")
    ...
    def explore(self, aggregate_name, workspace, cache_dir, steps_name, log_interval_name, 
        ...
        if job_helper.is_job_id(dest_name):
            ...
            records = self.store.db.get_info_for_jobs(workspace, filter_dict, fields_dict)
    ...
    def view_database(self, name, workspace, log_records):
        ...
        if run_helper.is_run_name(name):
            ...
            db_docs = self.store.db.get_info_for_runs(ws, filter_dict, fields_dict)
