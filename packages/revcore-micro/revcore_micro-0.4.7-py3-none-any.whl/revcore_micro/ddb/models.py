...
class Model:
    ...
    def get(cls, **kwargs):
        ...
        try:
            resp = client.get_item(Key=key, TableName=cls.TABLE_NAME)
    ...
    def delete(cls, **kwargs):
        ...
        if cls.sort_key:
            key[cls.sort_key] = kwargs[cls.sort_key]
        resp = client.delete_item(Key=key, TableName=cls.TABLE_NAME)
    ...
    def query(cls, index_name=None, page=False, max_items=60, starting_token=None, page_size=None, **kwargs):
        ...
        if page:
            client = client.get_paginator('query')
            page_config = {'MaxItems': 60}
            if starting_token:
                page_config['StartingToken'] = starting_token
            if page_size:
                page_config['PageSize'] = page_size
            extra['PaginationConfig'] = page_config
            return client.paginate(**extra)
    ...
    def batch_write(cls, items: list):
        ...
        with client.batch_writer() as batch:
            for item in items:
                ...
                batch.put_item(Item=_item, TableName=cls.TABLE_NAME)
                result.append(_item)
        return result
    ...
    def scan(cls, **kwargs):
        client = cls._get_client()
        kwargs['TableName'] = cls.TABLE_NAME
