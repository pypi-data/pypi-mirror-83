...
class VGG_16(dock.Craft):
    def __init__(self, in_shape, p_drop=0.5, scope='VGG_16'):
        ...
        pad = dock.autoPad((round(H / 4), round(W / 4)), (2, 2), stride=2)
        self.mpool_3 = dock.MaxPool((2, 2), padding=pad)

        pad = dock.autoPad((round(H / 8), round(W / 8)), (3, 3))
        ...
        pad = dock.autoPad((round(H / 8), round(W / 8)), (2, 2), stride=2)
        self.mpool_4 = dock.MaxPool((2, 2), padding=pad)

        pad = dock.autoPad((round(H / 16), round(W / 16)), (3, 3))
        ...
        pad = dock.autoPad((round(H / 16), round(W / 16)), (2, 2), stride=2)
        self.mpool_5 = dock.MaxPool((2, 2), padding=pad)

        self.dropout = dock.Dropout(0.2, dim=2)
        self.glob_conv_1 = dock.Conv(4096, 4096, (round(H / 32), round(W / 32)))
