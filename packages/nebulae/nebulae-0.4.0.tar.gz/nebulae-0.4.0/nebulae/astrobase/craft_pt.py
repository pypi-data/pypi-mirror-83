...
class Craft(nn.Module):
    ...
    def gear(self, gr):
        if isinstance(gr, bool):
            if gr:
                super(Craft, self).train()
            else:
                super(Craft, self).eval()
    ...
    def __getitem__(self, key):
        ...
        return craft.__dict.get(paths[-1], craft)
...
class MaxPool(Craft):
    def __init__(self, kernel: tuple, stride=2, padding=0, is_global=False, scope='MPOOL'):
...
class AvgPool(Craft):
    def __init__(self, kernel: tuple, stride=2, padding=0, is_global=False, scope='APOOL'):
...
class BN(Craft):
    def __init__(self, out_chs, dim, mmnt=0.9, beta=False, gamma=False, scope='BN'):
        ...
        self.bn = bn_fn(out_chs, momentum=1-mmnt, affine=beta or gamma, eps=1e-5)
