...
def _bottleNeckRes(sc, comp, inputs, shallow, deep, scope, name, down_sample=False, pre_activate=True):
    COMP = Component(comp.channel_major, comp.time_major)
    inputs, is_train = inputs
    if pre_activate:
    ...
    res_path = _residual(comp, (inputs, is_train), shallow, deep, stride, name)
    return sc.assemble(idt_path + res_path, sub_scope=scope)

def _resnet_v2(sc, comp, inputs, is_train, scope, blocks):
    COMP = Component(comp.channel_major, comp.time_major)
    ...
    # building block 1
    net = _bottleNeckRes(sc, comp, [net, is_train], 64, 256, scope, pre_activate=False, name='block1_0')
    for l in range(1, blocks[0]):
        net = _bottleNeckRes(sc, comp, [net, is_train], 64, 256, scope, name='block1_'+str(l))
    # building block 2
    net = _bottleNeckRes(sc, comp, [net, is_train], 128, 512, scope, down_sample=True, name='block2_0')
    for l in range(1, blocks[1]):
        net = _bottleNeckRes(sc, comp, [net, is_train], 128, 512, scope, name='block2_'+str(l))
    # building block 3
    net = _bottleNeckRes(sc, comp, [net, is_train], 256, 1024, scope, down_sample=True, name='block3_0')
    for l in range(1, blocks[2]):
        net = _bottleNeckRes(sc, comp, [net, is_train], 256, 1024, scope, name='block3_' + str(l))
    # building block 4
    net = _bottleNeckRes(sc, comp, [net, is_train], 512, 2048, scope, down_sample=True, name='block4_0')
    for l in range(1, blocks[3]):
        net = _bottleNeckRes(sc, comp, [net, is_train], 512, 2048, scope, name='block4_' + str(l))
    return sc.assemble(COMP.AVG_POOL(name='avg_pool', inputs=net,
                                     kernel=[7, 7], keep_size=False, if_global=True), sub_scope=scope)

def RESNET_V2_50(sc, comp, inputs, is_train, scope='Res_50'):
    return _resnet_v2(sc, comp, inputs, is_train, scope, [3, 4, 6, 3])

def RESNET_V2_101(sc, comp, inputs, is_train, scope='Res_101'):
    return _resnet_v2(sc, comp, inputs, is_train, scope, [3, 4, 23, 3])

def RESNET_V2_152(sc, comp, inputs, is_train, scope='Res_152'):
    return _resnet_v2(sc, comp, inputs, is_train, scope, [3, 8, 36, 3])