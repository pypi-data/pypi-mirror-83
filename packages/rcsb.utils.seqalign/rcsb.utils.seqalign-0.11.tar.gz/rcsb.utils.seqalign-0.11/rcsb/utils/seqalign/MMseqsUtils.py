...
class MMseqsUtils(object):
    ...
    def parseAlignmentFile(self, inpFileName="antibodyAlign.txt", outFileName="antibodyAlign.csv"):
        headerL = "query,target,pident,evalue,qlen,tlen,alnlen,taxid,taxname".split(",")
        ...
        self.__mU.doExport(outFileName, rowDictL, fmt="csv", fieldNames=headerL)
    ...
    def __formatSearchResults(self, queryDbPath, targetDbPath, resultDbPath, resultPath, outPath, **kwargs):
        ...
        try:
            ...
            ok = exU.run(
                self.__mmseqs2BinPath,
                execArgList=["convertalis", queryDbPath, targetDbPath, resultDbPath, resultPath] + fOptL,
                outPath=outPath,
                outAppend=True,
                timeOut=timeOut,
            )
