...
class FinderSidebar:
    ...
    def update(self) -> None:
        ...
        for item in self.snapshot[0]:
            ...
            if name not in self.favorites:
                self.favorites[name] = []
            self.favorites[name].append(path)
    ...
    def get_index_by_path(self, path: str) -> int:
        for index, item in enumerate(self.snapshot[0]):
            ...
            found_path = LSSharedFileListItemCopyResolvedURL(item, 0, None)[0].path()
            if os.path.realpath(path) == found_path:
                return index
