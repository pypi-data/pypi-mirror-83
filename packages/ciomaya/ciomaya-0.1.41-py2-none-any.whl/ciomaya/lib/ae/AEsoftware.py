...
def create_ui(node_attr):
    ...
    with AEcommon.ae_template():
        ...
        pm.menuItem(label="Refresh Software")
...
def _ensure_connection(host_attr, plugin_attr, widgets, force=False):
    """Fetch list of sw from Conductor.

    Use cached data on the module if force is False"""

    software_data = coredata.data(force_software=force).get("software")
    if not coredata.valid():
        return
...
def _configure_popup_menu(host_attr, plugin_attr, widgets):
    ...
    detect_item, add_plugin_item, refresh_item = pm.popupMenu(
    ...
    pm.menuItem(refresh_item, edit=True, command=pm.Callback(
        _ensure_connection, host_attr, plugin_attr, widgets, True))
