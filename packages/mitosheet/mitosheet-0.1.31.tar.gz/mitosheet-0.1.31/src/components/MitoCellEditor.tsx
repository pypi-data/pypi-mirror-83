...
export default class MitoCellEditor extends Component<CellEditorProps, CellEditorState> {
  ...
  */
  getCurrentSuggestions() : {match: string; suggestions: Array<FunctionDocumentationObject>} | undefined {
    ...
    if (typeof this.state.formula == 'string') {
      const functionMatches = this.state.formula?.match(suggestionBoxRe);
      if (!functionMatches) {
        return undefined;
      }
      const lastMatch = functionMatches[functionMatches.length - 1];
      const lastMatchUpper = lastMatch.toUpperCase();
      if (this.state.formula.endsWith(lastMatch)) {
  ...
  */
  ...
  selectSuggestion(startOfFunction: string, restOfFunction: string) : void {
    ...
    const stripped = this.state.formula.substring(0, this.state.formula.length - startOfFunction.length);
    const newFormula = stripped + startOfFunction.toUpperCase() + restOfFunction.toUpperCase() + '('
