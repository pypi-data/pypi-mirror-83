...
class ICsvDataService(IDataService):
    ...
    def add_or_update(self, obj_identifier: str, objs: List[T], objs_as_df: pd.DataFrame = None) -> List[T]:
        ...
        if all_data is not None:
            updated_data = objs_as_df if objs_as_df else pd.DataFrame([vars(obj) for obj in objs])
            ...
            all_data = objs_as_df if objs_as_df else pd.DataFrame([vars(obj) for obj in objs])
