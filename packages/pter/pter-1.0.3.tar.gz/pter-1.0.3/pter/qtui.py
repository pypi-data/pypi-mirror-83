...
class TaskList(QtWidgets.QTreeView):
    def __init__(self, config, menu, parent):
        ...
        self.human_friendly_dates = set(config.list(common.SETTING_GROUP_GENERAL,
                                                    common.SETTING_HUMAN_DATES))
        self.attr_highlight = parse_colors(config, common.SETTING_GROUP_GUICOLORS)
        self.color = parse_colors(config, common.SETTING_GROUP_GUIHIGHLIGHT)
...
class DockWithSettings(QtWidgets.QDockWidget):
    ...
    def restore_at(self, window):
        ...
        pos = [int(v) for v in self.settings.list(self.group, SETTING_POSITION, "")]
        if len(pos) == 2:
            self.move(*pos)

        size = [int(v) for v in self.settings.list(self.group, SETTING_SIZE, "")]
        if len(size) == 2:
            self.resize(*size)
