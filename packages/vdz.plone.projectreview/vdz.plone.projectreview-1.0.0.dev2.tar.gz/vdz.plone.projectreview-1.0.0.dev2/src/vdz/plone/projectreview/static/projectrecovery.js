...
$(document).ready(function() {
    var config = {
        '.chosen'           : {display_selected_options: false,
    ...
    $('.fileinput-reset-group').hide();

    $('select#recovery_option').on('change',
        function (evt, params) {
            var slave_dropdown = document.getElementById('recovery_type');
            var recovery_option = this.options[this.selectedIndex].value,
                recovery_type = slave_dropdown.options[slave_dropdown.selectedIndex].value;
            $.ajax({type: 'POST',
                    url: '/@@projectreview/ajax_recovery_types',
                    data: {'recovery_option': recovery_option,
                           'recovery_type':   recovery_type},
                    success: function (data) {
                        var opts = $.parseJSON(data),
                            slave_jq = $(slave_dropdown);
                        slave_jq.empty();
                        $.each(opts, function (i, d) {
                            slave_jq.append('<option value="'+d.id
                                            + (d.selected ? '" selected="selected' : '')
                                            +'">' + d.label
                                            + '</option>');
                        })
                        slave_jq.trigger('chosen:updated');
                    }})
        });

    $('select#recovery_status').each(function () {
        var status_field = $(this),
            cf_id = 'activity_notes',
            comment_field = $('#'+cf_id),
            switched_label = $('#'+cf_id+'-required'),
            switchfunc = function () {
                if (status_field.val() == '9') {
                    comment_field.attr('required', true);
                    switched_label.show();
                    return true;
                } else {
                    comment_field.removeAttr('required');
                    switched_label.hide();
                    return false;
                }
            };
        ...
        status_field.change(function () {
            if (switchfunc()) {
                var tim,
                    f = function () {
                            clearTimeout(tim);
                            document.getElementById(cf_id).focus();
                    };
                tim = setTimeout(f, 10);
            }
        })
    ...
    // wegen Konflikten mit browser-nativer type=date-Unterstützung;
    // alternative Möglichkeit: Modernizr.inputtypes.date
    /* ... oder Verarbeitung durch Bootstrap!
    ...
    */
    ...
    $('input[type=file]').change(function () {
        var filefield = $(this),
            selradio = filefield.data('radio'),
            unselect_group = filefield.data('reset-group');
        if (selradio) {
