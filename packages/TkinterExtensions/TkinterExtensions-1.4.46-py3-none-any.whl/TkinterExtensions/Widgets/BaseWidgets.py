...
class BaseTkinterWidget(tk.Widget):
    ...
    def ToString(self, IncludeState: bool = None) -> str:
        if IncludeState is None: return f'<{self.__class__.__name__} Tkinter Widget. State: {pp.pformat(self.Details)}>'
        if IncludeState: return f'<{self.__class__.__name__} Tkinter Widget. State: {pp.pformat(self.FullDetails)}>'

        return f'<{self.__class__.__name__} Tkinter Widget>'
    @property
