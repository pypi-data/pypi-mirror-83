...
class RasaSpacyTokenizer(Tokenizer, SparseFeaturizer, EntityExtractor):
    ...
    def __init__(
        ...
        if isinstance(self.predefined_replace_patterns,bool):
            pass
        elif isinstance(self.predefined_replace_patterns,list):
            print(replace_patterns)
            print(self.predefined_replace_patterns)
            replace_patterns = self.predefined_replace_patterns
            self.predefined_replace_patterns = True
        elif isinstance(self.predefined_replace_patterns,str):
            if self.predefined_replace_patterns == 'punct':
                replace_patterns = [(fr'\s?[{re.escape(string.punctuation)}]\s?', ' ')]
