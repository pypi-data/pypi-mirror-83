    ...
    def _install_and_start_code_server(self):
        if self.kaggle == False:
            ...
            if self.password is None:
                os.system(
                    f"code-server --port {self.port} --auth none")
        ...
        if self.kaggle == False:
            try:
                if self.folder_tree == None:
                    url = ngrok.connect(addr=self.port)
                    print(url.data['public_url'])
                else:
                    ngrok.kill()
                    url = ngrok.connect(addr=self.port)
                    print(
                        f"Your newly generated url: {url.data['public_url']}" + f"?folder=/content/drive/My%20Drive/{self.folder_tree}")
            except (HTTPError, PyngrokNgrokHTTPError):
                if self.folder_tree is None:
                    ngrok.kill()
                    url = ngrok.connect(addr=self.port)
                    print(
                        f"Your newly generated url: {url.data['public_url']}")
                else:
                    ngrok.kill()
                    url = ngrok.connect(addr=self.port)
                    print(
