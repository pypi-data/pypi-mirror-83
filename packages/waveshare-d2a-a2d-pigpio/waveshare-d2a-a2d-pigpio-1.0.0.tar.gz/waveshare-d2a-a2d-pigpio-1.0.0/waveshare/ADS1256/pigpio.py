# -*- coding: utf-8 -*-
"""ADS1256_PiGPIO - Python module for interfacing Texas Instruments SPI
bus based analog-to-digital converters with the Raspberry Pi.

-----------------

Comments from Mitch Kahn:

This is based 95% on https://github.com/ul-gh/PiPyADC with the low level wiringpi library
replaced with PiGPIO from http://abyz.me.uk/rpi/pigpio.

Unfortunately, the original code did not support the DAC8552 chip on the same
board and the code I found for that used PiGPIO. PiGPIO seems like a more flexible
option than wiringpi as it supports control over the network (Python code can run on a Mac/PC
and remotely control the GPIO hardware.)
...
PiGPIO port by Mitch Kahn, 4-2020 (Quarantine times...)
"""
...
class ADS1256:
    ...
    This is part of module waveshare_a2d_d2a_python:
    Download: https://github.com/makahn64/waveshare_a2d_d2a_python

    This project is a port of PiPyADC that used the wiringpi library. 90%+ of this
    code is from that project.
    Download Original Here: https://github.com/ul-gh/PiPyADC
    ...
    def __init__(self, conf=DefaultConfig(), pi=None):
        ...
        # MAK: T11 is 1us, CS is 2us with stock clock freq
    ...
    def _chip_release(self):
        #TODO the code below should be written delay->if?
        if self.CS_PIN is not None:
            # TODO replace with sleep?
            #wp.delayMicroseconds(self._CS_TIMEOUT_US)
            usleep(self._CS_TIMEOUT_US)
    ...
    def _send_uint8(self, *vals):
        ...
        self.pi.spi_write(self.spi_id, bytes(vals))
    ...
    def _read_uint8(self, n_vals=1):
        ...
        # Python3 only:
        bytes, data = self.pi.spi_read(self.spi_id, n_vals)
        return tuple(data)
    ...
    def _read_int24(self):
        ...
        # Python3 only:
        _, data = self.pi.spi_read(self.spi_id, 3)
    ...
    def _send_int24(self, val):
        ...
        self.pi.spi_write(self.spi_id, int.to_bytes(val, 3, "big"))
