...
class EventV2Controller(Controller):
    ...
    @ex(help='list events',
        arguments=[
            ...
            (['--order'],
             {
                'help': 'Sets the order for returned events based on created_at timestamp. Supported values ASC, DESC. Default is DESC.',
                'choices': ('ASC', 'DESC')
            }),
        ]
        )
    def list(self):
        events: List[EventV2] = self._client.list(before_dt=self.app.pargs.before, after_dt=self.app.pargs.after,
                                                  order=self.app.pargs.order, event_type=self.app.pargs.type, subtype=self.app.pargs.subtype,
                                                  action=self.app.pargs.action, email_address=self.app.pargs.email,
                                                  device_id=self.app.pargs.device_id, device_serial=self.app.pargs.serial_number,
                                                  container_id=self.app.pargs.container_id, service_name=self.app.pargs.service_name,
