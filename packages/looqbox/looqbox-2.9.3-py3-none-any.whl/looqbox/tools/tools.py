        ...
        try:
            if cols in by:
                ...
                if len(d2[cols]) > len(d1[cols]):
                    ...
                    d3["∆%" + cols] = {rows: (d2[cols][rows] / d1.get(cols, 0)[rows]) - 1
                    if d1.get(cols, 0)[rows] != 0 and rows not in different_keys else "-" for rows in d1[cols].keys()}
                else:
                    ...
                    d3["∆%" + cols] = {rows: (d2[cols][rows] / d1.get(cols, 0)[rows]) - 1 if d1.get(cols, 0)[rows] != 0
                                                                                             and rows not in different_keys else "-"
                                       for rows in d1[cols].keys()}
        except TypeError:
