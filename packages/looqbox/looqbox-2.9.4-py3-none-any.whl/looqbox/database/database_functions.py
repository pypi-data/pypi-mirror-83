...
def _get_query_result(connection, query, close_connection):
    ...
    try:
        ...
        GlobalCalling.log_query({"query": query, "time":str(total_sql_time.seconds) + '.' + str(total_sql_time.microseconds)[:4], "success":True})
        ...
        if close_connection and not isinstance(connection, str):
            ...
            sql_close(conn_curs)
            total_sql_time = datetime.datetime.now() - start_time
            GlobalCalling.log_query({"query": query, "time":str(total_sql_time.seconds) + '.' + str(total_sql_time.microseconds)[:4], "success":False})
