...
def train_DDPG(env_train, model_name, batch_size=128, timesteps=50000):
    """DDPG model"""

    start = time.time()
    #model = DDPG('MlpPolicy', env_train)
    model = TD3('MlpPolicy', env_train, batch_size=batch_size)
...
def run_ensemble_strategy(df, unique_trade_date, rebalance_window, validation_window) -> None:
    ...
    for i in range(rebalance_window + validation_window, len(unique_trade_date), rebalance_window):
        ...
        print("======DDPG Training========")
        model_ddpg = train_DDPG(env_train, model_name="DDPG_10k_dow_{}".format(i), batch_size=178, timesteps=25000)
...
# def run_ensemble_strategy(df, unique_trade_date, rebalance_window, validation_window, timesteps, batch_size) -> None:
#     """Ensemble Strategy that combines PPO, A2C and DDPG"""
#     print("============Start Ensemble Strategy============")
#     # for ensemble model, it's necessary to feed the last state
#     # of the previous model to the current model as the initial state
...
#         print("======DDPG Training========")
#         model_ddpg = train_DDPG(env_train, model_name="DDPG_10k_dow_{}".format(i), batch_size=batch_size, timesteps=timesteps)
#         print("======DDPG Validation from: ", unique_trade_date[i - rebalance_window - validation_window], "to ",
#               unique_trade_date[i - rebalance_window])
#         DRL_validation(model=model_ddpg, test_data=validation, test_env=env_val, test_obs=obs_val)
#         sharpe_ddpg = get_validation_sharpe(i)
