...
def scrape_technologies(url: str) -> Dict[str, Union[str, List[str]]]:
    result: Dict[str, Union[str, List[str]]] = {"technologies": {}}
    html = get_html(url=SITE_LINK.format(url))
    soup = bs(html, "lxml")
    technologies_json = read_technologies_json("pywappalyzer/db/technologies.json")
    ...
    for technology_name in technology_names:
        ...
        if technologies_json[technology_name] in result["technologies"]:
            result["technologies"][technologies_json[technology_name]].append(
                technology_name
            )

        elif technologies_json[technology_name] not in result["technologies"]:
            result["technologies"][technologies_json[technology_name]] = [
                technology_name
            ]
...
def scrape_technologies_to_json(
    filename: str = "pywappalyzer/db/technologies.json",
) -> None:
