...
def delete_function_if_exists(
    client: CogniteClient, function_name: str,
):
...
def deploy_function(
    client: CogniteClient, function_name: str, file_id: int, secrets: Dict[str, str], api_key: str, owner: str = ""
) -> None:
    ...
    while x < 3:
        try:
            ...
            x = 3
        except (TimeoutError, ReadTimeoutError, CogniteAPIError, MaxRetryError, ConnectionError, AttributeError) as e:
            if isinstance(e, CogniteAPIError) and e.code == 400:
                delete_function_if_exists(client, function_name)
            else:
                x += 1
            response = post_to_slack(
                WebClient(os.environ["SLACK_TOKEN"]), channel=CHANNEL, message_text=repr(e), thread=thread
            )
            if response:
...
def list_dangling_function(
    client: CogniteClient, expected_functions: Iterator[str], *, name_prefix: str = ""
) -> Iterator[Function]:
...
def get_function_name(
    path: Union[Path, str], *, version: str = "", ref: str = "", pr: bool = False, latest: bool = False
) -> str:
...
def post_to_slack(slack_client: WebClient, channel: str, message_text: str, thread: Optional[str] = None):
    x = 0
    while x < 3:
        try:
            if thread is None:
                response = slack_client.chat_postMessage(channel=channel, text=message_text)
            else:
                response = slack_client.chat_postMessage(channel=channel, text=message_text, thread_ts=thread)
            assert response.get("message").get("text") == message_text
            return response
