...
def retrieve_undeployed_infra_schedule_assets(client: CogniteClient) -> List[Schedule]:
    schedule_assets = retrieve_schedule_assets(client)
    schedules_external_ids = map(lambda s: s.schedule_asset_external_id, retrieve_air_schedules(client))
    ...
    undeployed_infra = filter(
        lambda u: u.parent_external_id in map(lambda a: a.name, infra_schedule_assets), undeployed_schedule_assets
    )
...
def retrieve_updated_infra_schedules(client: CogniteClient) -> Tuple:
    ...
    updated_filtered = filter(lambda x: x.model in map(lambda i: i.external_id, infra_assets), updated)
    to_be_deleted_filtered = filter(lambda x: x.model in map(lambda i: i.external_id, infra_assets), to_be_deleted)
