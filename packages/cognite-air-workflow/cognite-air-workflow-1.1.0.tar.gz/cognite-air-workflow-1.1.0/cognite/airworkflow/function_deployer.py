...
class FunctionDeployer:
    ...
    def handle_pull_request(self):
        ...
        file_id = cdf.zip_and_upload(
            self.client, self.deployment.code_path / const.FUNCTION_REL_PATH, self.file_name, "AIR"
        )
        functions.deploy_function(
            self.client, self.function_name, file_id, self.secrets, self.client_api_key, owner="AIR"
        )
    ...
    def handle_push(self) -> None:
        ...
        functions.deploy_function(
            self.client, self.function_name_latest, file_id, self.secrets, self.client_api_key, owner="AIR"
        )
        functions.delete_function(self.client, self.function_name)
