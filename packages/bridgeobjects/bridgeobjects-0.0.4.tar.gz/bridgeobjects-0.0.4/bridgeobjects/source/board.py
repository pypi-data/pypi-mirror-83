...
from .constants import SEATS, RANKS, SUITS, VULNERABLE, SUIT_NAMES
...
class Board(object):
    ...
    def board_to_pbn(self):
        ...
        if self.dealer:
            output.append(f'[Deal "{self.dealer}:{self._get_pbn_deal()}"]')
        return output

    def _get_pbn_deal(self, delimiter=' '):
