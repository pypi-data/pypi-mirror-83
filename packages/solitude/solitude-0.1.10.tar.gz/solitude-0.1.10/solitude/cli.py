...
def extend(cmd_files: List[str], jobids: List[int], workers: int):
    if are_jobids_specified(jobids=jobids) and is_config_valid():
...
def stop(cmd_files: List[str], jobids: List[int], workers: int):
    if are_jobids_specified(jobids=jobids) and is_config_valid():
        solitude.core.stop(
            cmd_files=cmd_files,
            jobids=jobids,
...
def link(
    cmd_files: List[str], jobids: List[int], workers: int, slurm_jobid: int
):
    if are_jobids_specified(jobids=jobids) and is_config_valid():
...
def unlink(cmd_files: List[str], jobids: List[int], workers: int):
    if are_jobids_specified(jobids=jobids) and is_config_valid():
        for jobid in jobids:
...
def list_jobs(
    cmd_files: List[str], jobids: List[int], workers: int, selected_only: bool
):
    if is_config_valid():
