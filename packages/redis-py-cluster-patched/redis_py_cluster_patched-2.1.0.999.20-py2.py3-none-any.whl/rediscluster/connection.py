...
class ClusterConnectionPool(ConnectionPool):
    ...
    def __init__(self, startup_nodes=None, init_slot_cache=True, connection_class=None,
        ...
        self.get_connection_failures = 0
        self.max_get_connection_failures = 10
    ...
    def make_connection(self, node):
        ...
        if num_connections >= self.max_connections:
            self.get_connection_failures += 1
            if self.get_connection_failures >= self.max_get_connection_failures:
                log.info("reset connection pool")
                self.disconnect()
                self.reset()
    ...
    def get_connection_by_node(self, node):
        ...
        try:
            ...
            try:
                ...
                log.exception("cannot get new connection")
                raise ConnectionError('Cannot get a new connection')
