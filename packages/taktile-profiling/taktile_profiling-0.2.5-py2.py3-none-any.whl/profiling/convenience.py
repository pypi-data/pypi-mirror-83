...
def mkdir(endpoint, spec_path=""):
    folder = os.path.join(BASE_RESULTS_PATH, endpoint.profile_func.__name__, spec_path)
...
def create_shapley_schema(endpoint):
    ...
    x, y = endpoint.get_input_and_output_for_profiling()
    explainer = ShapExplainer(func=endpoint.profile_func, X=x)
    folder = mkdir(endpoint=endpoint)
    input_descriptions = [create_description(x[col]) for col in x.columns]
...
def create_explainer_schema(endpoint, explainer, input_descriptions):
    ...
    x, y = endpoint.get_input_and_output_for_profiling()
    x = x.sample(n=1)
    pred = endpoint.profile_func(x)
    baseline, explanation = explainer.explain(x)
    example = {
        "inputs": endpoint.input_schema.pydantic.schema()["example"],
...
def create_summary(endpoint):
    endpoint_name = endpoint.profile_func.__name__
    logging.info(f"Summary - creating endpoint summary for endpoint: {endpoint_name}")
    as_json = {
        "name": endpoint_name,
        "kind": endpoint.KIND.value,
        "inputs": endpoint.input_names,
