...
import pandas as pd
import pytest
from profiling.shap import ShapExplainer, comb, n_choose_k
...
def test_large_df():
    ...
    X = pd.DataFrame(np.zeros((1000000, n_col)))
    intercept = 5.23
...
def test_various_coltypes():
    ...
    with pytest.warns(UserWarning):  # undefined modes for some columns
        shap_explainer = ShapExplainer(func=regression, X=X)
    ref, shap = shap_explainer.explain(X)
