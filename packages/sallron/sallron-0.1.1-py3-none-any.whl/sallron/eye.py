from sallron.util import settings, logger
...
class TheEyeofSauron:
    def __init__(self, customer, info, interface_class, interface_name):
        ...
        # Logging
        self.sauron_logger = logging.getLogger('sauron') if not(settings.SAVE_LOGS) else logger.setup_logbook('sauron')
...
def ring_ruler(interface, interface_name):
