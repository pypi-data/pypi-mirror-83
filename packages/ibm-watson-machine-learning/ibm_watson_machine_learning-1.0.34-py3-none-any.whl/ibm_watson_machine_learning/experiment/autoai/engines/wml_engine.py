...
class WMLEngine(BaseEngine):
    ...
    def fit(self,
        ...
        if background_mode:
            ...
            gen = ProgressGenerator(number_of_estimators)
            end = False

            progress_bar = ProgressBar(desc="Total", total=gen.get_total(), position=0, ncols=100, bar_format=None)
            ...
            while not end:
                ...
                for event in websocket.connect():
                    ...
                    if event.name == 'text':
                        ...
                        if 'message' in status and 'text' in status['message']:
                            text = status['message']['text']

                            try:
                                phase = status['metrics'][0]['context']['phase']
                            except:
                                phase = ""

                            try:
                                step_name = status['metrics'][0]['context']['step']['name']
                            except:
                                step_name = ""

                            try:
                                step_estimator = status['metrics'][0]['context']['step']['estimator']
                            except:
                                step_estimator = ""

                            log = f"{phase}:{step_name}:{step_estimator}:{text}"

                            if log not in texts:
                                texts[log] = 1
                            else:
                                texts[log] = texts[log] + 1
                            ...
                            progress_bar.increment_counter(gen.get_progress(log))
                            progress_bar.update()
