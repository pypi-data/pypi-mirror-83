...
async def get_async_rs232_protocol(serial_port_path, serial_config, protocol_config, loop):
    ...
    class RS232ControlProtocol(asyncio.Protocol):
        def __init__(self, serial_port_path, protocol_config, loop):
            ...
            LOG.info(f"RS232ControlProtocol setup for {serial_port_path}")
        ...
        def data_received(self, data):
            ...
            # LOG.debug(f"Received from {self._serial_port_path}: {data}")
        ...
        async def _apply_request_throttling(self):
            ...
            if delta_since_last_send < 0:
                ...
                LOG.debug(f"Throttling {delay} seconds before sending another RS232 request")
                await asyncio.sleep(delay)

            elif delta_since_last_send < min_time_between_commands:
                delay = max(0, min_time_between_commands - delta_since_last_send)
                delay = min(delay, min_time_between_commands)
                LOG.debug(f"Throttling {delay} seconds before sending another RS232 request")
                await asyncio.sleep(delay)
        ...
        async def send(self, request: bytes, skip=0):
            ...
            # send the request
            LOG.debug(f"Sending {self._serial_port_path} data: %s", request)
