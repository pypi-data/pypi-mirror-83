...
class AddressResolutionStatementBuilder(ReceiptBuilder):
    ...
    def __init__(self, size: int, version: int, type: ReceiptTypeDto, unresolved: UnresolvedAddressDto, resolutionEntries: List[AddressResolutionEntryBuilder]):
        """Constructor.
        Args:
            size: Entity size.
    ...
    def loadFromBinary(cls, payload: bytes) -> AddressResolutionStatementBuilder:
        ...
        while resolutionEntriesByteSize > 0:
            item = AddressResolutionEntryBuilder.loadFromBinary(bytes_)
            resolutionEntries.append(item)
            itemSize = item.getSize()
            resolutionEntriesByteSize -= itemSize
            bytes_ = bytes_[itemSize:]
        return AddressResolutionStatementBuilder(superObject.size, superObject.version, superObject.type, unresolved, resolutionEntries)
