...
class DoRequest(baseClass):
    ...
    def add_session_headers(self, dict_headers:dict):
    ...
    def run(self, method: str, url: str, data=None, is_auto=True, **kwargs):
        ...
        支持部分配置
            ...
            ：val  session_headers 格式dict,{xxx:yyy,sss:www} 设置通用请求头
        ...
        try:
            ...
            self.req['headers'] = dict(self.s.headers)  # 初始化请求头
    ...
    def _remove_httponly(self, header_dcit):
        for k, v in header_dcit.items():
            if k.lower().startswith('cookie') and ('HttpOnly' in v or 'path=/' in v):
                v = str(v).replace('HttpOnly,', '').replace(
                    'HttpOnly', '').replace('path=/,', '').replace('path=/', '')
                header_dcit[k] = v
    ...
    def commit(self):
        if self.status:
            try:
                ...
                for his_n in range(len(self.res['history'])):
                    his = self.res['history'][his_n]
                    self.res['headers_' +
                             str(his_n)] = self._remove_httponly(dict(his.headers))

                self.res['reason'] = str(self.res_value.reason)
                self.res['headers'] = self._remove_httponly(
                    dict(self.res_value.headers))
    ...
    def _check_headers(self, kwargs):
        ...
        is_multipart = False
        if kwargs.get('session_headers') is not None and len(kwargs.get('session_headers')) > 0:
            d = kwargs.get('session_headers')
            if isinstance(d, dict):
                self.s.headers.update(d)
        ...
        if 'Content-Type' not in self.req['headers']:
            ...
            if self.req['method'] == 'POST' or self.req['method'] == 'PUT':
                ...
                elif self.req['data'] is not None and (self.req['data'].startswith(b'{') or self.req['data'].startswith(b'[')):
        ...
        if is_multipart:
            dd = None
            if self.req['data'] is not None and (self.req['data'].startswith(b'{') or self.req['data'].startswith(b'[')):
                try:
                    dd = json.dumps(
                        self.req['data'].decode, ensure_ascii=False)
                except:
                    pass
            if len(self.req['files']) > 0:
                files = {}
                ...
                if dd is not None:
                    ...
                    dd = files
            encode_data = encode_multipart_formdata(dd)
