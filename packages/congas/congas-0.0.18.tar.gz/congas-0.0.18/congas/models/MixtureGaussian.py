...
class MixtureGaussian(Model):
    ...
    def model(self,*args, **kwargs):
        ...
        cc_norm = torch.sum(cc * self._data['mu'], dim=1)
        ...
        for i in range(len(self._data['pld'])):
            if self._params['kmeans']:
                if self._params['norm_init_factors'] is None:
                    ...
                    norm = norm * torch.sum(self._data['pld'])
            ...
            if site["name"] == "norm_factor":
                if self._params['norm_init_factors'] is None:
                    return torch.mean(self._data['data'] /
                                      (self._data['pld'].reshape(self._data['data'].shape[0],1) * self._data['mu'].reshape(self._data['data'].shape[0],1)), axis=0)* torch.sum(self._data['pld'])
                else:
