...
def metadata(mapId, Type, token = None):
    if Type == 'mapInfo':
        ...
        if Type == 'classes':
            ...
            raise ValueError('Type must be either classes, measurements, geometryLayers, bands, forms, mapLayers or timestamps')
...
def newMap(name,  token, shapeName = None, bands = [{'name':'red', 'number':1}, {'name':'green', 'number':2}, {'name':'blue', 'number':3}], visualizations= [{'name':'rgb', 'a':0,'b':0, 'c':1}], aggregationPixels = 256, storeRaw = True, model = None, fitToRaster = True, boundary = None, resolutionFromRaster = True, useMask=False, dataType = 'float32' , dryRun = True):
    ...
    boundary = gpd.GeoSeries([boundary]).__geo_interface__['features'][0]
    measurements = []
    body = {'name':name, 'dataSource':bands, 'useMask':useMask , 'captures': [], 'storeRaw':storeRaw, 'aggregationZoom':aggregationZoom, 'visualizations':visualizations, 'measurements':measurements, 'active':False, 'dataType':dataType, 'fitToRaster':fitToRaster, 'resolutionFromRaster':resolutionFromRaster, 'dryRun':dryRun }
...
def newOrder(name, boundary, captures, token, shapeName,  visualizations = ['rgb'], measurements = ['ndvi'],  dataSource = 'sentinel2RGBIR', aggregationPixels = 256, storeRaw = True, model = None,  useMask=True, dryRun = True ):
