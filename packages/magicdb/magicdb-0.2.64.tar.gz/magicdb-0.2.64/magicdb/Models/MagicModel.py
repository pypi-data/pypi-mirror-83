...
from magicdb.utils.async_helpers import threadpool_asyncio, threadpool
...
class MagicModel(BaseModel, metaclass=QueryAndBaseMetaClasses):
    ...
    def update(self, batch=None, transaction=None, create=False, ignore_fields=False):
        batch_or_transaction = batch or transaction
        ...
        try:
            with safe_span(f"update-{self.key}", use=(batch_or_transaction is None)):
    ...
    def save_history(self, sync: bool = True, user_id: str = None):
        ...
        return history_obj.save() if sync else history_obj.save_async()

    def save_deleted(self, sync: bool = True, user_id: str = None):
        ...
        return deleted_obj.save() if sync else deleted_obj.save_async()
