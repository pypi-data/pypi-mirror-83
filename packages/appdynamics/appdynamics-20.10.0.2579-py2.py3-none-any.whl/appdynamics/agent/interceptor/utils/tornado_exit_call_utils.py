...
def add_callback(future, end_exit_call_func, exit_call):
    ...
    def wrap_function(func):
        if int(tornado.version.split('.')[0]) >= 6:
            ...
            return tornado.stack_context.wrap(func)

    try:
        from asyncio import Future
        if isinstance(future, Future):
            future.add_done_callback(functools.partial(wrap_function(end_exit_call_func), exit_call))
        else:
            future._callbacks.insert(0, functools.partial(wrap_function(end_exit_call_func), exit_call))
    except ImportError:
        future._callbacks.insert(0, functools.partial(wrap_function(end_exit_call_func), exit_call))
