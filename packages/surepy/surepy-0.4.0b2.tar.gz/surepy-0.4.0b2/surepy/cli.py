...
#     # check env token
#     elif (env_token := environ.get(TOKEN_ENV, None)) and seems_valid(token=env_token):
#         return env_token
...
def cli(ctx: click.Context, user_token: str, version: bool) -> None:
    """surepy cli 🐾
    ...
    # ctx.obj["debug"] = debug
    ctx.obj["token"] = None  # find_token(user_token)
