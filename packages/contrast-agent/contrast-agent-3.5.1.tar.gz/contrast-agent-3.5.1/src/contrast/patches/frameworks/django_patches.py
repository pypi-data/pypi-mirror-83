...
def patch_django(django_module):
    patch_cls_or_instance(
        django_module.utils.safestring, MARK_SAFE_NAME, mark_safe_patch
    )


def register_patches():
    register_post_import_hook(patch_django, MODULE_NAME)


def reverse_patches():
    django_module = sys.modules.get(MODULE_NAME)
    if not django_module:
        return

    patch_manager.reverse_patches_by_owner(django_module.utils.safestring)
