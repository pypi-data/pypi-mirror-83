...
DEFAULT_PROGNAME = "Contrast Security"
...
def setup_agent_logger(config):
    ...
    print(
        "Contrast Agent Logger Initialized: {} : {}".format(
            getLevelName(logger.level), path
        )
...
def reset_agent_logger(log_path, log_level):
    ...
    if not hasattr(logger, "lock_path") or not hasattr(logger, "lock_level"):
        logger.debug("Will not reset agent logger without set lock_attr values.")
        return
...
def _set_handler(logger, path, progname):
    ...
    """
    formatter = Formatter(
        "%(asctime)s [%(progname)s] (%(filename)s:%(funcName)s:%(lineno)d)"
