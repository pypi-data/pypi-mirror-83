...
# Create the SDLogger Class with appropriate functions
...
class SDLogger:
    ...
    Attributes:
        _logger (Logger): Access the Logger object from the logging.Logger class
    """

    def __init__(self, log_level=None, log_file=True, file_log_level=None, log_file_prefix=None):
        """ 
        Constructor

        Args:
            log_level (String, optional): One of ['DEBUG', 'INFO', 'WARNING', 'ERROR']
            log_file (bool, optional): Flag to output a log file (Defaults to True)
            file_log_level (String, optional): One of ['DEBUG', 'INFO', 'WARNING', 'ERROR']
            log_file_prefix(String, optional): Set prefix of log file name.

            NOTE: When an SDLogger is instantiated, the name is derived from the module,
            so any subsequent SDLogger instantiation will return a reference to the 
            same Logger object.  This could be addressed by adding a logger_name 
            parameter, but it may also introduce other issues.

            NOTE: The underlying logging.Logger object root level is set to the most
            verbose input level between log_level and file_log_level, 
            so that these two levels are allowed to differ.
        """
        date_str = time.strftime('%Y%m%d')
        log_filename = f'{date_str}.log' if log_file_prefix is None else f'{log_file_prefix}_{date_str}.log'
        log_vals = {'DEBUG': logging.DEBUG, 'INFO': logging.INFO,
                    'WARNING': logging.WARNING, 'ERROR': logging.ERROR}
        log_level = logging.INFO if log_level is None else log_vals[log_level.upper()]
        file_log_level = log_level if file_log_level is None else log_vals[file_log_level.upper()]

        low_log_level = log_level if (log_level <= file_log_level) else file_log_level
    ...
    def info(self, msg, *args, **kwargs):
        ...
        Args:
            ...
            *args: The arguments merged into msg with the string formatting operator.
    ...
    def set_log_level(self, log_level=None):
        ...
        low_log_level = log_level if (log_level <= curr_log_level) else curr_log_level
    ...
    def set_file_log_level(self, file_log_level=None):
        ...
        curr_log_level = self._logger.getEffectiveLevel()
        low_log_level = file_log_level if (file_log_level <= curr_log_level) else curr_log_level
