...
def cntvDownload(url):
    ...
    definations = [(2000,'超清'),(1200,'高清'),(850,'标清'),(450,'流畅')]
    os.mkdir(os.path.join(export_folder,video_code))
    ...
    while True:
        ...
        if status == 200:
            save_video_name = str(segment) + suffix
            video = open(os.path.join(export_folder, video_code, save_video_name), 'wb')
            for block in res.iter_content(chunk_size=1024):
                video.write(block)
            video.close()
            print(video_code, '的第', segment + 1, '个视频片段已下载完成')
            segment = segment + 1
        else:
            print(str(segment),'个视频片段已全部下载完成')
            break
