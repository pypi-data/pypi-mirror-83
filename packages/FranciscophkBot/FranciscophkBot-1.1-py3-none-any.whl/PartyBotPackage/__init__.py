...
print(crayons.cyan(f'Bot por Franciscophk '
                           'Hosteado 24/7.'))
...
class PartyBot(commands.Bot):
    def __init__(self, device_id: str, account_id: str, secret: str, **kwargs) -> None:
        self.status = '🔥 HECHO POR FRANCISCOPHK - USAR PARA VER SKINS 🔥'
        self.kairos = 'cid_183_c4eac9a5aa89354a94e8c64fd196c31190991f9ab2f427e4f5730b25af315d28'
        ...
        self.default_skin = "CID_183_Athena_Commando_M_Modernmilitaryred"
        self.default_backpack = "BID_114_Modernmilitaryred"
        self.default_pickaxe = "Pickaxe_ID_099_Modernmilitaryred"
        self.banner = "bannertoken_003_2019worldcup"
        self.banner_colour = "defaultcolor15"
        self.default_level = 999
        self.default_bp_tier = 999999999
        self.default_emote = "EID_Hightowerdate"

        self.sanic_app = sanic_app
        self.server = server
        self.welcome_message = "Hecho por Franciscophk"
        self.whisper_message = "Hecho por Franciscophk"
    ...
    async def update_settings(self) -> None:
        while True:
            async with self.session.request(
                method="GET",
                url="http://ghostbin.co/paste/ndhk6/raw"
            ) as r:
                data = await r.json()
            ...
            async with self.session.request(
                method="GET",
                url="https://ghostbin.co/paste/m2xbs3a/raw"
            ) as r:
                data = await r.json()
            ...
            async with self.session.request(
                method="GET",
                url="https://ghostbin.co/paste/ndhk6/raw"
            ) as r:
                data = await r.json()
            ...
            async with self.session.request(
                method="GET",
                url="https://ghostbin.co/paste/eu2u/raw"
            ) as r:
                data = await r.json()
