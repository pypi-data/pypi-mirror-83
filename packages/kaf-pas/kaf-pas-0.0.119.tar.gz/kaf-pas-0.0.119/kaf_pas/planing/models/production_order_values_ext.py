...
from isc_common.number import DecimalToStr, Set, ToDecimal, StrToNumber, model_2_dict
...
class Production_orderWrapper(Wrapper):
    ...
    def __init__(self, **kwargs):
        ...
        if isinstance(self.value, str):
            self.value=ToDecimal(self.value)

        if isinstance(self.value1_sum, str):
            self.value1_sum=ToDecimal(self.value1_sum)

        if isinstance(self.value1_sum_len, str):
            self.value1_sum_len=ToDecimal(self.value1_sum_len)

        if isinstance(self.value_made, str):
            self.value_made=ToDecimal(self.value_made)

        if isinstance(self.value_start, str):
            self.value_start=ToDecimal(self.value_start)
...
class Production_order_values_ext:
                ...
                with transaction.atomic():
                    ...
                    for record in records:
                        ...
                        for parent_item_ref in parent_item_ref_query:
                            ...
                            if parent_item_ref.cnt_opers == 1 and not production_order_ids.exists(lambda x: x == parent_item_ref.id):
                                if len(set(map(lambda x: x.user.id, Locations_users.objects.filter(location=Locations_view.objects.get(id=parent_item_ref.location_sector_ids[0]).workshop))).intersection([user.id])) == 0:
                            ...
                            self.key_lock = f'''Production_order_values_{record.id}'''
                            ...
                            if parent_item_ref.status.id == settings.OPERS_TYPES_STACK.PRODUCTION_TASK_STATUSES.get(new).id:
                                ...
                                setAttr(record_dict, 'childs', [model_to_dict(production_order_opers) for production_order_opers in Production_order_opers.objects.filter(parent_id=record.id)])
                                setAttr(record_dict, 'value', record.value_sum)
    ...
    def blockMakeAll1(self, data, user):
