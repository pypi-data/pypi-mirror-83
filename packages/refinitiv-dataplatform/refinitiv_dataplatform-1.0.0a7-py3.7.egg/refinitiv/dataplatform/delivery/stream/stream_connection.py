...
class StreamConnection(threading.Thread, LogReporter):
    ...
    def run(self):
        ...
        while not self.is_closing and self._do_reconnect:
            ...
            self.debug(f"Streaming session {self.streaming_session_id} waits for start event")
            self.debug(f"Streaming session {self.streaming_session_id} received start event, "
