...
class Ptrms:
    ...
    def extract_windows(self):
        ...
        ref_window_stats = ref_window.drop(columns=['Cycle'], errors='ignore').describe()
        sample_window = self.frame.loc[int(self.sample_start)-1: int(self.sample_end)]
        sample_window_stats = sample_window.drop(columns=['Cycle'], errors='ignore').describe()
        ref_window_frame = pd.DataFrame(ref_window.max(axis=0)).transpose().drop(columns=['Cycle'], errors='ignore')
        sample_window_frame = pd.DataFrame(sample_window.max(axis=0)).transpose().drop(columns=['Cycle'], errors='ignore')
        ...
        out = pd.concat([dif_frame, dif_stats, ref_window_frame, sample_window_frame], axis=1)
        out_path = os.path.join(os.path.dirname(self.file.name), 'extracted_window.xlsx')
