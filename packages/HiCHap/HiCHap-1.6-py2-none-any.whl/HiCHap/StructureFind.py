...
class StructureFind(object):
    ...
    """
    def __init__(self, cooler_fil, Res, Allelic, genomeSize_fil, GapFile = None, 
                 Loop_ratio = 0.6, Loop_strength = 16, n_components = 3):
        
        #------------Initialization the HiC Matrix Dictionary----------------
        self.cooler_fil = '{}::{}'.format(cooler_fil, Res)
        self.Res = Res
        self.Allelic = Allelic
        self.genomeSize_fil = genomeSize_fil
...
#===============================Public Functions====================================
    ...
    def readChromSize(self):
        """
            Get the chromatin size.
        """
        
        chromsize = {}
        with open(self.genomeSize_fil,'r') as source:
            for line in source:
...
#============================Compartment Analysis modules===========================
    ...
    def Plot_Compartment(self, out, MS = 'IF'):
        ...
        for chro in self.chroms:
            ...
            pos = [t * self.Res for t in ticks[:4]]
            if self.Allelic == False:
                labels = [self.properU(p) for p in pos] + [self.properU(self.chromsizes[chro])]
            elif (self.Allelic == 'Maternal') or (self.Allelic == 'Paternal'):
                labels = [self.properU(p) for p in pos] + [self.properU(self.chromsizes[chro[1:]])]
            else:
                raise Exception ('Unkonwn key word %s, Only Maternal, Paternal, False allowed' % self.Allelic)
