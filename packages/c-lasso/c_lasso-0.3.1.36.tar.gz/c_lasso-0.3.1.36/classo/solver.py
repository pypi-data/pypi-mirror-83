...
class CVparameters:
    ...
    Attributes:
        ...
        lambdas (numpy.ndarray) : list of lambdas for computing lasso-path for cross validation on lambda.
    ...
    def __repr__(self): 
        if not self.lambdas is None : 
            ...
            typ = ' the list has been entered manually'
...
class solution_CV:
    ...
    def __init__(self, matrices, param, formulation, numerical_method, label):
        ...
        if param.lambdas is None : 
            if param.logscale : 
                param.lambdas = np.array([param.lamin**(i/(Nlam-1)) for i in range(Nlam)])
            else : 
