...
class solution_CV:
    ...
    def graphic(self, ratio_mse_max=10.,save=None, logScale= False):
        ''' Method to plot the graphic showing mean squared error over along lambda path once cross validation is computed. 

        Args:
            ratio_mse_max (float): float thanks to which the graphic will not show the lambdas from which MSE(lambda)> ratio * min(MSE) 
                Default value : 10
            logScale (bool) : input that tells to plot the mean square error as a function of lambda, or log ( lambda )
                Default value : False
        ...
        j = 0
        while(j < i_1SE and self.yGraph[j] > mse_max) : j+=1
