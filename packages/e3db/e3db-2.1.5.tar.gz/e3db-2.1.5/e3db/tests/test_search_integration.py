...
class TestSearchIntegration():
    ...
    def register_client(self):
        # config = e3db.Config.load("prod")
        # config["api_url"] = "https://tozauthtest.ngrok.io"

        # self.client1 = e3db.Client(config)
    ...
    def test_v2_pagination(self):
        ...
        results = self.client2.search(q)
        prevDate = datetime(0001, 0, 0)
        for r in results:
            assert(r.meta.created < firstDate)
            prevDate = r.meta.created
    ...
    def test_v2_special_char_search(self):
        q = e3db.types.Search(include_data=True, include_all_writers=True, count=1).match(record_types=["__SecureComputeConfig"])
        results = self.client1.search(q)
        assert(len(results) == 0)
