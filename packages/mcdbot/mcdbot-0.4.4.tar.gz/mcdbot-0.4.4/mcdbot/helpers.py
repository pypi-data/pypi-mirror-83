...
async def check_mc_player_available(main: Mcdbot, user: Union[Member, User], player: str):
    ...
    if await main.redis.has_user_player(user):
...
def check_mc_player_name_correctness(player: str):
    ...
    if len(player) < 3 or len(player) > 16 or not (str_is_ascii(player) and player.isalnum() and ' ' not in player):
        raise BadMcPlayerError


def check_mc_password_validity_under_policy(password: str):
    content_policy = (str_is_ascii(password) and password.isprintable() and ' ' not in password)
