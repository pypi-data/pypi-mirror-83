...
class Mcdbot(discord.Client):
    ...
    def turn_debug_on(self):
        ...
        logger.warning("Switching to debug mode! Look in the new log file.")
        logger.remove(self.logger_id)
        logger.add(global_config.mcdbot_log,
                   level="DEBUG",
                   backtrace=True,
                   diagnose=True,
                   catch=False,
                   rotation="24h",
                   retention="2 weeks")
        logger.warning("Activated debug mode!")
    ...
    async def on_error(self, event_method, *args, **kwargs):
        ...
        try:
            admin_role_mention = discord.utils.get(self.get_main_guild().roles,
            ...
            if not message.author.bot:
                if message.content.startswith("!mc "):
                    ...
                    try:
                        ...
                        await message.channel.send(
                            f"{message.author.mention} [ERROR] {e.NAME}{e.append}"
                        )
                        return
        except Exception as e:
