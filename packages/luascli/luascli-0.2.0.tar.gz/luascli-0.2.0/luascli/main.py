...
def map(ctx, stop):
    ...
    if s is not None:
        click.launch('https://www.openstreetmap.org/?mlat=' + s['lat'] + '&mlon=' + s['long'] + 'zoom=14#map=14')
    else:
        click.echo("Couldn't find the location for the stop " + stop + ' on the line ' + ctx.obj['line'])
