...
class Base(metaclass=abc.ABCMeta):
    ...
    def archive(self, source, output, **kwargs):
        ...
        if self.archive_mode == 'IN_PLACE':
            self._create_archive(source, output, **kwargs)
        elif self.archive_mode == 'IN_TMP_DIR':
            tmpout = os.path.join(
                self.tmp_dir,
                uuid.uuid4().hex,
            )
            self._create_archive(source, tmpout, **kwargs)
            logger.debug('Move archive "%s" -> "%s".', tmpout, output)
            shutil.move(tmpout, output)
        else:
            raise AttributeError('Unknown archive mode: %s' % self.archive_mode)
