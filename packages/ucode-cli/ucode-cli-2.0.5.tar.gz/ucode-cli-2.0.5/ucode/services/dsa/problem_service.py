...
- $1 ≤ M ≤ 46340$"""
    ...
    def load(problem_folder, load_testcase=False):
        ...
        if load_testcase:
            if problem.testcase_format == "ucode":
                ProblemService.load_ucode_testcases(problem_folder, problem)
            elif problem.testcase_format == "themis":
                ProblemService.load_themis_testcases(problem_folder, problem)
            else:
                CLog.error(f"Unknown testcase format {problem.testcase_format}")
    ...
    def save(problem : Problem, base_folder=".", problem_folder=None, problem_code=None, overwrite=False, lang=None):
        ...
        with open(os.path.join(template_path, 'testcases.txt.j2'), encoding="utf-8") as file_:
            ...
            f = open(problem_folder + f"/{problem_code}_testcases.txt", 'w', encoding="utf-8")
            f.write(content)
            f.close()
        ...
        with open(os.path.join(template_path, 'testcases.txt.j2'), encoding="utf-8") as file_:
            ...
            f = open(problem_folder + f"/{problem_code}_testcases_sample.txt", 'w', encoding="utf-8")
            f.write(content)
            f.close()
        ...
        with open(os.path.join(template_path, 'meta.json.j2'), encoding="utf-8") as file_:
            template = Template(file_.read())
            ...
            f = codecs.open(problem_folder + ("/%s_meta.json" % problem_code), "w", encoding="utf-8")
            f.write(meta)
            f.close()
        ...
        if not problem.solution:
            with open(os.path.join(template_path, 'solution.py.j2'), encoding="utf-8") as file_:
                ...
                f = open(problem_folder + (f"/{problem_code}.py"), 'w', encoding="utf-8")
                f.write(content)
                f.close()
        else:
            f = open(problem_folder + (f"/{problem_code}.py"), 'w', encoding="utf-8")
        ...
        if not problem.testcase_generator:
            with open(os.path.join(template_path, 'generator.py.j2'), encoding="utf-8") as file_:
                template = Template(file_.read())
                content = template.render(problem_code=problem_code)
                f = open(problem_folder + (f"/{problem_code}_generator.py"), 'w', encoding="utf-8")
                f.write(content)
                f.close()
        else:
        ...
        if problem.solutions:
            for solution in problem.solutions:
                f = open(f"{problem_folder}/{problem_code}_solution.{solution['lang']}", 'w', encoding="utf-8")
...
if __name__ == "__main__":
    ...
    #                       translations=['vi'], load_testcase=True)
    problem = ProblemService.load('../../../problems/bridge_passing', load_testcase=True)
    ...
    ProblemService.save(problem, "../../../problems", problem_folder="bridge_passing2", overwrite=True)
