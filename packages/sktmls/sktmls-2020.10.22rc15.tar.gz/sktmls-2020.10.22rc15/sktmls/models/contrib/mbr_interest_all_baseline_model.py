...
class MbrInterestAllBaselineModel(MLSRuleModel):
    ...
    def __init__(
        ...
        assert isinstance(categories, list) and len(categories) + 2 == len(
            features
        ), "전체 카테고리 리스트 길이가 `user_id`를 제외한 피쳐의 수와 다릅니다."
                ...
                candidates.append(
                    {
                        ...
                        "props": {
                            ...
                            "context_id": "context_preference"
                            if category in prefer_mbr_category
                            else "context_default",
                            "context_nm": "고객선호" if category in prefer_mbr_category else "#",
                        },
                    }
                )
