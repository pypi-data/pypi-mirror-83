...
class SceneDetector:
    ...
    def predict(self, image):
        ...
        try:
            ...
            if img.mode == 'RGBA':
                img = img.convert('RGB')
            img = Variable(self.tf(img).unsqueeze(0))
            # forward pass
        ...
        logs.info('TYPE OF ENVIRONMENT: %s' % predict_res['environment'])

        # output the prediction of scene category
        logs.info('SCENE CATEGORIES:')
        scene_categories = []
        for i in range(0, 5):
            category = {
