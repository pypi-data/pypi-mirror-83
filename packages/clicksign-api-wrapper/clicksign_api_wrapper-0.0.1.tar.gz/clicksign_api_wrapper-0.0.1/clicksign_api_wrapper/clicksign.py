...
class ClickSign:
    ...
    def __init__(self, token: str, api_env=ApiEnv.SANDBOX, timeout: float = 10):
        """Class constructor
    ...
    def check_token(self) -> Dict:
        ...
        """
        resp = make_response("GET", url=self.__url(
            'accounts'), params=self.query_string, timeout=self.timeout)
        return resp.json()

    def create_new_batch(self, docs_list: List[str], signer_key: str, summary: bool = True) -> Batch:
        ...
        body = {'batch': {'signer_key': signer_key,
                          'document_keys': docs_list, 'summary': summary}}
    ...
    def list_documents(self) -> List[Document]:
        ...
        """
        resp = make_response(method="GET", url=self.__url(
            f'documents'), params=self.query_string, timeout=self.timeout)
        list_of_metadata = resp.json()['documents']
        return list(map(lambda metadata: Document(self, {'document': metadata}), list_of_metadata))
    ...
    def __generate_body_for_sign_via_api(self, request_signature_key: str, secret: str) -> Dict:
        ...
        body = {"request_signature_key": request_signature_key,
    ...
    def sign_via_api(self, request_signature_key: str, secret: str) -> bool:
        ...
        body = self.__generate_body_for_sign_via_api(
            request_signature_key, secret)
    ...
    def create_new_doc_from_template(self, template_key: str, doc_path: str, data: Dict, auto_close=True) -> Document:
        ...
        if auto_close:
            ...
            body = {'document': {'path': doc_path, 'template': {
                'data': data}, 'auto_close': auto_close}}

        resp = make_response(method="POST",
                             url=self.__url(
                                 f'templates/{template_key}/documents'),
                             json=body,
                             params=self.query_string,
                             timeout=self.timeout)
        metadata = resp.json()
    ...
    def finalize_doc(self, document_key: str) -> Document:
        ...
        resp = make_response(method="PATCH",
                             url=self.__url(
                                 f'documents/{document_key}/finish'),
                             params=self.query_string,
                             timeout=self.timeout)
        metadata = resp.json()
        return Document(self, metadata)
    ...
    def cancel_doc(self, document_key: str) -> Document:
        ...
        resp = make_response(method="PATCH",
                             url=self.__url(
                                 f'documents/{document_key}/cancel'),
                             params=self.query_string,
                             timeout=self.timeout)
        metadata = resp.json()
        return Document(self, metadata)
    ...
    def add_signer_to_document(self, document_key: str, signer_key: str, sign_as: SignatureAsTypes) -> ListClass:
        ...
        """
        body = {"list": {"document_key": document_key,
                         "signer_key": signer_key, "sign_as": sign_as}}
        resp = make_response(method="POST",
