...
from mjooln import Dic, Doc, File, Key, FileName, Folder, Atom, FileError, \
    Element, Path, FileSystem, Identity
    ...
    def __init__(self, folder: Folder):
        ...
        if not self._folder.exists():
            raise NotRootException(f'Folder does not exist: '
                                   f'{self._folder}; '
                                   f'Use {self._name()}.plant()')
        if not Key.is_stub(self._folder.name()):
