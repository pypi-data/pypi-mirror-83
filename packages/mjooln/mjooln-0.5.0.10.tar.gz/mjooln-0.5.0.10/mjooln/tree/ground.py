from mjooln import Name, Path, Folder, Key, Element, Atom, FileName, File, \
    FileSystem
...
class Ground(Folder):
    ...
    @classmethod
    def find_ground(cls, folder: Folder, max_depth=1):
        grounds = []
        if cls.is_in(folder):
    ...
    @classmethod
    def settle_home(cls):
        return cls.settle(Folder.home(), key=Key('home'))

    @classmethod
    def home(cls):
        home = Folder.home()
        if not Ground.is_in(home):
            return cls.settle(home, key=Key('home'))
    ...
    def __init__(self, folder):
        ...
        self._ground_folder = self._get_ground_folder(folder, key)
        self.cave, cave_file = self._cave_and_file(self._ground_folder, key)
        if not self.cave.exists():
            raise GroundProblem(f'Missing cave folder: {self.cave}')
    ...
    def walk(self,
        ...
        for x in FileSystem.walk(Folder(self),
