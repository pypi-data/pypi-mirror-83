...
from telecom_coworker.kazoo_helpers import send_message
from telecom_coworker.models import WorkerInfo, Task, TaskState, Message, STATE_RUNNING, STATE_EXITED, STATE_FATAL, \
    STATE_PENDING
from telecom_coworker.utils import get_hostip, get_hostname, set_interval, Process
...
class Coworker(object):
    ...
    def run(self, handle_type, handle_num, func):
        ...
        worker_info = WorkerInfo(wid=self.worker_id, handle_type=handle_type, max_handle_num=handle_num,
                                 hostname=get_hostname(), hostip=get_hostip(),
                                 pid=os.getpid())
    ...
    def handle_added_tasks(self, added_task_ids, func):
        ...
        for tid in added_task_ids:
            ...
            state: TaskState = TaskState(tid=tid, state=STATE_RUNNING, ip=self.hostip, pid=p.pid)
    ...
    def update_task_state(self, tid, task_state):
        state: TaskState = dict(task_state)
        state['updated_at'] = datetime.now().isoformat()
        log.info(f"task[{tid}] state changed: => {state}")
        self.zk.ensure_path(f"{self.namespace}/tasks/{tid}/state")
        self.zk.set(f"{self.namespace}/tasks/{tid}/state", json.dumps(state).encode("utf-8"))

        if state['state'] == STATE_FATAL:
            msg_content = f"ERROR: {state.get('logs', '')}"
            msg = Message('failed_task'.upper(), tid, self.worker_id, msg_content)

            send_message(self.zk, self.namespace, msg)
