...
class TextEdge(object):
    ...
    def update_coords2(self, x, y0, edge_tol):
        """Updates the text edge's x and bottom y coordinates and sets
        the is_valid attribute.
        """
        self.x = (self.intersections * self.x + x) / float(self.intersections + 1)
        ...
        # a textedge is valid only if it extends uninterrupted
        # over a required number of textlines
        if self.intersections >= TEXTEDGE_REQUIRED_ELEMENTS or self.y1-y0 > edge_tol:
            self.is_valid = True
...
class TextEdges(object):
    ...
    def find(self, x_coord, align):
        ...
        for i, te in enumerate(self._textedges[align]):
            if np.isclose(te.x, x_coord, atol=1):
                return i
        return None
    ...
    def add(self, textline, align):
        ...
        te.tokens.append(textline.text)
    ...
    def update(self, textline):
        ...
        for align in ["left", "right", "middle"]:
            ...
            if idx is None:
                ...
                self._textedges[align][idx].tokens.append(textline.text)

    def update2(self, textline):
        """Updates an existing text edge in the current dict.
        """
        for align in ["left", "right", "middle"]:
            x_coord = self.get_x_coord(textline, align)
            ...
            if idx is None:
                ...
                self._textedges[align][idx].update_coords2(
                    x_coord, textline.y0, edge_tol=self.edge_tol
                )
                self._textedges[align][idx].tokens.append(textline.text)
    ...
    def left(self):
        return self._textedges['left']

    def right(self):
        return self._textedges['right']

    def get_column_separators(self, textlines, lines, relevant_textedges):
        ...
        for te in relevant_textedges:
            te.x0-=6
            te.x1-=6
            if te.intersections<=2:
                te.is_valid = False
            if te.is_valid:
                if self.nb_overlap(lines, te)>=2 or self.overlap([span for tl in textlines for span in tl.spans], te):
                    te.is_valid=False
        #keep only valid edges:
        relevant_textedges=[te for te in relevant_textedges if te.is_valid]


        if len(relevant_textedges)==0:
            return []
        # sort relevant textedges in length order and from left to right
        relevant_textedges.sort(key=lambda te: (-(te.y1- te.y0), te.x))


        vertial_splitters=[]

        for line_edge in relevant_textedges:
            while not self.overlap([span for tl in textlines for span in tl.spans], line_edge) and line_edge.y0>0 and not self.overlap(lines, line_edge):
                line_edge.y0-=5
            line_edge.y0+=5

            while not self.overlap([span for tl in textlines for span in tl.spans], line_edge) and line_edge.y1 < self.page_height and not self.overlap(lines, line_edge):
                line_edge.y1 += 5
            line_edge.y1 -= 5


        cleand_relevant_textedges=[relevant_textedges[0]]

        for i in range(1, len(relevant_textedges)):
            add_to_relevant=True
            for edge in cleand_relevant_textedges:
                if np.isclose(edge.x, relevant_textedges[i].x, atol=50) and min(edge.y1, relevant_textedges[i].y1)- max(edge.y0, relevant_textedges[i].y0) >10:
                    add_to_relevant=False
                    continue
            if add_to_relevant:
                cleand_relevant_textedges.append(relevant_textedges[i])
        relevant_textedges=cleand_relevant_textedges

        relevant_textedges.sort(key=lambda te: (-(te.y1 - te.y0), te.x0))
        frst_col_x = None
        Separator=namedtuple('Separator', ['x0', 'y0', 'x1', 'y1'])
        for line_edge in relevant_textedges:
            #typically first column will be third of the page
            if np.isclose(line_edge.x0 , self.left_margin+self.page_width/3, atol=25) or np.isclose(line_edge.x0 , self.left_margin+2*self.page_width/3, atol=25):
                vertial_splitters.append(Separator(line_edge.x0, line_edge.y0, line_edge.x0, line_edge.y1))
                frst_col_x=line_edge.x
                continue
            #if the third page column is detected then than the midle column would be at the half of the distance remaining
            if frst_col_x and np.isclose(line_edge.x0 ,(self.page_width+frst_col_x)/2.0, atol=10):
                vertial_splitters.append(Separator(line_edge.x0, line_edge.y0, line_edge.x0, line_edge.y1))
            #otherwise we could have a page splited in half.
            elif not frst_col_x and np.isclose(line_edge.x0,self.left_margin+self.page_width/2.0, atol=20):
                vertial_splitters.append(Separator(line_edge.x0, line_edge.y0, line_edge.x0, line_edge.y1))

        return vertial_splitters
    ...
    def overlap(self, texlines, rectangle):
        for tl in texlines:
            if isRectangleOverlap(tl, rectangle):
                return True
        return False

    def nb_overlap(self, texlines, rectangle):
        overlaps=0
        y0=0
        y1=0
        intersections={}
        ...
        if not intersections:
            return 0
        return intersections[min([*intersections])]
