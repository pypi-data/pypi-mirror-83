...
def get_fonts(textline):
    ...
    for c in textline._objs:
        if isinstance(c, LTChar):
            ...
            font_size.append(c.size)
            font_width.append(c.width)
            if 'Bold' in c.fontname:
                    isBold = True

        break

    return {'size': mean(font_size),
            'name': fontname,
            'isBold': isBold,
            'width': mean(font_width)}


def are_in_same_Line(textline1, textline2, tol=4):
    return np.isclose(textline1.y0, textline2.y0, atol=tol) and  np.isclose(textline1.y1, textline2.y1, atol=tol)
...
def is_rotated(tline, text):
    if len(text) >1:
        return tline.width < tline.height
    return False
    ...
    try:
        for obj in layout._objs:
            if isinstance(obj, LTLine):
                if hasattr(obj, 'linewidth') and obj.linewidth>0:
                    t.append(obj)
                elif not hasattr(obj, 'linewidth'):
                    t.append(obj)
            elif isinstance(obj, LTRect) and obj.height < 1:
                t.append(obj)
...
def get_graphical_objects(layout, ltype="line", t=None):
    """Recursively parses pdf layout to get a list of
    PDFMiner text objects.
    Parameters
    ----------
    ...
    try:
        for obj in layout._objs:
            if isinstance(obj, LTObject):
                ...
                t += get_graphical_objects(obj, ltype=ltype)
    except AttributeError:
        pass
    return t
