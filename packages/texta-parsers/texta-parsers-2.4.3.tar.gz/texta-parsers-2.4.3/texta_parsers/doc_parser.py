...
class DocParser:
    ...
    def __init__(self, 
        ...
        max_file_size = 100,
        parse_only_extensions = Extension.KNOWN_EXTENSIONS):
        ...
        :param: [str] parse_only_extensions: If the input to the parse function is a directory then only files with given extensions will be parsed
        """
        ...
        self.scanner = DocScanner(parse_only_extensions=parse_only_extensions)

        if(len(set(Extension.DIGIDOC_EXTENSIONS) & set(parse_only_extensions))> 0):
            if not utils.check_digidoc_exists():
                raise FileNotFoundError("Digidoc-tool not found. Either exclude digidoc extensions from parse_only_extensions or install the tool.")
