...
from shyft.time_series import statistics_property, TimeSeries, TsVector
...
class DsViewHandleCreator(ABC):
    ...
    def get_views(self,
        ...
        for figure in self.figure_container:
            if figure:
                y_axis = self.get_y_axis(unit) if self.y_axes else None
    ...
    def get_y_axis(self, unit: Union[str, Unit]) -> Optional[YAxis]:
        """
        Auxiliary function for the management of YAxis.
        """
...
class BasicDsViewHandleCreator(DsViewHandleCreator):
    ...
    def create_ds_view_handles(self,
                               data: List[Union[Quantity, TsVector, TimeSeries,
                                          Tuple[Union[Quantity, TsVector, TimeSeries], str]]]) -> List[DsViewHandle]:
        """
        :param data: A list of TimeSeries, TsVector, or tuples containing data and label
        :return: A list of DsViewHandles for the data
        """
        dsvhs = []
        for element in data:
            ts, label = element if isinstance(element, Tuple) else (element, "")
            unit = ts.units if isinstance(ts, self.unit_registry.Quantity) else ""
            tsa = BasicTsAdapter(data=ts, unit_registry=self.unit_registry, unit=unit)
            data_source = DataSource(ts_adapter=tsa, unit=unit, tag=label)
