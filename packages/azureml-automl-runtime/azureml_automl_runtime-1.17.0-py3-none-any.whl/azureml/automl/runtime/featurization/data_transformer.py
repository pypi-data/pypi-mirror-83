...
class DataTransformer(AutoMLTransformer, FeaturizationInfoProvider):
    ...
    def __init__(self,
                 ...
                 allow_chargram: Optional[bool] = False) -> None:
        """
        ...
        :param allow_chargram: Flag to enable char-gram based features
        """
        super().__init__()
        ...
        # only allow char-gram based features in some cases
        self._allow_chargram = allow_chargram
    ...
    def _get_transforms(self, df: pd.DataFrame,
        ...
        for column_purpose in column_groups.keys():
            if column_purpose == _FeatureType.Text:
                max_len_text = sys.maxsize
                for text_col_name in column_groups[column_purpose]:
                    # max_len_text tracks the minimum length among the maximum string length values from
                    # all text columns in the dataset, to decide whether to apply char-gram based features
                    max_len_text = min(max_len_text, len(max(np.array(df[text_col_name].astype(str)), key=len)))
                min_len_for_chargram = min(TFIDF_VECTORIZER_CONFIG.CHAR_NGRAM_RANGE)
                self._logger_wrapper(
                    "info",
                    "Max length of any text column: {max_len_text}, min length for "
                    "char-gram: {min_len_for_chargram}".format(
                        max_len_text=max_len_text,
                        min_len_for_chargram=min_len_for_chargram))
                if max_len_text < min_len_for_chargram:
                    self._logger_wrapper('info', "Char-gram based features are not allowed.")
                else:
                    self._allow_chargram = True
                    self._logger_wrapper('info', "Char-gram based features are allowed.")
        ...
        for column in columnspurpose:
            ...
            if detected_column_purpose == _FeatureType.Numeric:
                ...
                self._text_transformer = self._text_transformer or TextTransformer(
                    ...
                    featurization_config=self._featurization_config,
                    allow_chargram=self._allow_chargram)
                if self._engineered_feature_names_class is None:
                    self._engineered_feature_names_class = _GenerateEngineeredFeatureNames()
