...
class BagOfWordsTransformer(AutoMLTransformer):
    ...
    def fit(self, X: DataInputType, y: DataSingleColumnInputType) -> "BagOfWordsTransformer":
        ...
        min_len_for_chargram = min(self._chargram_range)
        self.info_logger("Max length of any sample in text column = {}. Min length required for char-gram "
                         "featurizer to be applied = {}.".format(max_len_text, min_len_for_chargram))
        if max_len_text >= min_len_for_chargram and self._chargram_range != (0, 0):
            ...
            self.info_logger("Char-gram based features were added.")
        ...
        if self._wordgram_range != (0, 0):
            ...
            self.info_logger("Word-gram based features were added.")
