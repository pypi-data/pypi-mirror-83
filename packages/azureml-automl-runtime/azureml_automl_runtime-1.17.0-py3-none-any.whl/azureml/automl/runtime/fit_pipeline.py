        ...
        try:
            with automl_run_context.get_run() as run:
                ...
                if isinstance(e, TimeoutException):
                    logger.warning("Cancelling the Run {} due to timeout.".format(str(automl_run_context.run_id)))
                    run.cancel()
                else:
            ...
            try:
                with dataset.open_dataset():
                    ...
                    automl_model_explain_driver = AutoMLModelExplainDriverFactory._get_model_explain_driver(
                        ...
                        dataset=dataset,
                        automl_settings=automl_settings,
                        model_exp_workspace_config=cast(Dict[str, Any], feature_configs).get("model_explainability"),
                    )
