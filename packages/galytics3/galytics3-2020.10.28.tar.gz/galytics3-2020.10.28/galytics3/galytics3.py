...
class GoogleAnalytics:
    ...
    def _generate_periods(self, start, end, delta):
        intervals = [(start, start + dt.timedelta(delta))]
        while intervals[-1][-1] < end:
            d1 = intervals[-1][-1] + dt.timedelta(1)
            d2 = d1 + dt.timedelta(delta)
    ...
    def _generate_body(self, body, date1, date2, level_group_by_date, delta):
        ...
        intervals = self._generate_periods(date1, date2, delta)

        body_list_ = []
        for period in intervals:
            body_ = body.copy()
            body_["start_date"] = str(period[0].date())
            body_["end_date"] = str(period[1].date())
    ...
    def get_report(
        ...
        :param sort: str
        :param filters: str
        :param limit: int, максимальное кол-во строк в одном запросе. По умолчанию 10000.
        :param max_results: int, максимальное кол-во строк в отчете
        :param level_group_by_date: параметр устарел, больше не используется
