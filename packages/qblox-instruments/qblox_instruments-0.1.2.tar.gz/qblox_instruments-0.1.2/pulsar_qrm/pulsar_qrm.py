    ...
    def __init__(self, name, transport_inst, debug=0):
        ...
        for seq_idx in range(0, self._num_sequencers):
            ...
            self.add_parameter(
                ...
                get_parser = float,
                set_cmd    = pulsar_qrm._gen_set_func_par(self._set_sequencer_config_val, seq_idx, "phase_degree"),
                get_cmd    = pulsar_qrm._gen_get_func_par(self._get_sequencer_config_val, seq_idx, "phase_degree")
            )
            ...
            self.add_parameter(
                ...
                label      = "Sequencer {} trigger mode for acquisition path 0 (False = triggered by sequencer, True = triggered by input level)".format(seq_idx),
                docstring  = "Sets/gets sequencer {} trigger mode for acquisition path 0".format(seq_idx),
                unit       = '',
                vals       = vals.Bool(),
                get_parser = bool,
            ...
            self.add_parameter(
                ...
                label      = "Sequencer {} trigger mode for acquisition path 1 (False = triggered by sequencer, True = triggered by input level)".format(seq_idx),
                docstring  = "Sets/gets sequencer {} trigger mode for acquisition path 1".format(seq_idx),
                unit       = '',
                vals       = vals.Bool(),
                get_parser = bool,
