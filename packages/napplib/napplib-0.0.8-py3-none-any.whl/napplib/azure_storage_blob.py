...
class BlobStorage:
    ...
    @classmethod
    def get_latest_blob(self, account_name='', account_key='', path='', project='', store_name=''):
        # Azure configs
        block_blob_service = BlockBlobService(
            account_name=account_name, 
            account_key=account_key
        ...
        for blob in generator:
            ...
            match_name = f'{project}/{store_name}'
    ...
    def upload_blob(self, account_name='', account_key='', path='', project='', store_name='', output_file=''):
        ...
        try:
            block_blob_service.create_blob_from_path(path, target, out_filename)
            print(f'Upload file {target} finished!')
        except Exception as e:
            print(f'Failed to upload file {target} on azure blob storage: {e.message}')