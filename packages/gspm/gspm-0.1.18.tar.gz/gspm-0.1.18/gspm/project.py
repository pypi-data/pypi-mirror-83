...
def _validate(project):
    ...
    if 'version' in project.godot:
        if Version('{0}'.format(project.godot.version)) > Version('3.2.3'):
            raise Exception('Godot Version {0} is not supported.'.format(project.godot.version))
