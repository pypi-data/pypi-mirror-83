        ...
        if self.type == TYPE_CAMERA:
            try:
                ...
                while data is not None and offset < len(data):
                    image = CameraImage()
                    date_int = int.from_bytes(data[offset:offset + 8], "little") / 1000000
                    image.date = datetime.utcfromtimestamp(date_int).strftime('%Y-%m-%d_%H:%M:%S')
                ...
                Logger.error("Data error: data '" + self.type + "' is empty, invalid or corrupted")
            ...
            try:
                ...
                if data is not None and len(data) >= 20:
                    date_int = int.from_bytes(data[0: 8], "little") / 1000000
                    image.date = datetime.utcfromtimestamp(date_int).strftime('%Y-%m-%d_%H:%M:%S')
