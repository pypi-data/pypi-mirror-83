...
from h5_info.errors import GroupError
...
class H5Info:
    ...
    def save_metadata(self, filename):
        ...
        try:
            ...
            Logger.error("Permission error: {0}".format(error))
            sys.exit(1)
        except FileNotFoundError as error:
            Logger.error("FileNotFound error: {0}".format(error))
            sys.exit(1)
        except OSError as error:
            Logger.error("OS error: {0}".format(error))
            sys.exit(1)
    ...
    def load_data(self, h5_file, sensor_names=[]):
        ...
        try:
            with h5py.File(h5_file, 'r') as hdf:
                ...
                if session_gp is None:
                    raise GroupError(session_id)
                self.session = Session()
                self.session.date = str(session_gp.attrs['Date'], "utf-8")
                ...
                while geo_data is not None and offset < len(geo_data):
                    date.append(int.from_bytes(geo_data[offset:offset + 8], "little"))
                    longitude.append(struct.unpack('d', geo_data[offset + 8:offset + 16])[0])
                    latitude.append(struct.unpack('d', geo_data[offset + 16:offset + 24])[0])
                    uncertainty.append(struct.unpack('d', geo_data[offset + 24:offset + 32])[0])
                    tray_height.append(struct.unpack('d', geo_data[offset + 32:offset + 40])[0])
                    heading.append(struct.unpack('d', geo_data[offset + 40:offset + 48])[0])
                    course.append(struct.unpack('d', geo_data[offset + 48:offset + 56])[0])
                    roll.append(struct.unpack('d', geo_data[offset + 56:offset + 64])[0])
                    pitch.append(struct.unpack('d', geo_data[offset + 64:offset + 72])[0])
                    sog.append(struct.unpack('d', geo_data[offset + 72:offset + 80])[0])
                    offset += 80

                self.geo['date'] = np.array(date)
                self.geo['longitude'] = np.array(longitude)
                self.geo['latitude'] = np.array(latitude)
                self.geo['uncertainty'] = np.array(uncertainty)
                self.geo['tray_height'] = np.array(tray_height)
                self.geo['heading'] = np.array(heading)
                self.geo['course'] = np.array(course)
                self.geo['roll'] = np.array(roll)
                self.geo['pitch'] = np.array(pitch)
                self.geo['sog'] = np.array(sog)
            ...
            Logger.error("Group error: {0}".format(error))
            sys.exit(1)
        except PermissionError as error:
            Logger.error("Permission error: {0}".format(error))
            sys.exit(1)
