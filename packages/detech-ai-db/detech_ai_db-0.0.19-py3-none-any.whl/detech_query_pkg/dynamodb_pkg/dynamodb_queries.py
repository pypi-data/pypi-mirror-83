...
def increment_atomic_counter(counter_type, dynamodb):
  ...
  update_expression = "set value = value + :val"
  expression_attribute_values = {':val': decimal.Decimal(1)}
  table_name = ATOMIC_COUNTER_TABLE
  values = update_item(key_dict, update_expression, expression_attribute_values, table_name, dynamodb)
  return values
