...
def create_metric(id, name, provider, namespace, region_name, agent, org_id, component_id, alignment, dimension_name,
                  dimension_value, description, is_active, dynamodb):
  """
  create_metric(
...
def get_metric_details(metric_id, dynamodb):
  ...
  attr_list = ['id', 'name', 'namespace', 'alignment', 'dimension_name', 'dimension_value', 'org_id', 'is_active']
...
def insert_anomaly(id,
                   ...
                   is_dev_env=False):
  """
  insert_anomaly(id="125123", event_type="NEW", metric_id="m412", timestamp=1599563224, value=123.44,
                 end_timestamp=1599583224,
                 cross_correlations={
  ...
  item = {
    ...
    'possible_related_anomalies': possible_related_anomalies,
    'possible_related_matches': possible_related_matches
  }
  item = json.loads(json.dumps(item), parse_float=decimal.Decimal)
  if is_dev_env:
    response = put_item(item, DEV_ANOMALIES_TABLE, dynamodb)
  else:
    response = put_item(item, ANOMALIES_TABLE, dynamodb)
  return response
