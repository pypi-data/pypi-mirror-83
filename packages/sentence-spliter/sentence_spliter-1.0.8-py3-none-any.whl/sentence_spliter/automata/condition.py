...
class IsEndSymbol(Criteria):
    ...
    def accept(self, state: StrSequence) -> bool:
        if SYMBOLS['end_symbols'].match(state.current_value) and \
                not (SYMBOLS['en_dot'].match(state.next_value)
                     or SYMBOLS['end_symbols'].match(state.next_value)
                     or SYMBOLS['special_symbols'].match(state.next_value)):
            return True
...
class SpecialEnds(SpecialCondition):
    ...
    def accept(self, state: StrSequence) -> bool:
        if SYMBOLS['special_symbols'].match(state.pre_value) and self.pattern_cur.match(state.current_value):
            return True
        else:
            return False
