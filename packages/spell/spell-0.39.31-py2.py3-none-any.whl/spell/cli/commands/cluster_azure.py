...
def create_azure(ctx, name, resource_group_name, service_principal_name):
    ...
    Within this wrapper, the resource_id argument dictates which token is returned.
    For example, creating apps (graph_rbac_creds) and creating roles (management_cred)
    each have different tokens because they hit different endpoints in Azure.
    ...
    # TODO(sruthi/ben): find a way to query for tenant id
    tenant_id = click.prompt(
        "Enter tenant id of Azure Active Directory", default="c3084a14-3305-4fee-98fc-4f70239eec3f"
    )
    # Get Region
    available_regions = [
        location.name
...
def create_service_principal(client, service_principal_name):
    ...
    return client_id, sp_object_id, app_object_id
