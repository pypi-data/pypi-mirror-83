...
class CommonObjectDetectionDatasetWrapper(ObjectDetectionDatasetBaseWrapper):
    ...
    def __getitem__(self, index):
        ...
        boxes, labels, iscrowd, _ = self._filter_invalid_bounding_boxes(boxes, labels, iscrowd)
        if not boxes.shape[0]:
            logger.warning("(train: {}) No valid bbox for image index: {}.".format(self._is_train, index))
            return None, {}, {}

        # data augmentations
        image, boxes, areas, height, width = augmentations_transform(
            image, boxes, self._is_train, self._prob, self._transform)

        # validate bounding boxes after transform
        # if no valid bbox left, return None to skip this image, other pass only valid ones
        boxes, labels, iscrowd, areas = self._filter_invalid_bounding_boxes(boxes, labels, iscrowd, areas)
        if not boxes.shape[0]:
    ...
    @staticmethod
    def _filter_invalid_bounding_boxes(boxes, labels, iscrowd, areas=None):
        """validate bbox w/ condition of (x_min < x_max and y_min < y_max)
        ...
        :type iscrowd: List
        :param areas (optional): Bounding box areas.
        :type areas (optional): List
        ...
        return valid_boxes, valid_labels, valid_iscrowd, valid_areas
    ...
    def train_val_split(self, valid_portion=0.2):
        ...
        if number_of_samples == 1:
            ...
            training_indices = training_indices if len(training_indices) > validation_indices else validation_indices
