    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            if raise_exception:
                ...
                run = Run.get_context()
                automl_run = AutoMLRun(run.experiment, run.id)
                automl_run._is_local_run = False
                automl_run._fail_with_error(e)
...
def download_required_files(settings, dataset_class, model_factory_class):
    ...
    :param model_factory_class: ModelFactory used to initiate the model
    :type model_factory_class: Class derived from vision.common.base_model_factory.BaseModelFactory
    """
    ...
    model_factory_class.download_model_weights(model_name=model_name)
