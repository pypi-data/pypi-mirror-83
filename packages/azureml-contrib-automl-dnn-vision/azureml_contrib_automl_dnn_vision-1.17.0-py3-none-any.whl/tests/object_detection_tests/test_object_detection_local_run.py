...
def _get_settings(tmp_output_dir, add_validation_dataset, use_aml_dataset):
    ...
    if use_aml_dataset:
        from object_detection_tests.test_datasets import TestAmlDatasetObjectDetectionWrapper
...
def test_use_transform_local_run(mock_dataset_class, mock_transform,
    ...
    with patch.object(Run, 'get_context', return_value=mock_run):
        with tempfile.TemporaryDirectory() as tmp_output_dir:
            ...
            try:
                ...
                runner.run(settings)
                # if the training proceeds normally, the transform was not patched
                assert False, "Transform not patched or no images in the dataset"
            except (TypeError, AttributeError):
                _check_transform_mock(mock_transform)


def _check_transform_mock(mock_transform):
