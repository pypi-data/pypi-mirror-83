...
def reorder_trials(
    ...
    variables_axes = axes[np.logical_not(samples_in_arr1_are_in_arr2(axes, axes_table))]
    ...
    array_independent_variables_1_case = trials[advanced_indexing(indexes_array_independent_variables)]
    ...
    conditions_independent_variables = trials_to_conditions(
        array_independent_variables_1_case, axis_combinations=axis_combinations_in_combinations)
    combinations_independent_variables = conditions_to_combinations(
    ...
    combinations_variables_axes = n_conditions_to_combinations(shape_trials[variables_axes])
    ...
    if n_combinations_variables_axes == 0:
        ...
        for c in range(n_combinations_independent_variables):
            ...
            indexes_trials[axis_samples] = np.all(
                ...
                combinations_independent_variables[
                    advanced_indexing(indexes_combinations_independent_variables)],
            ...
            trials_ordered[advanced_indexing(indexes_trials_ordered)] = trials[advanced_indexing(indexes_trials)]
            t += k
    else:
        ...
        for i in range(n_combinations_variables_axes):
            ...
            array_independent_variables_i = trials[advanced_indexing(indexes_array_independent_variables)]
            for a in variables_axes_inverted:
