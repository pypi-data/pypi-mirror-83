...
class MixamoBot:
    ...
    def signup(self, email=None, password=None, first_name=None, last_name=None, birthday=None):
        ...
        if email is None:
            email = strings.generate_a_random_email()

        if password is None:
            password = strings.n_to_string_of_n_random_digits(n=10)

        if first_name is None:
            first_name = strings.generate_a_random_name()

        if last_name is None:
            last_name = strings.generate_a_random_name()
        ...
        while not job_done:
            try:
                ...
                if timer.get_time() > self.timeout:
    ...
    def download_animation(self, saved_fbx_as, rename_fbx_as):
        ...
        download.wait_downloading(saved_fbx_as, max_seconds_wait=180)
    ...
    def download_t_pose(self, rename_fbx_as, timeout_dowloading=600):
        ...
        while n_fbx_to_rename != 1:
            if timer.get_time() > timeout_dowloading:
