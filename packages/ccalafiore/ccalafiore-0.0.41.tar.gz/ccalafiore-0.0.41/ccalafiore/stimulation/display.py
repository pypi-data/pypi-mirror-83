import numpy as np
import sys
from ..maths import convert_to_int_or_float, convert_to_int
from ..array import MyRange
from ..combinations import (
    conditions_to_combinations_on_the_fly_with_indexes as
    cc_conditions_to_combinations_on_the_fly_with_indexes)
...
class Rectangle:
    ...
    def convert_to_pygame_coordinates(self, size_frame):
        ...
        left_pg = convert_to_int(left_pg)
        top_pg = convert_to_int(top_pg)
        width = convert_to_int(self.width)
        height = convert_to_int(self.height)
...
class Frame:
    ...
    def blit(self, source, point, alignment_x='c', alignment_y='c', area_sources=None, special_flags=0):
        ...
        #     # rect_destination_pg.centerx = convert_to_int(point_pg.x + .5)
        # elif alignment_x_low in ['l', 'left']:
        ...
        #     # rect_destination_pg.centery = convert_to_int(point_pg.y + .5)
        # elif alignment_y_low in ['b', 'bottom']:
    ...
    def blits_in_different_columns_and_rows(
        ...
        if not isinstance(points_y, (list, tuple, np.ndarray)):
            points_y = [points_y]
        for (j, i), point_ji in cc_conditions_to_combinations_on_the_fly_with_indexes([points_x, points_y]):
            self.blit(
