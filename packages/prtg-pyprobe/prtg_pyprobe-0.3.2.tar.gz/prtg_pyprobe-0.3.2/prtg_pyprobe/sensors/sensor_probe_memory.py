...
class Sensor(SensorPSUtilBase):
    ...
    def definition(self) -> dict:
        probe_health_def = SensorDefinition(
            name=self.name,
            kind=self.kind,
            description="Sensor used to monitor the memory usage of the system on which the python probe is runnning",
            default="yes",
            sensor_help="Sensor used to monitor the memory usage of the system on which the python probe is runnning",
