...
class OsuAPIWrapper:
	def __init__(self, api_key: str, server_name: str = 'bancho') -> None:
		"""
		Hello!
		...
		# get the server from the name given.
		self.server = self.resolve(server_name)
		if not self.server:
	...
	def get_score(self, user: Union[str, int], beatmapid: int,
				  mode: int = 0, relax: bool = False, limit: int = 0):
		"""
		...
		elif self.server == Server.Akatsuki:
			params = {
				'u': user_id,
				'm': mode,
				'rx': int(relax),
		...
		elif self.server == Server.Ripple:
			params = {
				'u': user_id,
				'mode': mode,
				'relax': int(relax),
		...
		elif self.server == Server.Gatari:
			params = {
				'u': user_id,
				'mode': mode,
				'b': beatmapid
		...
		r = self.http_sess.get(f'{self.server.url}/{path}', params=params)
		if not r or r.status_code != 200:
			return
	...
	def profile(self, user: Union[str, int],
		...
		if self.server == Server.Bancho:
			...
			r = self.http_sess.get(f"{self.server.url}/get_user", params=params)
			if not r or r.status_code != 200:
				return
			...
			r = self.http_sess.get(f"{self.server.url}/users/full", params=params)
			if not r or r.status_code != 200:
				return
			...
			r = self.http_sess.get(f"{self.server.url}/users/get", params=params)
			if not r or r.status_code != 200:
				return
			...
			r = self.http_sess.get(f"{self.server.url}/user/stats", params=params)
			if not r or r.status_code != 200:
				return
	...
	def recent_score(self, user: Union[str, int], mode: int = 0,
		...
		if self.server == Server.Bancho:
			...
			r = self.http_sess.get(f"{self.server.url}/get_user_recent", params=params)
			if not r or r.status_code != 200:
				return
			...
			if stats['rank'] != 'F':
				...
				r = self.http_sess.get(f'{self.server.url}/get_scores', params=params)
			...
			r = self.http_sess.get(f"{self.server.url}/get_beatmaps", params=params)
			if not r or r.status_code != 200:
				return
			...
			r = self.http_sess.get(f"{self.server.url}/users/scores/recent", params=params)
			if not r or r.status_code != 200:
				return
			...
			r = self.http_sess.get(f"{self.server.url}/user/scores/recent", params=params)
			if not r or r.status_code != 200:
				return
	...
	def best_score(self, user: Union[str, int], mode: int = 0,
		...
		if self.server == Server.Bancho:
			...
			r = self.http_sess.get(f"{self.server.url}/get_user_best", params=params)
			if not r or r.status_code != 200:
				return
			...
			r = self.http_sess.get(f"{self.server.url}/users/scores/best", params=params)
			if not r or r.status_code != 200:
				return
			...
			r = self.http_sess.get(f"{self.server.url}/user/scores/best", params=params)
			if not r or r.status_code != 200:
				return
