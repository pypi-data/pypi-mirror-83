...
class osu():
	...
	def recent_score(self, userid, mode = 0, relax = 0, limit = 0):
		...
		if 'bancho' in self.api_base:
			...
			_stats = requests.get(f"{base_api}/get_user_recent?", params=params)
			if not _stats:
				raise Exception("User Coudn't be found.")
			try:
				stats = _stats.json()[limit - 1 if limit > 0 else limit]
			...
			if stats['rank'] != 'F':
				...
				_info = requests.get(f'{base_api}/get_scores?', params=params)
	...
	def best_score(self, userid, mode = 0, relax = 0, limit = 0):
		...
		if 'bancho' in self.api_base:
			...
			_stats = requests.get(f"{base_api}/get_user_best?", params=params)
			if not _stats:
				raise Exception("User Coudn't be found.")
			try:
				stats = _stats.json()[limit - 1 if limit > 0 else limit]
