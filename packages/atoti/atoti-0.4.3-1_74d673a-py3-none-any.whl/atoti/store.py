...
from ._sources.kafka import KafkaDataSource
...
class Store:
    ...
    @property
    def _types(self) -> Mapping[str, AtotiType]:
    ...
    def drop(self, *coordinates: Mapping[str, Any], in_all_scenarios: bool = False):
        ...
        Each set of coordinates can only contain one value for each column. To specify mulitple
        values for one column, mulitple mappings must be passed.

        Args:
            coordinates: Mappings between store columns and values. Rows which match
                the provided mappings will be deleted from the store.
            in_all_scenarios: Whether or not the rows should be dropped on all of the store's
                scenarios or just the current one.
        """
    ...
    def load_kafka(
        ...
        *,
        ...
        """
        KafkaDataSource(self._java_api).load_kafka_into_store(
            self,
            bootstrap_server,
            topic,
            group_id,
