...
configini = r"Z:\DeepLabCut\DLC_extract\Troubleshooting\SLEAP_9Test_2\project_folder\project_config.ini"
...
for currentFile in filesFound:
    ...
    csv_df = pd.read_csv(currentFile, usecols=columns2grab)
    ...
    for index, row in csv_df_combined.iterrows():
        ...
        for path in range(len(dequePaths)):
            currentPath = dequePaths[path]
            currColor = animalColors[path]
            for line, color in zip(currentPath, animalColors):
                print(line[2])
                cv2.line(img, (line[2], line[3]), (line[0], line[1]), currColor, 2)
        rowCounter += 1
        ...
        for currTuple, color, animalID in zip(animalTuples, animalColors, multiAnimalIDList):
            ...
            cv2.putText(image_new, str(multiAnimalIDList[animal]),(currTuple[0], currTuple[1]), cv2.FONT_HERSHEY_TRIPLEX, textScale, color, 2)
