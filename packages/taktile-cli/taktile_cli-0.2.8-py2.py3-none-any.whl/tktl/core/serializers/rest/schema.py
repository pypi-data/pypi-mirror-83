...
def get_dataframe_model(
    base_model: Type[BaseModel], example: List[Dict]
) -> Type[CustomDeserializingModelT]:
    validates_dt_model = _dt_validator_model(base_model)

    class DataFrame(CustomDeserializingModelT):
        __root__: List[validates_dt_model]
...
def get_flat_array_model(inner_type: type, example: List[Any]):
    class FlatArray(BaseModel):
        __root__: List[inner_type] = Field(..., alias="value")
...
def _dt_validator_model(base_model):
...
def get_typed_sequence_model(inner_type):
    class SimpleTypedSequenceEncoderModel(CustomDeserializingModelT):
        __root__: List[inner_type]
    ...
    return SimpleTypedSequenceEncoderModel
