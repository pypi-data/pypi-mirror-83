...
class TabularEndpoint(Endpoint):
    ...
    def __init__(self, func, X, y):
        super().__init__(func)
        self.input_schema = EndpointInputSchema(value=X, endpoint_kind=self.KIND)
        self.output_schema = EndpointOutputSchema(value=y, endpoint_kind=self.KIND)

    def _profiling_supported(self):
        if (
