...
from trains.utilities.resource_monitor import ResourceMonitor
...
from ..training.callbacks import Callbacks, EarlyStopping
...
class Trainer(TrainerBase):
    def __init__(
        ...
        run_validation_on_checkpoint: bool = False,
        trains_logging: bool = True,
        trains_task: Optional[Task] = None,
        trains_additional_params_to_log: Optional[Dict[str, Any]] = None,
    ) -> None:
        """
        ...
        trains_logging : ``bool``, optional (default = True)
        trains_task: ``Task``, optional  (default = None). Will be created automatically if trains_logging is True
        """
        ...
        if trains_task is None and trains_logging:
            ...
            trains_experiment_name = self._experiment_name
            if experiment_name is not None:
                language = experiment_name.split("_")[0]
                trains_experiment_name = "_".join(experiment_name.split("_")[1:])
            trains_task = Task.init(
                project_name=project_name,
                task_name=trains_experiment_name,
                auto_resource_monitoring=False,
                auto_connect_arg_parser=False,  # The config file path. Maybe it's needed?
            )
            trains_task.add_tags([language])
            # Fix for the GPU metrics reporting
            trains_resource_daemon = ResourceMonitor(
                trains_task,
                report_mem_used_per_process=True,
                wait_for_first_iteration_to_start_sec=int(1e9),
                max_wait_for_first_iteration_to_start_sec=int(1e9),
            )
            trains_resource_daemon.start()

        self._callbacks = Callbacks(
            early_stopping=early_stopping_configuration, trains_task=trains_task if trains_logging else None,
        )
        ...
        self._run_validation_on_checkpoint = run_validation_on_checkpoint
        ...
        trains_init_params = {
            ...
            "run_validation_on_checkpoint": run_validation_on_checkpoint,
        }
        trains_init_params.update(self._model.get_serializable_params())
        if trains_additional_params_to_log is not None:
            trains_init_params.update(trains_additional_params_to_log)
        self._callbacks.trains_update_parameters(trains_init_params)
    ...
    def _train_epoch(self, master_bar_logger: master_bar, epoch: int) -> Dict[str, float]:
        ...
        for batch_group in progress_bar(
            ...
            if batches_this_epoch % self._accumulation_steps == 0:
                ...
                self._callbacks.trains_report_scalar(
                    title="Training",
                    series="loss",
                    value=loss.item(),
                    iteration=(100 * epoch)
                    + int(100.0 * global_steps / num_training_batches / self._accumulation_steps),
                )
            ...
            if self._checkpoint_steps is not None and (global_steps + 1) % self._checkpoint_steps == 0:
                checkpoint_folder = os.path.join(self._serialization_dir, f"epoch_{epoch}_checkpoint_{global_steps}")
                ...
                if self._run_validation_on_checkpoint:
                    with torch.no_grad():
                        val_metrics = self._validation_run(master_bar_logger=master_bar_logger)
    ...
    def train(self) -> None:
        ...
        for epoch in mb:
            ...
            for metric_name, metric_value in train_metrics.items():
                self._callbacks.trains_report_scalar(
                    title="Training", series=metric_name, value=metric_value, iteration=(epoch + 1) * 100
                )
            ...
            with torch.no_grad():
                ...
                if val_metrics is not None:
                    ...
                    for metric_name, metric_value in val_metrics.items():
                        self._callbacks.trains_report_scalar(
                            title="Test", series=metric_name, value=metric_value, iteration=(epoch + 1) * 100
                        )
