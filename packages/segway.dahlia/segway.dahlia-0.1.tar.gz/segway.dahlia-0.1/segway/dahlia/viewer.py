...
class DahliaViewer():
    ...
    def _set_state_from_viewer_state(self, target_state):
        ...
        layer_json = layer_json['pr']
        target_state['name'] = layer_json['prNeuronName']
        # target_state['blacklist_segments'] = self.blacklist_segments
        target_state['annotator'] = layer_json['prAnnotator']
        target_state['cell_type'] = layer_json['prCellType']
        target_state['tags'] = layer_json['prTags'].split()
        target_state['finished'] = True if (layer_json['prFinished'] is True or layer_json['prFinished'] == "1" or layer_json['prFinished'] == "true") else False
        target_state['reviewed'] = True if (layer_json['prReviewed'] is True or layer_json['prReviewed'] == "1" or layer_json['prReviewed'] == "true") else False
        # target_state['loc_tags'] = []
        target_state['notes'] = layer_json['prNotes']
        target_state['uncertain_xyz'] = layer_json['prUncertainCon']
        target_state['mergers_xyz'] = layer_json['prMergers']
        target_state['soma_loc'] = layer_json['prSomaLoc']
    ...
    def search_neuron(self, s):
        ...
        dbNeuronPrefix = attrs['neurondb']['dbNeuronPrefix']
        dbFindAnnotator = attrs['neurondb']['dbFindAnnotator']
        dbFindType = attrs['neurondb']['dbFindType']
        dbFindTags = attrs['neurondb']['dbFindTags']
        dbFindFinished = attrs['neurondb']['dbFindFinished']
        dbFindReviewed = attrs['neurondb']['dbFindReviewed']
    ...
    def load_neuron_by_name(self, s, slot=0):
        ...
        if slot == 0:
            name = attrs['neurondb']['dbLoadNeuronName']
        elif slot == 1:
            name = attrs['neurondb']['dbLoadNeuronName1']
        elif slot == 2:
            name = attrs['neurondb']['dbLoadNeuronName2']
        elif slot == 3:
            name = attrs['neurondb']['dbLoadNeuronName3']
        else:
        ...
        return self._load_neuron(name, with_children)
    ...
    def prSomaLocCopyLocEvent(self, s):
        ...
        coord = s._json_data['navigation']['pose']['position']['voxelCoordinates']
