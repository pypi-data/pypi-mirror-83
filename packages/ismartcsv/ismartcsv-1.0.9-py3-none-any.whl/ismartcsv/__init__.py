...
from .configuration import config_file, create_field
...
def read_from_file(datfile_listname, config_filename, base_folder="./"):
    
    if not os.path.isfile(datfile_listname):
        raise TypeError("datfile_listname should be a ascii file with list of filenames to be parsed")

    config = config_file(config_filename)

    if not config.is_valid():
        raise ValueError("Invalid configuration filename provided")
    ...
    with open(datfile_listname,'r') as datfile:
        filenames = datfile.read().splitlines()
        for fname in filenames:
            filename = os.path.abspath(os.path.join(base_folder, fname))
