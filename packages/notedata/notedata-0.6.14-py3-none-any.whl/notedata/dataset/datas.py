...
class CriteoData(DataSet):
    ...
    def _create_criteo_dataset(self, file, embed_dim=8, read_part=True, sample_num=100000, test_size=0.2, batch_size=128):
        ...
        data_df[dense_features] = data_df[dense_features].fillna(
            0.).astype('float32')
        data_df[sparse_features] = data_df[sparse_features].fillna('-1')
        data_df[['label']] = data_df[['label']].astype('float32')
        # for feat in sparse_features:
        #     le = LabelEncoder()
        #     data_df[feat] = le.fit_transform(data_df[feat])
        ...
        sparse_json_list = []
        for feat in sparse_features:
            vocabulary = data_df[feat].unique()
            para = define_feature_json(feat,
                                       feature_type='CateEmbeddingColumn',
                                       dimension=embed_dim,
                                       share_name=feat + '-emb',
                                       vocabulary_list=vocabulary,
                                       # bucket_size=len(vocabulary),
                                       dtype='string'
                                       )
            sparse_json_list.append(para)
        ...
        return feature_layers, (train.to_dict(orient='list'), train.pop('label')), (test.to_dict(orient='list'), test.pop('label'))
