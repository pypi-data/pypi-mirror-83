...
def CC_Cap(xset, yset, current, m1=False, m2=False, ESR_method = True, setting = False, filename=False, norm_cap=False):
    ...
    #print([yset[peaks[i]:troughs[i]][mid_ind[i]] for i in range(len(peaks))])
    cc_grad = [polyfit(xset[peaks[i]:troughs[i]][mid_ind[i]:-1], yset[peaks[i]:troughs[i]][mid_ind[i]:-1],1, cov=False)[0] for i in range(len(peaks))]
