...
class BaseNotification:
    ...
    def __init__(self, api_key, client_id, instance=None, stage='DEV'):
...
class MobileNotification(BaseNotification):
    ...
    def get_url(self, target=None, **kwargs):
        ...
        if target == None:
            ...
            if self.dest == 'group':
                target = f'{self.client_id}-group-{target}'
            elif self.dest == 'topic':
                target = f'{self.client_id}-{target}'
            elif self.dest == 'user':
                path = 'user'
        host = self.get_host()
        return f'{host}/notification/publish/{path}/{target}'
