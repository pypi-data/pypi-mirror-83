...
#
  ...
  def readInternal(self, st): # PoserFileParser st)
    ...
    while not fin:
      ...
      if st.ttype == PoserFileParser.TT_WORD:
        ...
        if st.sval=="numbVerts":
          st.getToken()
          numbVerts = int(st.nval)

        if st.sval=="numbTVerts":
          st.getToken()
          numbTVerts = int(st.nval)

        if st.sval=="numbTSets":
          st.getToken()
          numbTSets = int(st.nval)

        if st.sval=="numbElems":
          st.getToken()
          numbElems = int(st.nval)

        if st.sval=="numbSets":
          st.getToken()
          numbSets = int(st.nval)

        if st.sval=="usemtl":
          # FIX 20110314-ODY: Material names could contain white chars and others (like #)
          st.getToNextLine()
          self.addMaterial(st.sval)
        ...
        if st.sval=="g":
          st.getToken() #  should read a NAME
          self.curGroup = self.createGeomGroup(st.sval)
          self.curGroup.setMaterialName(self.curMatIdx)
          continue

        # Record mtllibs that could be read and resolved later
        if st.sval=="mtllib":
          st.getToken() #  should read a FILENAME
          if st.getFileNature() == OBJ_FNAT:
            self._libMaterialName = st.sval
            # log.info("mtllib " + st.sval + " stored")
          else:
            # log.info("mtllib " + st.sval + " ignored")
            pass
          continue

        if (st.sval[0]=="g") and (len(st.sval) > 1):
          # Strange case where there's no space after 'g' (OBJ regular ??)
          self.curGroup = self.createGeomGroup(st.sval[1:])
          self.curGroup.setMaterialName(self.curMatIdx)
          continue
        ...
        # log.warning("L[" + st.lineno() + "] - Not Accepted :" + st.sval)
        raise Exception("L[%d] - Not Accepted : %s", st.lineno(), st.sval)

    if (st.getFileNature() != OBJ_FNAT) and not self.groups:
  ...
  def readFace(self, st): # throws ParsingErrorException
    ...
    line = st.getToNextLine()

    tabVert = line.split(" ")

    if ((tabVert != None) and (len(tabVert) > 0)):
    ...
    # In case we exited early st.skipToNextLine()
    # // End of readFace
  ...
  def readLine(self, st):
    ...
    while (st.ttype != PoserFileParser.TT_EOL):
