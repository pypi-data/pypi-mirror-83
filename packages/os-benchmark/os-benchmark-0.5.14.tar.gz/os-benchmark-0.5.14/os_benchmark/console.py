...
class Controller:
    ...
    def time_sql(self):
        DEFAULT_EXPRESSION = 'SELECT COUNT(*)'
        self.subparser.add_argument('--storage-class', required=False)
        self.subparser.add_argument('--rows', type=int, required=False)
        self.subparser.add_argument('--columns', type=str, default='int', required=False)
        self.subparser.add_argument('--randomseed', type=int, required=False)
        self.subparser.add_argument('--expression', type=str, default=DEFAULT_EXPRESSION,
                                    required=False)
        parsed_args = self.parser.parse_known_args()[0]

        benchmark = benchmarks.SqlBenchmark(self.driver)
        benchmark.set_params(
            storage_class=parsed_args.storage_class,
