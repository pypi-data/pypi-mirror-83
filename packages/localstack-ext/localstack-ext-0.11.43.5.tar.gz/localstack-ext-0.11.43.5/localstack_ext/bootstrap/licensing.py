...
def read_api_key():
 ...
 if key:
  return key
 raise FeuSY('Unable to retrieve API key. Please configure $LOCALSTACK_API_KEY in your environment')
def fetch_key():
 ...
 try:
  ...
  result=requests.post('%s/activate'%config.API_URL,json.dumps(data),verify=FeuSJ)
...
def load_cached_key():
 ...
 if(now-cached_key['timestamp'])>MAX_KEY_CACHE_DURATION_SECS:
  raise FeuSY('Cached key expired')
 timestamp=FeuSl(cached_key['timestamp'])
