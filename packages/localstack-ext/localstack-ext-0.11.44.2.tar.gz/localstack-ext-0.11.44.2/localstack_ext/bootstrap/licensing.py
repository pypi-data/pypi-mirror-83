...
def read_api_key():
 ...
 if key:
  return key
 raise mOyNR('Unable to retrieve API key. Please configure $LOCALSTACK_API_KEY in your environment')
def fetch_key():
 ...
 try:
  ...
  result=requests.post('%s/activate'%config.API_URL,json.dumps(data),verify=mOyNf)
...
def load_cached_key():
 ...
 if(now-cached_key['timestamp'])>MAX_KEY_CACHE_DURATION_SECS:
  raise mOyNR('Cached key expired')
 timestamp=mOyNG(cached_key['timestamp'])
