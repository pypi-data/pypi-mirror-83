...
def install_postgres():
 if not in_docker():
  return
 try:
  run('which postgres',print_error=rihKg)
 except rihKt:
  install_alpine_package('postgresql','RDS')
 try:
  run('which pg_config',print_error=rihKg)
 except rihKt:
  install_alpine_package('postgresql-dev','RDS')
def install_redis():
 try:
  run('which redis-server',print_error=rihKg)
 except rihKt:
  install_alpine_package('redis','ElastiCache')
 return 'redis-server'
def install_mqtt():
 try:
  run('which mosquitto',print_error=rihKg)
 except rihKt:
...
def setup_ssl_cert():
 ...
 if os.path.exists(target_file):
  ...
  if mod_time>(now()-cache_duration_secs):
   return
 def do_download(url,print_error=rihKg):
  try:
 ...
 do_download(SSL_CERT_URL)or do_download(SSL_CERT_URL_2,rihKb)
def install_h2():
