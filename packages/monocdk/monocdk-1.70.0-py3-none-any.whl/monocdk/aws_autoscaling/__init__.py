...
class CommonAutoScalingGroupProps:
    def __init__(
        ...
        *,
        ...
        :param replacing_update_min_successful_instances_percent: (experimental) Configuration for replacing updates. Only used if updateType == UpdateType.ReplacingUpdate. Specifies how many instances must signal success for the update to succeed. Default: minSuccessfulInstancesPercent
        :param resource_signal_count: (experimental) How many ResourceSignal calls CloudFormation expects before the resource is considered created. Default: 1
        :param resource_signal_timeout: (experimental) The length of time to wait for the resourceSignalCount. The maximum value is 43200 (12 hours). Default: Duration.minutes(5)
        :param rolling_update_configuration: (experimental) Configuration for rolling updates. Only used if updateType == UpdateType.RollingUpdate. Default: - RollingUpdateConfiguration with defaults.
        ...
        :param update_type: (experimental) What to do when an AutoScalingGroup's instance configuration is changed. This is applied when any of the settings on the ASG are changed that affect how the instances should be created (VPC, instance type, startup scripts, etc.). It indicates how the existing instances should be replaced with new instances matching the new config. By default, nothing is done and only new instances are launched with the new config. Default: UpdateType.None
    ...
    def replacing_update_min_successful_instances_percent(
        ...
        """(experimental) Configuration for replacing updates.
    ...
    def resource_signal_count(self) -> typing.Optional[jsii.Number]:
        """(experimental) How many ResourceSignal calls CloudFormation expects before the resource is considered created.
    ...
    def rolling_update_configuration(
        ...
        """(experimental) Configuration for rolling updates.
...
class RequestCountScalingProps(BaseTargetTrackingProps):
    def __init__(
        ...
        *,
        ...
        :param disable_scale_in: (experimental) Indicates whether scale in by the target tracking policy is disabled. If the value is true, scale in is disabled and the target tracking policy won't remove capacity from the autoscaling group. Otherwise, scale in is enabled and the target tracking policy can remove capacity from the group. Default: false
        :param estimated_instance_warmup: (experimental) Estimated time until a newly launched instance can send metrics to CloudWatch. Default: - Same as the cooldown.
        :param target_requests_per_second: (experimental) Target average requests/seconds on each instance.
    ...
    def target_requests_per_second(self) -> jsii.Number:
        """(experimental) Target average requests/seconds on each instance.
...
class UpdateType(enum.Enum):
    """(experimental) The type of update to perform on instances in this AutoScalingGroup.

    :stability: experimental
    """

    NONE = "NONE"
    """(experimental) Don't do anything.

    :stability: experimental
    """
    REPLACING_UPDATE = "REPLACING_UPDATE"
    """(experimental) Replace the entire AutoScalingGroup.
    ...
    """(experimental) Replace the instances in the AutoScalingGroup.
