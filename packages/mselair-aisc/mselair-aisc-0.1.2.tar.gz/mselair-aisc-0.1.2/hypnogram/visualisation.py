...
def hypnogram(orig_df, hypnogram_values=None, hypnogram_colors=None, fontsize=12):
    _hypnogram_values = {
        'AWAKE': 6,
        'Arrousal': 5,
        'REM': 4,
    ...
    plt.xlabel('\n Time [' + df.start[0].strftime('%d.%m.%Y') + ' - ' + df.iloc[-1].end.strftime('%d.%m.%Y') + ']', fontsize=fontsize)
