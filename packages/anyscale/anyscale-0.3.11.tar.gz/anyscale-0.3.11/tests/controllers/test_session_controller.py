...
def test_anyscale_up_success(mock_up_tuple: Tuple[Mock, Project, Cloud]) -> None:
    ...
    with patch.multiple(
        "anyscale.controllers.session_controller", rsync=Mock()
...
def test_anyscale_up_no_config_provided(
    ...
    with patch.multiple("anyscale.controllers.session_controller", rsync=Mock()):
        session_controller.up(
            session_name=None,
            config=None,
            min_workers=None,
...
def test_anyscale_push_session(
    ...
    with patch.multiple("subprocess", run=mock_subprocess_run), patch.multiple(
        ...
        mock_validate_cluster_configuration.assert_called_once_with(config, ANY)
