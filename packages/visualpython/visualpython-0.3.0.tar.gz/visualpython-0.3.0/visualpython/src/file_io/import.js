define([
    ...
    var initOption = function(callback) {
        vpCommon.loadHtml(vpCommon.wrapSelector(vpConst.OPTION_GREEN_ROOM), "file_io/import.html", optionLoadCallback, callback);
    }
    ...
    ImportPackage.prototype.bindImportPackageGrid = function() {
        ...
        this.packageList.forEach((package, i) => {
            ...
            //     </select>
            var tagTr = $(`<tr><td>import</td><td><input id="vp_library${i}" type="text" class="vp-add-library" placeholder="library name" required value="${package.name.toLowerCase()}"/></td>
            <td>
                as
            </td>
            <td><input id="vp_alias${i}" type="text" class="vp-add-alias" placeholder="as" value="${package.variable}"/></td>
        ...
        $(this.wrapSelector('#vp_addLibrary')).click(function() {
            var tagTr = $(`<tr><td>import</td><td><input type="text" class="vp-add-library" placeholder="library name" required/></td>
            <td>as</td>
            <td><input type="text" class="vp-add-alias" placeholder="alias"/></td>
            <td><input type="button" class="vp-remove-option w100" style="width:100%;" value="x"></td></tr>`);
    ...
    ImportPackage.prototype.generateCode = function(exec) {
        ...
        for (var idx = 0; idx < libraryList.length; idx++) {
            ...
            var pacType = $(libraryList[idx]).find('.vp-add-type').val();
            var pacAlias = $(libraryList[idx]).find('.vp-add-alias').val();

            // if (pacName == "" || (pacType == "import" && pacAlias == "")) {
            //     vpCommon.renderAlertModal('빈 라이브러리 입력칸을 채우거나 삭제하세요');
            //     return "BREAK_RUN";
            // }

            // if (pacType == 'as') {
                // import ~ as ~
                sbCode.appendFormatLine("import {0}{1}", pacName, ((pacAlias === undefined || pacAlias === "") ? "" : (" as " + pacAlias)));
            // } else {
            //     // from ~ import ~
            //     sbCode.appendFormatLine("from {0} import {1}", pacName, pacAlias);
            // }

            this.packageList.forEach(pack => {
                if (pack.name == pacName) {
