define([
    ...
    const { BLOCK_CODE_BTN_TYPE
            ...
            , COLOR_RED
            , COLOR_GREEN
            , DEFAULT_VARIABLE_ARRAY_LIST } = constData;
    ...
    var Block = function(blockContainerThis, type, pointObj) {
        ...
        if (blockCodeType === BLOCK_CODE_TYPE.CLASS) {
            var defBlock = mapTypeToBlock(this.blockContainerThis, BLOCK_CODE_TYPE.DEF, {pointX: 0, pointY: 0});
            defBlock.setState({
                defName: '__init__'
                , defInParamList:['self']
            });
            ...
            var returnBlock = mapTypeToBlock(this.blockContainerThis, BLOCK_CODE_TYPE.RETURN, {pointX: 0, pointY: 0});
            var holderBlock = mapTypeToBlock(this.blockContainerThis, BLOCK_CODE_TYPE.HOLDER, {pointX: 0, pointY: 0});
            ...
            var passBlock = mapTypeToBlock(this.blockContainerThis, BLOCK_CODE_TYPE.PASS, {pointX: 0, pointY: 0});
            var holderBlock = mapTypeToBlock(this.blockContainerThis, BLOCK_CODE_TYPE.HOLDER, {pointX: 0, pointY: 0});
    ...
    /**
     * 생성한 elifList 중에 가장 아래에 위치한 elif block을 set, get
    ...
    Block.prototype.deleteBlockScope = function(isFirst) {
        ...
        if (this.getType() === BLOCK_CODE_TYPE.CODE || this.getType() === BLOCK_CODE_TYPE.IMPORT ||
            this.getType() === BLOCK_CODE_TYPE.PASS || this.getType() === BLOCK_CODE_TYPE.CONTINUE || this.getType() === BLOCK_CODE_TYPE.PROPERTY
            || this.getType() === BLOCK_CODE_TYPE.BREAK ) {
    ...
    Block.prototype.makeChildBlockDomList = function(enumData) {
        ...
        selectChildBlockList.forEach((block, index) => {
            ...
            if (index === 0) {
                ...
                } else if (currBlock.getType() === BLOCK_CODE_TYPE.RETURN || currBlock.getType() === BLOCK_CODE_TYPE.PROPERTY) {
                    backColor = `${COLOR_BLUE}`;
                    $(mainDom).addClass('vp-block-class-def');

                } else if (currBlock.getType() === BLOCK_CODE_TYPE.BREAK || currBlock.getType() === BLOCK_CODE_TYPE.CONTINUE
                            || currBlock.getType() === BLOCK_CODE_TYPE.PASS) {
                    backColor = `${COLOR_RED}`;
                    $(mainDom).addClass('vp-block-if');
                } else {
    ...
    Block.prototype._renderBlockColor = function() {
        ...
        } else if (blockCodeType === BLOCK_CODE_TYPE.RETURN || blockCodeType === BLOCK_CODE_TYPE.PROPERTY) {
            $(mainDom).css(STR_BACKGROUND_COLOR, COLOR_BLUE);
    ...
    Block.prototype.renderBlockLeftHolderDom = function(mainDom, blockType , optionData) {
        ...
        if (blockType === BLOCK_TYPE.BLOCK) {
            if (blockCodeType === BLOCK_CODE_TYPE.CLASS || blockCodeType === BLOCK_CODE_TYPE.DEF || blockCodeType === BLOCK_CODE_TYPE.RETURN
                || blockCodeType === BLOCK_CODE_TYPE.PROPERTY) {
                $(mainDom).addClass('vp-block-class-def');
            }
            ...
            if (blockCodeType === BLOCK_CODE_TYPE.IF || blockCodeType === BLOCK_CODE_TYPE.FOR
                ...
                || blockCodeType === BLOCK_CODE_TYPE.FINALLY || blockCodeType === BLOCK_CODE_TYPE.BREAK || blockCodeType  === BLOCK_CODE_TYPE.CONTINUE 
                || blockCodeType === BLOCK_CODE_TYPE.PASS) {
                $(mainDom).addClass('vp-block-if');
            }
            ...
            if (blockCodeType === BLOCK_CODE_TYPE.CLASS 
                ...
                || blockCodeType === BLOCK_CODE_TYPE.RETURN
                || blockCodeType === BLOCK_CODE_TYPE.PROPERTY ) {
                var blockLeftHolderDom = $('<div class="vp-block-left-holder"></div>');
            ...
            if (currBlock.getType() === BLOCK_CODE_TYPE.CLASS || currBlock.getType() === BLOCK_CODE_TYPE.DEF || currBlock.getType() === BLOCK_CODE_TYPE.RETURN
                || currBlock.getType() === BLOCK_CODE_TYPE.PROPERTY) {
                var blockLeftHolderDom = $('<div class="vp-block-left-holder"></div>');
            ...
            if (currBlock.getType() === BLOCK_CODE_TYPE.IF || currBlock.getType() === BLOCK_CODE_TYPE.FOR
                ...
                || currBlock.getType() === BLOCK_CODE_TYPE.ELSE || currBlock.getType() === BLOCK_CODE_TYPE.ELIF
                || currBlock.getType() === BLOCK_CODE_TYPE.FOR_ELSE || currBlock.getType() === BLOCK_CODE_TYPE.EXCEPT 
                || currBlock.getType() === BLOCK_CODE_TYPE.FINALLY || currBlock.getType()  === BLOCK_CODE_TYPE.BREAK || currBlock.getType()   === BLOCK_CODE_TYPE.CONTINUE 
                || currBlock.getType()  === BLOCK_CODE_TYPE.PASS) {
                var blockLeftHolderDom = $('<div class="vp-block-left-holder"></div>');
                var blockLeftHolderHeight = currBlock.getTempBlockLeftHolderHeight();
                blockLeftHolderDom.css(STR_HEIGHT,`${blockLeftHolderHeight}px`);
    ...
    Block.prototype.renderBottomOption = function() {
        ...
        switch(type) {
            ...
            case BLOCK_CODE_TYPE.CLASS: {
                ...
                var classNameDom = renderBottomOptionName(that, classNameState, BLOCK_CODE_TYPE.CLASS);
            ...
            case BLOCK_CODE_TYPE.DEF: {
                ...
                var defNameDom = renderBottomOptionName(that,defName, BLOCK_CODE_TYPE.DEF);
            ...
            case BLOCK_CODE_TYPE.IF: {
                ...
                var defaultOrDetailButton = renderDefaultOrDetailButton(that, uuid, BLOCK_CODE_TYPE.IF);

                /* --------------------------- if ----------------------------- */
                var ifTitle = renderBottomOptionTitle(STR_IF);
                var ifNameDom = renderBottomOptionName(that, that.getState(STATE_ifCodeLine), BLOCK_CODE_TYPE.IF);
                ...
                $(`.vp-nodeeditor-elif-plus-btn`).click(function() {
                    ...
                    // rootBlock.selectChildBlockList().forEach(block => {
                    //     var width = block.getWidth();
                    //     block.setWidth(width);
                    //     $(block.getMainDom()).css('width', width);
                    // })
                    // var elifBlockWidth = that.getWidth();
                    // newBlock.setWidth(elifBlockWidth);
                    // $(newBlock.getMainDom()).css('width', elifBlockWidth);

                    // that.getNextBlockList().some(block => {
                    //     if (block.getDirection() === BLOCK_DIRECTION.INDENT) {
                    //         var passBlockWidth = block.getWidth();
                    //         $(newBlock.getNullBlock().getMainDom()).css('width', passBlockWidth);
                    //         return true;
                    //     }
                    // });
            ...
            case BLOCK_CODE_TYPE.ELIF: {
                ...
                var elifNameDom = renderBottomOptionName(that, that.getState(STATE_elifCodeLine), BLOCK_CODE_TYPE.ELIF, uuid);
            ...
            case BLOCK_CODE_TYPE.FOR: {
                ...
                var forNameDom = renderBottomOptionName(that, that.getState(STATE_forCodeLine), BLOCK_CODE_TYPE.FOR);
                ...
                $(`.vp-nodeeditor-else-yes-${uuid}`).click(function() {
                    ...
                    if (that.getState('isForElse') === true) {
                        that.renderBottomOption();     
                        return;
                    }
                    that.setState({
                    ...
                    var selectedBlock = that.getLastElifBlock() || that;
                    var blockContainerThis = that.getBlockContainerThis();
                    var newBlock = mapTypeToBlock(blockContainerThis, BLOCK_CODE_TYPE.FOR_ELSE, {pointX: 0, pointY: 0});
                ...
                $(`.vp-nodeeditor-else-no-${uuid}`).click(function() {
                    ...
                    if (that.getState('isForElse') === false) {
                        that.renderBottomOption();     
                        return;
                    }
            ...
            case BLOCK_CODE_TYPE.WHILE: {
                ...
                var whileDom = renderBottomOptionName(that, that.getState(STATE_whileCodeLine), BLOCK_CODE_TYPE.WHILE);
            ...
            case BLOCK_CODE_TYPE.IMPORT: {
                ...
                var flexRow = renderBottomOptionContainer();
                var defaultOrDetailButton = renderDefaultOrDetailButton(that, uuid, BLOCK_CODE_TYPE.IMPORT);
            ...
            case BLOCK_CODE_TYPE.CODE: {
                ...
                var codeDom = renderBottomOptionName(that, that.getState(STATE_customCodeLine), BLOCK_CODE_TYPE.CODE);
    ...
    */
    ...
    Block.prototype.bindDragEvent = function() {
        ...
        $(oldMainDom).draggable({ 
            ...
            start: (event, ui) => {
                ...
                $(newMainDom).css('width', blockWidth);
                $(newMainDom).css(STR_POSITION, STR_ABSOLUTE);
            ...
            drag:(event, ui) => {
                ...
                var blockWidth = that.getWidth();
                x = buttonX - pos2 - $(STR_CSS_CLASS_VP_NODEEDITOR_LEFT).offset().left  - blockWidth / 2;
                ...
                blockList.forEach(block => {
                    ...
                    if ( x < buttonX
                        ...
                        // if ( block.getHolderBlock() ) {
                        //     $( block.getHolderBlock().getMainDom() ).css(STR_DISPLAY, STR_BLOCK);
                        // }
                        // var blockLeftHolderDom = block.getBlockLeftHolderDom();
    ...
    Block.prototype.bindClickEvent = function() {
        ...
        if (blockCodeType === BLOCK_CODE_TYPE.CLASS || blockCodeType === BLOCK_CODE_TYPE.DEF || blockCodeType === BLOCK_CODE_TYPE.RETURN
            ...
            || blockCodeType === BLOCK_CODE_TYPE.ELSE || blockCodeType === BLOCK_CODE_TYPE.ELIF || blockCodeType === BLOCK_CODE_TYPE.FOR_ELSE 
            || blockCodeType === BLOCK_CODE_TYPE.EXCEPT || blockCodeType === BLOCK_CODE_TYPE.FINALLY
            || blockCodeType === BLOCK_CODE_TYPE.BREAK || blockCodeType === BLOCK_CODE_TYPE.CONTINUE || blockCodeType === BLOCK_CODE_TYPE.PASS ) {
    ...
    Block.prototype.renderHolderRadius = function() {
        if (this.getType() === BLOCK_CODE_TYPE.HOLDER) {
            $(this.getMainDom()).removeClass('vp-block-style-border-top-left-radius');
        } else if (this.getHolderBlock() && this.getHolderBlock().getType() === BLOCK_CODE_TYPE.HOLDER) {
    ...
    BLOCK_MAP.set(BLOCK_CODE_TYPE.PROPERTY, Block);
