define([
...
], function (requirejs, $, vpCommon, vpConst, sb, vpFuncJS, libPandas, pdGen, VpCode) {
    // 옵션 속성
    const funcOptProp = {
        stepCount : 1
        , funcName : "Variables"
    ...
    var initOption = function(callback) {
        vpCommon.loadHtml(vpCommon.wrapSelector(vpConst.OPTION_GREEN_ROOM), "file_io/variables.html", optionLoadCallback, callback);
    }
            ...
            varList.forEach(varObj => {
                if (types.includes(varObj.varType) && varObj.varName[0] !== '_') {
                    ...
                    $(tagTdName).click(function() {
                        ...
                        // TEST: vpCode
                        var block = { code: varObj.varName, type: 'var', funcID: that.funcID };
                        vpCode.addBlock(block);
                    });
     ...
     */
    VariablePackage.prototype.generateCode = function(exec) {
