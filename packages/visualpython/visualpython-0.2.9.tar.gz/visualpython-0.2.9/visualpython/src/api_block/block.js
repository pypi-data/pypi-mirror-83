define([
    ...
    Block.prototype.deleteBlockScope = function(isFirst) {
        ...
        if (this.getType() === BLOCK_CODE_TYPE.CODE || this.getType() === BLOCK_CODE_TYPE.IMPORT ||
            this.getType() === BLOCK_CODE_TYPE.PASS || this.getType() === BLOCK_CODE_TYPE.CONTINUE || 
            this.getType() === BLOCK_CODE_TYPE.PROPERTY || this.getType() === BLOCK_CODE_TYPE.BREAK|| 
            this.getType() === BLOCK_CODE_TYPE.RETURN ) {
    ...
    Block.prototype.renderBottomOption = function() {
        ...
        switch(type) {
            ...
            case BLOCK_CODE_TYPE.BREAK: {
                ...
                var codeDom = renderBottomOptionName(that, that.getState(STATE_breakCodeLine), BLOCK_CODE_TYPE.BREAK);
            ...
            case BLOCK_CODE_TYPE.CONTINUE: {
                ...
                var codeDom = renderBottomOptionName(that, that.getState(STATE_continueCodeLine), BLOCK_CODE_TYPE.CONTINUE);
            ...
            case BLOCK_CODE_TYPE.PASS: {
                ...
                var codeDom = renderBottomOptionName(that, that.getState(STATE_passCodeLine), BLOCK_CODE_TYPE.PASS);
            ...
            stop: function(event, ui) {
                ...
                if (isDisappeared === true) {
                    ...
                    if (selectedBlock) {
                        ...
                        var containerDom = document.createElement(STR_DIV);
                        containerDom.classList.add(STR_CSS_CLASS_VP_BLOCK_CONTAINER);
                        // console.log(`x : ${rootBlock.getContainerPointX()}`);
                        // console.log(`y : ${rootBlock.getContainerPointY()}`);
                        $(containerDom).css(STR_TOP, `${rootBlock.getContainerPointY()}` + STR_PX);
                        $(containerDom).css(STR_LEFT, `${rootBlock.getContainerPointX()}` + STR_PX);
                        rootBlock.setContainerDom(containerDom);
                        ...
                        // var rootBlockChildBlockList = rootBlock.selectChildBlockList();
                        // var lastBlock = rootBlockChildBlockList[rootBlockChildBlockList.length - 1];
