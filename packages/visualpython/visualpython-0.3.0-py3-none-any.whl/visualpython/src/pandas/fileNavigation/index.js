define([
    ...
    var vp_bindEventFunctions = function() {
        ...
        $('.directoryComponent-btn-up').click(() => {
            /** 나의 현재 path가 Jupyternotebook path와 동일할 경우  */
            if ( fileNavigationState.getCurrentDir() === fileNavigationState.getNotebookDir() ) {
                vpCommon.renderAlertModal('기본 디렉토리보다 상위로 검색할 수 없습니다');
                return;
            }

            var currentDirStr = fileNavigationState.getCurrentDir();
            var index = currentDirStr.length;
            var upDirStr = ``;

            /** 한 depth 상위의 path를 만든다 */
            while (index-- > 0) {
                if ( currentDirStr[index] === "/" ) {
                    upDirStr = currentDirStr.substring(0, index);
                    break;
                }
            }
        ...
        $('.directoryComponent-btn-prev').click(() => {
            if (fileNavigationState.getDirHistoryStack().length === 0) {
                vpCommon.renderAlertModal('이전 디렉토리 검색 목록이 없습니다');
                return;
            }
