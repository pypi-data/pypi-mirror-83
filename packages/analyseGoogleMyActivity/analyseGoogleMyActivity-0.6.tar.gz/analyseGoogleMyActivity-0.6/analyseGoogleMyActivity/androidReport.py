...
FileFullPath=''
ReportName=FileFullPath+'Android-Activity'+'-'+datetime.now().strftime("-%m-%d-%H%M")+'/'
...
# Gives the Passed Arguments, Pass the Optional Arguments in dict (or string if single) type as customarg
def getArg(usagestr='', customarg=''): # Takes Doc String & Default Arguments
    args = sys.argv[1:] # Exclude Filename from obtained Arguments list
    if len(args) < 1:
        print(usagestr)
        return {}
    else:
        validarg={} ; validarg['main']=args[0]
        if len(args) > 2:
            if customarg:
                if isinstance(customarg,str): # If string is passed to getArg 
                    if args[1] == customarg.replace('-','') :
                        return { 'main': args[0], customarg : args[2] }
                else:
                    for i in range(1,len(args)-1): # Excluding First Arg.
                        if args[i].replace('-','') in list(customarg.keys()):
                            if args[i+1].replace('-','') not in list(customarg.keys()):
                                validarg[args[i].replace('-','')]=args[i+1]
                            else:
                                print("   - No Value Given for the Argument: '"+args[i]+"'")
                                sys.exit()
                    customarg.update(validarg)
                    return customarg
        else:
            if isinstance(customarg,dict):
                customarg.update(validarg)
                return customarg
            return validarg
...
def TopApps(AppUsage, n=0, yearly=0):
    ...
    if yearly:
        year=AppUsage.iloc[0,1].year  # datetime.now().year
        ApUsgYrChange = [0]
...
def appUsageData(topApps, AppUsage, Yearly=0, All=0): # Works on AppUsage DataFrame
    ...
    year=AppUsage.iloc[0,1].year; month=AppUsage.iloc[0,1].month
...
def SleepAnalysis(Sleep):
    ...
    global Verbose
    #year=datetime.now().year; month=datetime.now().month
    year=list(Sleep.values())[0][1].year; month=list(Sleep.values())[0][1].month
    YrChange = []; MthChange = []
    ...
    for n in range(len(YrChange)-1):
        ...
        if len(xdata)>1:
            ...
            SleepYearlyMeanDF.columns.name = 'Year (days)'
    ...
    for n in range(len(MthChange)-1):
        ...
        if len(xdata)>1:
            ...
            SleepMonthlyMeanDF[Year].columns.name = 'Month (days)'
...
def barplot(xdata, ydata, plotName, yearly=0):
    ...
    global IdlSleepT,redMarkSleepT, ReportName
    ...
    col = [bGreen  if i>=IdlSleepT  else bYellow if i>redMarkSleepT else bRed  for i in ydata]
    ...
    if yearly: # To plot yearly data
        ...
        col = [bGreen  if round(i,1)>IdlSleepT  else bYellow if round(i,1)>redMarkSleepT else bRed for i in ydata]
    ...
    sliceLabel = [ 'Sleep > '+str(IdlSleepT)+' Hrs.', 'Sleep < '+str(redMarkSleepT)+' Hrs.', 'Sleep b/w '+str(redMarkSleepT)+' & '+str(IdlSleepT)+' Hrs.'] # Label for Each Slice
    ...
    '''
    ## Add Custom Legends
    legend_dict = { 'Sleep > '+str(IdlSleepT)+' Hrs. : '+str(round((col.count(bGreen)/len(col)*100),2))+'%' : bGreen, 'Sleep < '+str(redMarkSleepT)+' Hrs. : '+str(round((col.count(bRed)/len(col)*100),2))+'%' : bRed, 'Sleep b/w '+str(redMarkSleepT)+' & '+str(IdlSleepT)+' Hrs. : '+str(round((col.count(bYellow)/len(col)*100),2))+'%' : bYellow }
...
def generateSleepReportHTML(SleepYearlyMeanDF,SleepMonthlyMeanDF, barplots, scatterplots):
    ...
    print("\n   -- Opening '"+sleepFile+'" ..')
    os.system('start CHROME.EXE "'+os.getcwd()+'/'+sleepFile+'"')
    return sleepFile
...
######### Main Program ########
def androidReport(file='MyActivity.json', apps=12, timezone='in', excludeapps=['com.miui.home' ], idealsleeptime=8, redmarksleeptime=5.5, inlineimg=1, showmarkerday=0, output=1, yeartabs=1 ,verbose=0):
    '''
    ...
    apps : int or list , optional
        No. of Top Apps or List of Apps to find usage for. The default is 12. Larger number takes more processing time.
    timezone : str
    ...
    '''
    global TopAppsN, IdlSleepT, redMarkSleepT, Timezone, showMarkerday, InlineImg, Verbose
    TopAppsN=apps # Top Apps to Get App Usage for
    
    IdlSleepT=idealsleeptime
    redMarkSleepT=redmarksleeptime
    Timezone=timezone
    ...
    # Get Passed File Full Path
    global FileFullPath
    FileFullPath=os.path.abspath(file).replace(file,'')
...
def androidreportcmd():
    ...
    global TopAppsN, IdlSleepT, redMarkSleepT, Timezone, showMarkerday, InlineImg, Verbose
    customarg = { 'apps':TopAppsN, 'timezone':Timezone, 'excludeapps':['com.miui.home' ], 'idealsleeptime':IdlSleepT, 'redmarksleeptime':redMarkSleepT, 'inlineimg':InlineImg, 'showmarkerday':showMarkerday, 'output':1, 'yeartabs':1 ,'verbose':Verbose  }
    argDict=getArg(usagestr=Usgstr, customarg=customarg)
    if argDict:
        file=argDict['main']
        androidReport( file=file, apps=int(argDict['apps']), timezone=argDict['timezone'], excludeapps=argDict['excludeapps'], idealsleeptime=int(argDict['idealsleeptime']), redmarksleeptime=int(argDict['redmarksleeptime']), inlineimg=int(argDict['inlineimg']), showmarkerday=int(argDict['showmarkerday']), output=int(argDict['output']), yeartabs=int(argDict['yeartabs']) ,verbose=int(argDict['verbose']) )
...
if __name__ == '__main__' :
    reports = androidReport(file='E:/PY/activity/sib/takeout-20201001T044940Z-0011.zip', apps=20)
