...
class AzureAutoMLRunContext(AutoMLAbstractRunContext):
    ...
    def _get_run_internal(self) -> Run:
        ...
        if not isinstance(self._run, AutoMLRun) and isinstance(self._run, Run):
            return AutoMLRun(self._run.experiment, self._run.id, _run_dto=self._run._client.run_dto)
        return self._run
