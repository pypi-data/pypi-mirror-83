...
from azureml.train.automl.runtime.automl_explain_utilities import AutoMLExplainerSetupClass, \
    MaximumEvaluationSamples, automl_check_model_if_explainable, _get_feature_map, \
    _get_estimator_non_streaming, _automl_pick_surrogate_model_and_set_params
from azureml.train.automl.runtime._automl_model_explain.automl_model_explain_helper import \
    _should_compute_raw_explanations, _automl_pick_evaluation_samples_explanations, \
...
class AutoMLModelExplainDriver:
    """
    This is class implements the model explain data and configuration for explaining an AutoML model.
    ...
    setup_class_labels(), setup_surrogate_model_and_params() and setup_model_explain_data(). The
    functions log_explanation_data_stats() and validate_model_explain_driver() are static functions
    to log the explain data stats and validate the explain data.
    """
    def __init__(self, automl_child_run: Run, dataset: DatasetBase,
                 max_cores_per_iteration: int, model_exp_workspace_config: Optional[Dict[str, Any]] = None,
        ...
        self._should_explain_raw_features = _should_compute_raw_explanations(model_exp_workspace_config)
    ...
    def setup_model_explain_metadata(self) -> None:
        ...
        if self._should_explain_raw_features:
    ...
    def validate_model_explain_driver(explain_driver: Any) -> None:
        ...
        if explain_driver._should_explain_raw_features:
