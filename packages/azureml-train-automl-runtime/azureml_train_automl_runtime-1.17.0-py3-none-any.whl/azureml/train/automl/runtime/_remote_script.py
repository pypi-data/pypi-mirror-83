...
from azureml.train.automl.runtime._automl_job_phases import (
    FeaturizationPhase, PhaseUtil, SetupPhase, TrainingIteration, TrainingPhase)
...
from azureml.train.automl.runtime._automl_model_explain import _automl_auto_mode_explain_model, \
    _automl_perform_best_run_explain_model, AutoMLModelExplainDriverFactory, AutoMLModelExplainTelemetryStrings
...
from ._remote.remote_utilities import (
    get_automl_run_from_context, _get_parent_run_id, _init_directory, _parse_settings)
...
def _get_feature_configs(parent_run_id: str, automl_settings_obj: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        jasmine_client = JasmineClient(experiment.workspace.service_context,
                                       experiment.name,
                                       experiment.id,
        ...
        if not child_run_id:
            ...
            with dataset.open_dataset():
                ...
                automl_model_explain_driver = AutoMLModelExplainDriverFactory._get_model_explain_driver(
                    ...
                    model_exp_workspace_config=feature_configs.get(MODEL_EXPLAINABILITY_ID))
                _automl_auto_mode_explain_model(automl_model_explain_driver=automl_model_explain_driver)
    except Exception as e:
        if not child_run_id:
            logger.info(AutoMLModelExplainTelemetryStrings.REMOTE_BEST_RUN_MODEL_EXPLAIN_ERROR_STR)
        else:
            logger.info(AutoMLModelExplainTelemetryStrings.REMOTE_ON_DEMAND_RUN_MODEL_EXPLAIN_ERROR_STR)
...
def batch_driver_wrapper(
    ...
    """
    batch_job_run = get_automl_run_from_context()  # current batch job context
    ...
    try:
        ...
        logger.info("Error in batch_driver_wrapper.")
        batch_job_run._fail_with_error(e)
